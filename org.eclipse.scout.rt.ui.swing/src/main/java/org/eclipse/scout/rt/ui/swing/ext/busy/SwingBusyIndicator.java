...
public class SwingBusyIndicator {
   ...
   */
  public void startBlocking(RunMonitor monitor) {
    setBlocking(monitor);
  }
  ...
  private void setBlocking(final RunMonitor monitor) {
    SwingUtilities.invokeLater(
        new Runnable() {
          @Override
          public void run() {
  ...
  private void setBlocking0(RootPaneContainer r, RunMonitor monitor) {
    JRootPane rootPane = r.getRootPane();
    if (rootPane == null) {
      return;
    }
  ...
  private static class BusyGlassPane extends JPanel {
    ...
    public void block(RunMonitor monitor) {
      m_monitor = monitor;
