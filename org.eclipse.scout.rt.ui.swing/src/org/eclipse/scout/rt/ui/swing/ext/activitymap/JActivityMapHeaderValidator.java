...
class JActivityMapHeaderValidator {
   ...
   */
  List<Rectangle> calculateTextSizeRectangles(Object[] columns, ColumnType columnType, ActivityMapColumnModel columnModel, ArrayList<Rectangle> textRects, FontMetrics fm, ArrayList<String> texts) {
  ...
  List<Rectangle> mergeTexts(Map<Integer, ValidatedTextData> validatedTextDataMap, List<String> texts) {
    ...
    for (int i = 0; i < largeTexts.size(); i++) {
      ...
      if (!addTextAndRectangleIfFit(largeTextData, i, texts, validatedRectangles)) {
        if (!addTextAndRectangleIfFit(mediumTextData, i, texts, validatedRectangles)) {
