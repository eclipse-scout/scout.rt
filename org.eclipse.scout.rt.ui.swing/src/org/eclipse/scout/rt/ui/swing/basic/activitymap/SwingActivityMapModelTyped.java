...
/**
 * In Scout 3.9 SwingActivityMapModel will be typed and this class will be removed.
 */
public class SwingActivityMapModelTyped implements ActivityMapModel {
  private final IActivityMapTyped m_map;
  private final JTable m_metricsTable;
  private final SwingActivityComponentTyped[] m_components;

  @SuppressWarnings("unchecked")
  public SwingActivityMapModelTyped(IActivityMapTyped map, JTable metricsTable) {
    m_map = map;
    m_metricsTable = metricsTable;
    Object[] resourceIds = map.getResourceIds();
    ArrayList<SwingActivityComponentTyped> list = new ArrayList<SwingActivityComponentTyped>();
    for (int i = 0; i < resourceIds.length; i++) {
      for (ActivityCellTyped cell : map.getActivityCells(resourceIds[i])) {
        list.add(new SwingActivityComponentTyped(cell, i));
      }
    }
    m_components = list.toArray(new SwingActivityComponentTyped[list.size()]);
  }
  ...
  public double[] getActivityRange(ActivityComponent a) {
    SwingActivityComponentTyped sa = (SwingActivityComponentTyped) a;
    ActivityCellTyped cell = sa.getScoutActivityCell();
