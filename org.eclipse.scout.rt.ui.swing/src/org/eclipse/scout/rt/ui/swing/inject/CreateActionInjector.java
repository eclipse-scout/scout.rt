...
import org.eclipse.scout.rt.ui.swing.action.SwingScoutActionButton;
 ...
 *
 */
public class CreateActionInjector {
  private static final IScoutLogger LOG = ScoutLogManager.getLogger(CreateActionInjector.class);

  public CreateActionInjector() {
  }

  public ISwingScoutAction<?> inject(ISwingEnvironment env, JComponent parent, IAction action) {
    if (action instanceof IToolButton) {
      ISwingScoutAction<IToolButton> ui = new SwingScoutActionButton<IToolButton>();
      ui.createField((IToolButton) action, env);
      return ui;
    }
    else if (action instanceof IViewButton) {
      ISwingScoutAction<IViewButton> ui = new SwingScoutActionButton<IViewButton>();
      ui.createField((IViewButton) action, env);
      return ui;
    }
