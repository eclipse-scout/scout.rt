...
class ConcurrencyQueue {
  ...
  public void add(Runnable r) {
    if (r != null) {
      synchronized (m_listLock) {
        if (LOG.isInfoEnabled()) LOG.info(m_infoCached + " add: " + r);
        m_list.add(r);
        updateInternalInfo();
        m_listLock.notifyAll();
      }
  ...
  public void dispatchNext() {
    ...
    if (r != null) {
      try {
        ...
        if (LOG.isInfoEnabled()) LOG.info(m_infoCached + " dispatchStart: " + r);
        r.run();
        if (LOG.isInfoEnabled()) LOG.info(m_infoCached + " dispatchEnd " + (System.currentTimeMillis() - startTime) + " ms:   " + r);
      }
      catch (Throwable t) {
        LOG.error(null, t);
      }
