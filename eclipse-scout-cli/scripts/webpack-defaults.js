...
module.exports = (env, args) => {
  ...
  let typeCheck = nvl(args.typeCheck, 'auto');
  ...
  let transpileOnly;
  if (typeCheck === 'auto') {
    // In prod mode always only transpile (no type-checks).
    // In dev mode type checking is skipped in watch mode. Instead, ForkTsCheckerWebpackPlugin is used then (see below).
    transpileOnly = !devMode || isWatchMode;
  } else if (typeCheck === 'fork') {
    transpileOnly = true;
  } else {
    transpileOnly = !typeCheck;
  }
  console.log('TypeScript transpileOnly: ' + transpileOnly);
  const tsOptions = {
    ...args.tsOptions,
  ...
  if (typeCheck === 'auto') {
    // devMode and no type-checks: perform checks asynchronously (watch mode).
    typeCheck = devMode && transpileOnly;
  }
  if (typeCheck === 'fork') {
