...
module.exports = class DataObjectTransformer {
  ...
  transform(node) {
    ...
    if (ts.isClassDeclaration(node)) {
      ...
      if (typeNameDecorator) {
        ...
        const namespace = this._detectNamespaceFor(node); // detectModuleOf() will not find anything because a ClassDeclaration node is passed here. But this is fine as the namespace of the own module is required here.
        this.doRegistryAddStatements.push(this._createDoInventoryAddStatement(className, typeName, namespace));
  ...
  _createTypeReferenceNode(node) {
    ...
    const qualifiedName = this._createObjectType(namespace, name);
  ...
  _createDoInventoryAddStatement(className, typeName, namespace) {
    ...
    const doRegistry = ts.factory.createElementAccessExpression(scout, ts.factory.createStringLiteral('DataObjectInventory'));
