...
export class LogicalGridLayoutInfo implements LogicalGridLayoutInfoModel {
    ...
    for (let i = 0; i < compCount; i++) {
      ...
      if (cons.gridw === 1) {
        ...
        for (let j = cons.gridx; j < cons.gridx + cons.gridw && j < this.cols; j++) {
          ...
          minWidths[j] = Math.max(minWidths[j], cons.minWidth);
   ...
   * @param usedCols the columns that are used by a grid data ignoring spans (=the start of a new grid data).
   *   If specified, only these columns are considered for the width distribution
   */
  protected _distribute<T>(distWidth: number, spanWidth: number, widths: number[], cons: LogicalGridData, fixedWidths: boolean[], calc: (equalWidth: number, width: number) => number, usedCols?: boolean[]) {
    // Specifies how many cols in the span range (grid.w) are used
    let emptySpan = !usedCols ? 0 : usedCols
      .slice(cons.gridx, cons.gridx + cons.gridw)
      .filter(used => !used).length;
    let hSpan = cons.gridw - emptySpan;
    let equalWidth = Math.floor((distWidth + spanWidth) / hSpan);
    let remainder = (distWidth + spanWidth) % hSpan;
    let last = -1;
    for (let j = cons.gridx; j < cons.gridx + cons.gridw && j < this.cols; j++) {
      last = j;
      if (!fixedWidths[j] && (!usedCols || usedCols[j])) {
        widths[j] = calc(equalWidth, widths[j]);
      }
      if (cons.weightx === 0) {
        fixedWidths[j] = true; // TODO CGU can this be moved outside?
      }
    }
    if (last > -1) {
  ...
  protected _initializeRows() {
    ...
    let minHeights = arrays.init(this.rows, 0);
    ...
    for (let i = 0; i < compCount; i++) {
      ...
      if (cons.gridh === 1) {
        ...
        for (let j = cons.gridy; j < cons.gridy + cons.gridh && j < this.rows; j++) {
          ...
          minHeights[j] = Math.max(minHeights[j], cons.minHeight);
    ...
    for (let i = 0; i < compCount; i++) {
      ...
      if (cons.gridh > 1) {
        ...
        if (cons.heightHint > 0) {
          distHeight = cons.heightHint - spanHeight - vGaps;
        } else if (cons.useUiHeight) {
          distHeight = compSize[i].height - spanHeight - vGaps;
        } else {
          distHeight = this.logicalHeightInPixel(cons) - spanHeight - vGaps;
        }
        if (distHeight > 0) {
          let equalHeight = Math.floor((distHeight + spanHeight) / vSpan);
          let remainder = (distHeight + spanHeight) % vSpan;
          let equalMinHeight = Math.floor((cons.minHeight - vGaps) / vSpan);
          let remainderMinHeight = (cons.minHeight - vGaps) % vSpan;
          let equalMaxHeight = Math.floor((cons.maxHeight - vGaps) / vSpan);
          let remainderMaxHeight = (cons.maxHeight - vGaps) % vSpan;
          let last = -1;
          for (let j = cons.gridy; j < cons.gridy + cons.gridh && j < this.rows; j++) {
            last = j;
            if (!fixedHeights[j]) {
              prefHeights[j] = Math.max(equalHeight, prefHeights[j]);
              minHeights[j] = Math.max(equalMinHeight, minHeights[j]);
              maxHeights[j] = Math.min(equalMaxHeight, maxHeights[j]);
            }
            if (cons.weighty === 0) {
              fixedHeights[j] = true;
            }
          ...
          if (last > -1) {
            ...
            minHeights[last] += remainderMinHeight;
            maxHeights[last] += remainderMaxHeight;
          }
        }
      }
    }
    ...
    for (let i = 0; i < this.rows; i++) {
      ...
      if (fixedHeights[i]) {
        ...
        this.height[i][lc.MIN] = minHeights[i];
        this.height[i][lc.PREF] = prefHeights[i];
  ...
  layoutCellBounds(size: Dimension, insets: Insets): Rectangle[][] {
    ...
    for (r = 0; r < this.rows; r++) {
      ...
      for (c = 0; c < this.cols; c++) {
        ...
        x += w[c];
        if (w[c + 1] === 0) {
          // Do not add gap if column width is reduced to 0
          // This is mainly necessary for columns to reach their configured minWidth
          continue;
        }
        x += this.hgap;
      }
