 ...
 */
import {arrays, BatchCall, BatchCallResult, Cell, ColumnValidationResult, InitModelOf, LookupBox, LookupCallColumn, LookupColumnEventMap, LookupColumnModel, LookupEditor, scout, Status, strings, TableRow, ValueField} from '../../index';
...
export class LookupColumn<TValue> extends LookupCallColumn<TValue[], TValue> implements LookupColumnModel<TValue> {
  ...
  protected _batchFormatValues(keys: TValue[][]): JQuery.Promise<BatchCallResult<TValue[], string>> {
    ...
    return LookupEditor.formatValues(allKeys, this.lookupCall, lookupCall => this.trigger('prepareLookupCall', {lookupCall})).then(rows => {
      ...
      return new Map(keys.map(k => [k, strings.join(', ', ...k.map(e => rowsByKey.get(e)?.text))]));
    });
  }
  ...
  protected override _formatValue(value: TValue[], row?: TableRow): string | JQuery.Promise<BatchCallResult<TValue[], unknown>> {
    ...
    return this._prepareAndExecuteLookupCall(this.lookupCall.cloneForKeys(value), row);
  }
