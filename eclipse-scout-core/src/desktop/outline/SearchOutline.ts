 ...
 */
import {
  aria, Event, InitModelOf, InputFieldKeyStrokeContext, keys, KeyStrokeContext, keyStrokeModifier, Outline, Page, PropertyChangeEvent, SearchOutlineEventMap, SearchOutlineLayout, SearchOutlineModel, SearchState,
  TreeAllChildNodesDeletedEvent, TreeNodesDeletedEvent, TreeNodesInsertedEvent
} from '../../index';
import $ from 'jquery';
...
export class SearchOutline extends Outline implements SearchOutlineModel {
  ...
  protected _setResultCount(resultCount: number) {
    this._setProperty('resultCount', resultCount);
  ...
  protected _setLimited(limited: number) {
    this._setProperty('limited', limited);
