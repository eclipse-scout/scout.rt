...
export abstract class NavigateButton extends Menu implements NavigateButtonModel {
  ...
  /**
   * Toggles the visibility of the detail form according to {@link _toggleDetail}. Only relevant when {@link _isDetail} is `true`.
   */
  ...
  /**
   * @returns `true` if the page as a detail form and the detail form is marked as visible.
   */
  ...
  /**
   * @returns `true` if the page as a detail table and the detail table is marked as visible and the detail table contains at least one row.
   */
  protected _hasDetailTable(): boolean {
    return this.node.detailTableVisible && !!this.node.detailTable && this.node.detailTable.rows.length > 0;
  }
   ...
   * the button does {@link _drill drill} up or down like normal, but only changes the displayed "detail content".
   */
  ...
  /**
   * Specifies the new value of {@link Page#detailFormVisibleByUi} when this button is clicked. Only relevant when {@link _isDetail} is `true`.
   */
