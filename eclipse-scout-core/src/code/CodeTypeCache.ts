 ...
 */
import {App, arrays, CodeType, CodeTypeCacheEventMap, CodeTypeChangeEvent, CodeTypeRemoveEvent, DoEntity, EventEmitter, ObjectFactory, ObjectModel, ObjectOrModel, scout, systems, UiNotificationEvent, uiNotifications} from '../index';
...
export let codes: CodeTypeCache;

// use app listener to create the codes variable so that it can be overwritten using the ObjectFactory (which is only initialized after prepare phase of the App).
App.addListener('prepare', e => {
  codes = scout.create(CodeTypeCache);
  ObjectFactory.get().registerNamespace('scout', {codes});
});
