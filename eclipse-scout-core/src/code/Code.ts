...
export class Code<TCodeId> implements ObjectWithType {
  ...
  init(model: InitModelOf<this>) {
    ...
    this.active = scout.nvl(model.active, true);
    this.enabled = scout.nvl(model.enabled, true);
    ...
    this.partitionId = scout.nvl(model.partitionId, 0);
    this.sortCode = scout.nvl(model.sortCode, -1);
