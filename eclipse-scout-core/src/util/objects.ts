...
export const objects = {
   ...
   * * `Map`
   * * `Set`
   * * All objects having a `clone` function (expected to create a deep clone without taking any arguments). These are classes like `BaseDoEntity`, `Dimension`, `GridData`, `Insets`, `Point`, `Status`, `URL`, ...
  ...
  /**
   * Recursively copies the contents of two or more objects together into the first object.
   *
   * The implementation creates deep clones for the following types:
   * * Pojo (properties are merged if existing in multiple objects)
   * * `Array`
   * * `Date`
   * * `Map`
   * * `Set`
   * * All objects having a `clone` function (expected to create a deep clone without taking any arguments). These are classes like `BaseDoEntity`, `Dimension`, `GridData`, `Insets`, `Point`, `Status`, `URL`, ...
   *
   * All unsupported other types are *NOT* re-constructed and will be copied over as is. So the result points to the same instance as the source.
   * Undefined properties are not copied, while properties with null value are.
   *
   * Keep in mind that the target object (first argument) will be modified, and will also be returned.
   * Use `objects.extend({}, objA, objB, objC)` if source objects should stay untouched.
   *
   * The difference to `Object.assign` is that this implementation copies recursively.
   * The difference to `$.extend(true, ...)` is that this implementation does not merge arrays but replaces them. Furthermore, this implementation supports creating copies for more types (see above).
   *
   * @param objs The objects to merge (vararg). Null or undefined objects are ignored.
   * @returns the first object that was passed to this function with the other objects copied into. If no object or only null/undefined objects are passed, an empty pojo is returned.
   */
  extend(...objs: object[]): any {
    if (!objs?.length) {
      return {};
    }
    const result = objs[0] || {};
    for (let i = 1; i < objs.length; i++) {
      ...
      for (const [k, v] of Object.entries(objs[i])) {
        if (objects.isPojo(v)) {
          result[k] = objects.extend({}, result[k], v);
        } else if (v !== undefined) {
          result[k] = deepCloneClass(v); // create deep clones for classes without merging anything
        }
      }
    }
    return result;
 ...
 * * `Map`
 * * `Set`
 * * All objects having a `clone` function (expected to create a deep clone without taking any arguments).
