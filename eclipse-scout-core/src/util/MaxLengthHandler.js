...
export default class MaxLengthHandler {
  ...
  install($textInputField) {
    ...
    if ($textInputField) {
      ...
      this.$textInputField.on('paste', this._onInputFieldPaste.bind(this));
    }
  }

  uninstall() {
    if (this.$textInputField) {
      this.$textInputField.off('paste', this._onInputFieldPaste.bind(this));
    }
  }
  ...
  render() {
    ...
    if (this.$textInputField[0].maxLength) {
      this.$textInputField.attr('maxlength', this.target.maxLength);
    } else {
      // Fallback for IE9
      this.$textInputField.on('keyup paste', e => {
