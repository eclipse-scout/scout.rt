...
export class PlainTextEncoder {
  ...
  constructor() {
    this._cachedElement = new CachedElement('textarea');
  }
  ...
  encode(text: string, options?: PlainTextEncoderOptions): string {
    ...
    // It is not possible to use jquery's text() function or to create a html element and use textContent/innerText, because the new lines get omitted.
