...
export const focusUtils = {
  ...
  isFocusableByMouse(element: HTMLElement | JQuery): boolean {
    return !$.ensure(element).closest('.unfocusable').length;
  },

  /**
   * @returns whether the element must not gain focus even if it has a tabindex. This is only true for elements with tabindex="-2".
   */
  ...
  /**
   * @param nativeFocusable true to include elements that should actually not be focusable but would gain the focus by the browser if we did not prevent it (elements with tabindex="-2"). Default is false.
  ...
  focusLater(target: HTMLElement | JQuery, options?: FocusOptions) {
    ...
    requestAnimationFrame(() => {
      ...
      // behavior to prevent cancelling other events, such as 'dragstart').
      let focusedElement = doc.activeElement as HTMLElement;
