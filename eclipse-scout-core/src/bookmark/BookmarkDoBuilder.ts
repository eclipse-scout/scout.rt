 ...
 */
import {
  arrays, BookmarkDo, BookmarkDoBuilderModel, BookmarkDoBuilderOptionsDo, BookmarkSupport, BookmarkTableRowIdentifierDo, ChartTableControlConfigDo, Desktop, HybridActionContextElement, HybridActionContextElements, HybridManager,
  IBookmarkDefinitionDo, IBookmarkDo, IBookmarkPageDo, InitModelOf, NodeBookmarkPageDo, objects, ObjectWithType, OutlineBookmarkDefinitionDo, Page, PageBookmarkDefinitionDo, PageWithTable, scout, Session, strings, TableBookmarkPageDo,
  ...
  protected async _createBookmarkDescription(bookmarkDefinition: IBookmarkDefinitionDo): Promise<string> {
    ...
    pagePath.forEach((bookmarkPage, index) => {
      ...
      if (bookmarkPage instanceof TableBookmarkPageDo && bookmarkPage.searchData) {
        let searchData = bookmarkPage.searchData.toPojo();
