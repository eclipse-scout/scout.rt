...
export const config = {
  ...
  /**
   * Add the {@link ConfigProperty} instances given to the system given. If a property with the same key already exists, this method does nothing.
  ...
  _init(property: ConfigProperty<any>, system?: string) {
    ...
    if (existingProperty) {
      if (!objects.equalsRecursive(existingProperty.value, property.value)) {
        $.log.warn(`Already existing config property '${existingProperty.key}' with existing value ${JSON.stringify(existingProperty.value)}.\nIgnoring new value ${JSON.stringify(property.value)}.`);
      }
  ...
  /**
   * Loads the properties from the system given and returns the property with given key.
   * <p>
