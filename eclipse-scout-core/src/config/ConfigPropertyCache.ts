 ...
 */
import {App, arrays, ConfigProperties, DoEntity, ObjectFactory, ObjectModel, objects, scout, System, systems} from '../index';
import $ from 'jquery';
...
export let config: ConfigPropertyCache;

// use app listener to create the config variable so that it can be overwritten using the ObjectFactory (which is only initialized after prepare phase of the App).
App.addListener('prepare', e => {
  config = scout.create(ConfigPropertyCache);
  ObjectFactory.get().registerNamespace('scout', {config});
});
