 ...
 */
import {
  AbstractLayout, Action, aria, arrays, ContextMenuPopupLayout, ContextMenuPopupModel, Event, graphics, HtmlComponent, InitModelOf, Menu, MenuDestinations, MenuFilter, menuNavigationKeyStrokes, ObjectUuidProvider, Popup, PopupLayout,
    ...
    menus.forEach((menu: Menu & { __originalParent?: Menu }) => {
      ...
      if (this.cloneMenuItems && !menu.cloneOf || !this.has(menu)) {
        menu = menu.clone({
          ...
          uuid: ObjectUuidProvider.get().createDependentUuid('cmi', menu) // cmi: context-menu-item
        }, {
