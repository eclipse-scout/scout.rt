...
import {InputFieldKeyStrokeContext, keys, KeyStrokeContext, keyStrokeModifier, scout, VirtualKeyStrokeEvent} from '../index';
import $ from 'jquery';
 ...
 */
export default class IFrameKeyStrokeContext extends KeyStrokeContext {
  ...
  _applyPropagationFlags(event) {
    ...
    if (inputKeyStrokeContext.isInput($target)) {
      let vrEvent = new VirtualKeyStrokeEvent(event.which, event.ctrlKey, event.altKey, event.shiftKey, event.type, $target[0]);
   ...
   */
  _resolveTarget(event) {
    return event.originalEvent?.sourceEvent?.target;
  }
}
