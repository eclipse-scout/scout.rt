 ...
 */
import {
  Action, DropType, FormField, FormFieldCheckEmpty, FormFieldLabelPosition, FormFieldStatusPosition, FormFieldStyle, FormFieldSuppressStatus, FormFieldTooltipAnchor, FormFieldValidationResultProvider, GridData, Menu, ObjectOrChildModel,
  ObjectOrModel, ObjectOrType, StatusMenuMapping, StatusOrModel, TooltipSupportOptions, WidgetModel
} from '../../index';
...
export interface FormFieldModel extends WidgetModel {
   ...
   *
   * The property also accepts a function that is called when the {@link FormField.empty} property is computed.
   * If a function is provided, the default implementation {@link FormField._computeEmpty} won't be called anymore but passed as parameter instead.
   * Depending on your implementation, it may be necessary to call {@link updateEmpty} whenever a condition changes that you use in your empty computation.
   *
   * Default is true.
   */
