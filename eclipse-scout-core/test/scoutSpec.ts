 ...
 */
import {HorizontalGrid, Menu, NullWidget, NumberField, ObjectIdProvider, objects, scout, Session, Status, StringField, Tooltip, ValueField, Widget} from '../src/index';
  ...
  describe('create', () => {
    ...
    describe('local object', () => {
      ...
      it('sets property \'id\' correctly when no ID is provided', () => {
        let expectedSeqNo = ObjectIdProvider.get().uiSeqNo + 1,
          menu = scout.create(Menu, {
            parent: new NullWidget(),
            session: session
          });
        expect(menu.id).toBe(ObjectIdProvider.UI_SEQ_ID_PREFIX + expectedSeqNo.toString());
        expect(ObjectIdProvider.get().uiSeqNo).toBe(expectedSeqNo);
      });
