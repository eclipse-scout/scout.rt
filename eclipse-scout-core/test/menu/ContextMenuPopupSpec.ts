 ...
 */
import {ContextMenuPopup, keys, Menu, ObjectIdProvider, Popup, scout, Widget} from '../../src/index';
...
describe('ContextMenuPopup', () => {
  ...
  describe('uuid', () => {
    it('of menu items is set to cloned menu items', () => {
      ...
      contextMenu.render();
      expect(findClone(contextMenu, menuItem1).uuid).toBe(`cmi${ObjectIdProvider.DEPENDENT_UUID_DELIMITER}1`);
      expect(findClone(contextMenu, menuItem2).uuid).toBe(`cmi${ObjectIdProvider.DEPENDENT_UUID_DELIMITER}MyMenu`);
    });
  });
});
