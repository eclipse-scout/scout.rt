 ...
 */
import {HtmlComponent, ObjectIdProvider, scout, TabBox, TabItem} from '../../../../src/index';
...
describe('TabItem', () => {
  ...
  describe('uuid', () => {
    it('of tab item is set to the tab', () => {
      ...
      expect(tabBox.header.tabArea.tabs[0].uuid).toBe(`tab${ObjectIdProvider.DEPENDENT_UUID_DELIMITER}1`);
      expect(tabBox.header.tabArea.tabs[1].uuid).toBe(`tab${ObjectIdProvider.DEPENDENT_UUID_DELIMITER}2`);
    });
  });
});
