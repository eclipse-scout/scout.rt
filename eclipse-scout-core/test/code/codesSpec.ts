...
describe('codes', () => {
  ...
  describe('add', () => {
    it('adds a code type or an array of code types and removes it afterwards', () => {
      let origSize = codes.getCodeTypeCache().registry.size;
      ...
      expect(codes.getCodeTypeCache().registry.size).toBe(origSize + 1);

      // cleanup
      codes.remove('codeType.123');
      expect(codes.getCodeTypeCache().registry.size).toBe(origSize);
    });
  });
});
