...
import {BaseDoEntity, DataObjectInventory, dataObjects, dates, Id, IdParser, NumberId, ObjectFactory, scout, StringId, typeName, UuId} from '../../src/index';
...
describe('MapDoNodeSerializer', () => {
  ...
  it('map keys are handles correctly', () => {
    ...
    IdParser.get().registerIdTypeName('TestNumId', NumberId);
    IdParser.get().registerIdTypeName('TestUuId', UuId);
    ...
    IdParser.get().removeIdTypeName('TestNumId');
    IdParser.get().removeIdTypeName('TestUuId');
  });
});
