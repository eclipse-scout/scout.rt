...
public class JsonTableTest {
  ...
  public void testUserRowFilter_UpdateEvent() throws Exception {
    ...
    assertEquals(0, m_uiSession.currentJsonResponse().getEventList().size());
    //TODO [6.2] bsh: CGU delete ? assertEquals(6, jsonTable.eventBuffer().size()); // TYPE_ROW_FILTER_CHANGED + TYPE_ROWS_UPDATED = TYPE_ROWS_DELETED + TYPE_ROWS_INSERTED + TYPE_ROWS_UPDATED (row0) + TYPE_ROWS_UPDATED (row2)
