...
describe("Tree", function() {
  ...
  describe("tree filter", function() {
    ...
    it("filtered nodes stays removed even if it they are selected explicitly", function() {
      // +- node 0
      // |  +- node 0_0
      // |  +- node 0_1
      // +- node 1
      ...
      expect(tree.nodes[0].childNodes.length).toBe(2);
      expect(tree.nodes[0].attached).toBe(true);
      expect(tree.nodes[0].childNodes[0].attached).toBe(true);
      expect(tree.nodes[0].childNodes[1].attached).toBe(true);
      expect(tree.nodes[1].attached).toBe(true);
      expect(tree.nodes[1].childNodes[0].attached).toBe(true);
      expect(tree.nodes[1].childNodes[1].attached).toBe(true);
      expect(tree.selectedNodes.length).toBe(0);

      // Add a filter that hides the first child node of the first node (still no nodes selected)
      var filter = {
        accept: function(node) {
          return node.text !== 'node 0_0';
        }
      };
      tree.addFilter(filter);
      tree.filter();
      expect(tree.nodes[0].childNodes.length).toBe(2);
      expect(tree.nodes[0].attached).toBe(true);
      expect(tree.nodes[0].childNodes[0].attached).toBe(false); // <--
      expect(tree.nodes[0].childNodes[1].attached).toBe(true);
      expect(tree.nodes[1].attached).toBe(true);
      expect(tree.nodes[1].childNodes[0].attached).toBe(true);
      expect(tree.nodes[1].childNodes[1].attached).toBe(true);
      ...
      // Explicitly select the invisible node, expect still no selected row
      tree.selectNodes(tree.nodes[0].childNodes[0]);
      expect(tree.nodes[0].childNodes.length).toBe(2);
      expect(tree.nodes[0].attached).toBe(true);
      expect(tree.nodes[0].childNodes[0].attached).toBe(false);
      expect(tree.nodes[0].childNodes[1].attached).toBe(true);
      ...
      // Now select the first node again, this time the selection should change
      tree.selectNodes(tree.nodes[0].childNodes[0]);
      expect(tree.nodes[0].childNodes.length).toBe(2);
      expect(tree.nodes[0].attached).toBe(true);
      expect(tree.nodes[0].childNodes[0].attached).toBe(true);
