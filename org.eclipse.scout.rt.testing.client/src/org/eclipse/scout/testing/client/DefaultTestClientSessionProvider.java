...
import org.eclipse.scout.rt.client.ClientSessionProvider;
          ...
          try {
            ...
            IFuture<Void> future = OBJ.one(IModelJobManager.class).schedule(new IRunnable() {
              ...
              public void run() throws Exception {
                IClientSession s = ClientSessionProvider.get();
                beforeStartSession(s, runAs);
                s.startSession(bundle);
                simulateDesktopOpened(s);
                afterStartSession(s, runAs);
              }
            }, ClientJobInput.defaults().session(clientSession).name("Session startup"));
            future.get();
          }
          catch (Throwable t) {
