...
import org.eclipse.scout.rt.testing.client.DefaultTestClientSessionProvider;
import org.eclipse.scout.rt.testing.client.ITestClientSessionProvider;
 ...
 * <p/>
 * <code>org.eclipse.scout.rt.testing.client.runner.CustomClientTestEnvironment</code>
 */
public class ScoutClientTestRunner extends BlockJUnit4ClassRunner {
  ...
  private static IClientTestEnvironment createClientTestEnvironmentFactory() {
    ...
    if (SerializationUtility.getClassLoader() != null) {
      ...
      try {
        Class<?> customTestEnvironment = SerializationUtility.getClassLoader().loadClass("org.eclipse.scout.rt.testing.client.runner.CustomClientTestEnvironment");
        ...
        environment = createLegacyClientTestEnvironmentFactory(); //Legacy: ensure compatibility with the depreciated package name. This code line is to be removed with L-Release.
      }
      catch (Exception e) {
  ...
  /**
   * Legacy version of {@link #createClientTestEnvironmentFactory()} to support the old class name:
   * "org.eclipse.scout.testing.client.runner.CustomClientTestEnvironment"
   * To be removed with L-Release.
   */
  private static IClientTestEnvironment createLegacyClientTestEnvironmentFactory() {
    IClientTestEnvironment environment = null;
    if (SerializationUtility.getClassLoader() != null) {
      // check whether there is a custom test environment available
      try {
        ...
        LOG.info("loaded custom test environment: [" + customTestEnvironment + "]");
        LOG.warn("MIGRATION: consider moving your CustomClientTestEnvironment in the new package: org.eclipse.scout.rt.testing.client.runner (instead of current org.eclipse.scout.testing.client.runner)");
        if (org.eclipse.scout.testing.client.runner.IClientTestEnvironment.class.isAssignableFrom(customTestEnvironment)) {
          LOG.warn("MIGRATION: org.eclipse.scout.testing.client.runner.IClientTestEnvironment is depreciated, use " + IClientTestEnvironment.class + " instead.");
        }
        if (!IClientTestEnvironment.class.isAssignableFrom(customTestEnvironment)) {
          LOG.warn("custom test environment is not implementing [" + IClientTestEnvironment.class + "]");
        }
