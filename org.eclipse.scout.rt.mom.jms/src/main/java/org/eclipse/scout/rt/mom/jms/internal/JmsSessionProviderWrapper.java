 ...
 * @since 6.1
 */
public class JmsSessionProviderWrapper implements IJmsSessionProviderEx {
  ...
  @Override
  public Message receive(final SubscribeInput input) throws JMSException {
    try {
      return JmsSessionProviderMigration.receive(trySessionProvider(), input);
    }
    catch (JMSException e) {
      waitForRetry(e);
      return JmsSessionProviderMigration.receive(trySessionProvider(), input);
  ...
  public ISubscriptionStats getStats() {
    ...
    if (impl instanceof IJmsSessionProviderEx) {
      return ((IJmsSessionProviderEx) impl).getStats();
    }
    return null;
  }
}
