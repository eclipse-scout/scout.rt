 ...
 */
@Bean
public class HttpServerSessionRunContextProducer {

  public static final String SCOUT_SESSION_ID_KEY = HttpServerSessionRunContextProducer.class.getName() + ".SCOUT_SESSION_ID";
  ...
  public IServerSession getOrCreateScoutSession(HttpServletRequest req, ServerRunContext serverRunContextForSessionStart, String scoutSessionId) {
    ...
    if (session == null) {
      LOG.warn("{} is configured to create a Scout session but no class implementing {} could be found. Consider disabling session support.",
          HttpServerSessionRunContextProducer.class.getName(), IServerSession.class.getName());
    }
    return session;
  }
  ...
  public HttpServerSessionRunContextProducer withSessionSupport(boolean sessionSupport) {
