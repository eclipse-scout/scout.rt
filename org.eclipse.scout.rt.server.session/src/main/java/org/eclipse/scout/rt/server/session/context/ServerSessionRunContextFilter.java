...
import org.eclipse.scout.rt.server.session.ServerSessionProviderWithCache.ServerSessionCacheExpirationProperty;
 ...
 * @since 6.1
 */
public class ServerSessionRunContextFilter implements Filter {

  private ServerSessionRunContextProducer m_sessionContextProducer;
  private HttpServerSessionRunContextProducer m_httpServerRunContextProducer;
  ...
  protected HttpServerSessionRunContextProducer createHttpServerRunContextProducer() {
    return BEANS.get(HttpServerSessionRunContextProducer.class)
        .withSessionSupport(false); // session is provided by #getSessionContextProducer()
  }

  protected HttpServerSessionRunContextProducer getHttpServerRunContextProducer() {
    return m_httpServerRunContextProducer;
  }

  protected ServerSessionRunContextProducer createSessionRunContextProducer() {
    // this producer uses ServerSessionProviderWithCache which is a TTL based cache. Not bound to the HTTP session!
    return BEANS.get(ServerSessionRunContextProducer.class);
  }

  protected ServerSessionRunContextProducer getSessionContextProducer() {
  ...
  protected ServerRunContext lookupRunContext(HttpServletRequest req, HttpServletResponse resp) {
    ...
    boolean isWithoutSession = req.getHeader(SessionId.HTTP_HEADER_NAME) == null && isRequestAllowedWithoutSession(req);
