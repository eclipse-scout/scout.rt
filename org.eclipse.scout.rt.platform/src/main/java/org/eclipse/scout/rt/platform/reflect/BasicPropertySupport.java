...
public class BasicPropertySupport implements IListenerListWithManagement {
   ...
   *
   * @param propertyMap
   *     either pre-filled or empty; this {@link Map} must not be changed anymore outside of {@link BasicPropertySupport} (otherwise properties would be silently changed)
   */
  ...
  public void addPropertyChangeListener(String propertyName, PropertyChangeListener listener) {
    synchronized (m_listenerLock) {
      ...
      List<Object> children = m_childListeners.computeIfAbsent(propertyName, _ -> new ArrayList<>());
      if (listener instanceof WeakEventListener) {
        ...
        for (Entry<String, List<Object>> entry : m_childListeners.entrySet()) {
          ...
          if (propertySpecificListeners != null) {
            for (Object o : propertySpecificListeners) {
              ...
              if (o != null) {
                List<PropertyChangeListener> children = listeners.computeIfAbsent(propertyName, _ -> new ArrayList<>());
