  ...
  protected void handlePlatformException(final PlatformException e) {
    if (e instanceof VetoException) {
      ...
      switch (((ProcessingException) e).getStatus().getSeverity()) {
        case IProcessingStatus.INFO, IProcessingStatus.OK -> LOG.info(LOG_PATTERN, toLogArguments(e));
        case IProcessingStatus.WARNING -> LOG.warn(LOG_PATTERN, toLogArguments(e));
        default -> LOG.error(LOG_PATTERN, toLogArguments(e));
      }
    }
    else {
