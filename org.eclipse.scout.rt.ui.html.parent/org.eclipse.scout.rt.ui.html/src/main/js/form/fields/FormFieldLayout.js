...
scout.FormFieldLayout.prototype.layout = function($container) {
  ...
  if (formField.$label && formField.labelVisible) {
    ...
    leftWidth += formField.$label.outerWidth(true);
  }
  if (formField.$mandatory) {
    formField.$mandatory.cssLeft(leftWidth).cssWidth(this.mandatoryIndicatorWidth);
    leftWidth += formField.$mandatory.outerWidth(true);
  }
