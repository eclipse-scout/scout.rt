 ...
 * <p>
 * Multipart also acts as the base class for the content object returned by most
 * Multipart DataContentHandlers. For example, invoking getContent() on a
 * DataHandler whose source is a "multipart/signed" data source may return an
 * appropriate subclass of Multipart.
 * <p>
 * Some messaging systems provide different subtypes of Multiparts. For example,
 * MIME specifies a set of subtypes that include "alternative", "mixed",
 * "related", "parallel", "signed", etc.
 * <p>
...
public abstract class Multipart{
  ...
  /**
   * This field specifies the content-type of this multipart object. It defaults
   * to "multipart/mixed".
   */
  protected String contentType="multipart/mixed"; // Content-Type

  /**
   * The <code>Part</code> containing this <code>Multipart</code>, if known.
   * 
   * @since JavaMail 1.1
   */
   ...
   * Multipart. This Multipart's contentType is set to that of the
   * MultipartDataSource.
   * <p>
   * This method is typically used in those cases where one has a multipart data
   * source that has already been pre-parsed into the individual body parts (for
   * example, an IMAP datasource), but needs to create an appropriate Multipart
   * subclass that represents a specific multipart subtype.
   * 
   * @param mp
   ...
   * <p>
   * This implementation just returns the value of the <code>contentType</code>
   * field.
   * 
   * @return content-type
   * @see #contentType
  ...
  public synchronized BodyPart getBodyPart(int index)
      throws MessagingException{
    if(parts == null) throw new IndexOutOfBoundsException("No such BodyPart");
  ...
  public synchronized boolean removeBodyPart(BodyPart part)
      throws MessagingException{
    if(parts == null) throw new MessagingException("No such body part");
  ...
  public synchronized void addBodyPart(BodyPart part)
      throws MessagingException{
    if(parts == null) parts=new Vector();
  ...
  /**
   * Output an appropriately encoded bytestream to the given OutputStream. The
   * implementation subclass decides the appropriate encoding algorithm to be
   * used. The bytestream is typically used for sending.
   * 
   ...
   * <code>Part</code>. Normally called by the <code>Message</code> or
   * <code>BodyPart</code> <code>setContent(Multipart)</code> method.
   * <code>parent</code> may be <code>null</code> if the <code>Multipart</code>
   * is being removed from its containing <code>Part</code>.
   * 
   * @since JavaMail 1.1
   */
