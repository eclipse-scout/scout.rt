 ...
 * implementation provides only for substring matching. We could add comparisons
 * (like strcmp ...).
 * 
 * @author Bill Shannon
 * @author John Mani
...
public abstract class StringTerm extends SearchTerm{
  ...
  protected boolean match(String s){
    ...
    for(int i=0; i <= len; i++){
      if(s.regionMatches(ignoreCase, i,
          pattern, 0, pattern.length())) return true;
    }
    return false;
  }
  ...
  public boolean equals(Object obj){
    ...
    if(ignoreCase) return st.pattern.equalsIgnoreCase(this.pattern) &&
        st.ignoreCase == this.ignoreCase;
    else return st.pattern.equals(this.pattern) &&
        st.ignoreCase == this.ignoreCase;
  }
