...
public final class AndTerm extends SearchTerm{
   ...
   * The AND operation.
   * <p>
   * The terms specified in the constructor are applied to the given object and
   * the AND operator is applied to their results.
   * 
   * @param msg
   *          The specified SearchTerms are applied to this Message and the AND
   *          operator is applied to their results.
   * @return true if the AND succeds, otherwise false
   */
  public boolean match(Message msg){
    for(int i=0; i < terms.length; i++)
      if(!terms[i].match(msg)) return false;
    return true;
  }
  ...
  public boolean equals(Object obj){
    if(!(obj instanceof AndTerm)) return false;
    AndTerm at=(AndTerm)obj;
    if(at.terms.length != terms.length) return false;
    for(int i=0; i < terms.length; i++)
      if(!terms[i].equals(at.terms[i])) return false;
    return true;
  }
