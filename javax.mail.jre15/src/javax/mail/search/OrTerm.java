...
public final class OrTerm extends SearchTerm{
   ...
   * the OR operator is applied to their results.
   * 
   * @param msg
   *          The specified SearchTerms are applied to this Message and the OR
  ...
  public boolean match(Message msg){
    for(int i=0; i < terms.length; i++)
      if(terms[i].match(msg)) return true;
    return false;
  }
  ...
  public boolean equals(Object obj){
    if(!(obj instanceof OrTerm)) return false;
    OrTerm ot=(OrTerm)obj;
    if(ot.terms.length != terms.length) return false;
    for(int i=0; i < terms.length; i++)
      if(!terms[i].equals(ot.terms[i])) return false;
    return true;
  }
