 ...
 * file by placing it in the <code>lib</code> directory where the
 ...
 * requested file and then stops. JDK 1.1 has a limitation that the number of
 * files of each name that will be found in the CLASSPATH is limited to one.
 * However, this only affects method two, above; method one is loaded from a
 * specific location (if allowed by the SecurityManager) and method three uses a
 * different name to ensure that the default resource file is always loaded
 * successfully. J2SE 1.2 and later are not limited to one file of a given name.
 * <p>
 ...
 * available.
 * <p>
 * <b><code>javamail.providers</code></b> and <b>
 * <code>javamail.default.providers</code></b>
 * <p>
 ...
 * <tr>
 * <td>protocol</td>
 * <td>Name assigned to protocol. For example, <code>smtp</code> for Transport.</td>
 * </tr>
 * <tr>
 * <td>type</td>
 ...
 * </pre>
 * <p>
 * <b><code>javamail.address.map</code></b> and <b>
 * <code>javamail.default.address.map</code></b>
 * <p>
...
public final class Session{
  ...
  private Session(Properties props, Authenticator authenticator) {
    ...
    if(Boolean.valueOf(props.getProperty("mail.debug")).booleanValue()) debug=true;

    if(debug) pr("DEBUG: JavaMail version " + Version.version);

    // get the Class associated with the Authenticator
    Class cl;
    if(authenticator != null) cl=authenticator.getClass();
    else cl=this.getClass();
    // load the resources
    loadProviders(cl);
    loadAddressMap(cl);
  }
   ...
   * 
   * @param props
   *          Properties object that hold relevant properties.<br>
   ...
   *          mail.user, and mail.from) as the defaults are unlikely to work in
   *          all cases.
   * @param authenticator
   *          Authenticator object used to call back to the application when a
   ...
   * the same Java virtual machine, and the session can contain security
   * sensitive information such as user names and passwords, access to the
   * default session is restricted. The Authenticator object, which must be
   * created by the caller, is used indirectly to check access permission. The
   * Authenticator object passed in when the session is created is compared with
   ...
   * <p>
   * Note also that the Properties object is used only the first time this
   * method is called, when a new Session object is created. Subsequent calls
   * return the Session object that was created by the first call, and ignore
   * the passed Properties object. Use the <code>getInstance</code> method to
   * get a new Session object every time the method is called.
   * <p>
   * In JDK 1.2, additional security Permission objects may be used to control
   * access to the default session.
   * 
   * @param props
   *          Properties object. Used only if a new Session object is created.<br>
   *          It is expected that the client supplies values for the properties
   *          listed in Appendix A of the JavaMail spec (particularly
   *          mail.store.protocol, mail.transport.protocol, mail.host,
   *          mail.user, and mail.from) as the defaults are unlikely to work in
   *          all cases.
   * @param authenticator
   *          Authenticator object. Used only if a new Session object is
   *          created. Otherwise, it must match the Authenticator used to create
   *          the Session.
   * @return the default Session object
   */
  public static synchronized Session getDefaultInstance(Properties props,
      Authenticator authenticator){
    if(defaultSession == null) defaultSession=new Session(props, authenticator);
    else{
   ...
   * Session object is created and installed as the default.
   * <p>
   * Note that a default session created with no Authenticator is available to
   * all code executing in the same Java virtual machine, and the session can
   * contain security sensitive information such as user names and passwords.
   * 
   * @param props
   *          Properties object. Used only if a new Session object is created.<br>
   *          It is expected that the client supplies values for the properties
   ...
   * constructor to true. The value of the <code>mail.debug</code> property is
   * used to initialize the per-Session debugging flag. Subsequent calls to the
   * <code>setDebug</code> method manipulate the per-Session debugging flag and
   * have no affect on the <code>mail.debug</code> property.
   * 
   * @param debug
   *          Debug setting
   */
  public synchronized void setDebug(boolean debug){
    this.debug=debug;
    if(debug) pr("DEBUG: setDebug: JavaMail version " + Version.version);
  }
  ...
  /**
   * Set the stream to be used for debugging output for this session. If
   * <code>out</code> is null, <code>System.out</code> will be used. Note that
   * debugging output that occurs before any session is created, as a result of
   * setting the <code>mail.debug</code> system property, will always be sent to
  ...
  /**
   * This method returns an array of all the implementations installed via the
   * javamail.[default.]providers files that can be loaded using the ClassLoader
  ...
  /**
   * Returns the default Provider for the protocol specified. Checks
   * mail.&lt;protocol&gt;.class property first and if it exists, returns the
   * Provider associated with this implementation. If it doesn't exist, returns
   * the Provider that appeared first in the configuration files. If an
   * implementation for the protocol isn't found, throws NoSuchProviderException
   * 
   * @param protocol
   ...
   * 
   * @param provider
   *          Currently configured Provider which will be set as the default for
   *          the protocol
   * @exception NoSuchProviderException
  ...
  /**
   * Get a Store object that implements this user's desired Store protocol. The
   * <code>mail.store.protocol</code> property specifies the desired protocol.
   * If an appropriate Store object is not obtained, NoSuchProviderException is
   * thrown
   * 
   ...
   * appropriate Store object cannot be obtained, NoSuchProviderException is
   * thrown.
   * 
   * @param protocol
   * @return a Store object
  ...
  /**
   * Get a Store object for the given URLName. If the requested Store object
   * cannot be obtained, NoSuchProviderException is thrown. The "scheme" part of
   * the URL string (Refer RFC 1738) is used to locate the Store protocol.
   * <p>
   * 
   * @param url
   ...
   * @exception NoSuchProviderException
   *              If a provider for the given URLName is not found.
   */
  ...
  /**
   * Get an instance of the store specified by Provider. Instantiates the store
   * and returns it.
   * 
   * @param provider
  ...
  /**
   * Get an instance of the store specified by Provider. If the URLName is not
   * null, uses it, otherwise creates a new one. Instantiates the store and
   * returns it. This is a private method used by getStore(Provider) and
   * getStore(URLName)
   * 
   * @param provider
   *          Store Provider that will be instantiated
   * @param url
   *          URLName used to instantiate the Store
   * @return Instantiated Store
   * @exception NoSuchProviderException
   *              If a provider for the given Provider/URLName is not found.
   */
  ...
  /**
   * Get a closed Folder object for the given URLName. If the requested Folder
   * object cannot be obtained, null is returned.
   * <p>
   * The "scheme" part of the URL string (Refer RFC 1738) is used to locate the
   * Store protocol. The rest of the URL string (that is, the "schemepart", as
   * per RFC 1738) is used by that Store in a protocol dependent manner to
   * locate and instantiate the appropriate Folder object.
   * <p>
   ...
   *              if the Folder could not be located or created.
   */
   ...
   * protcol. The <code>mail.transport.protocol</code> property specifies the
   * desired protocol. If an appropriate Transport object cannot be obtained,
  ...
  /**
   * Get a Transport object that implements the specified protocol. If an
   * appropriate Transport object cannot be obtained, null is returned.
   * 
   * @return a Transport object
   * @exception NoSuchProviderException
  ...
  /**
   * Get a Transport object for the given URLName. If the requested Transport
   * object cannot be obtained, NoSuchProviderException is thrown. The "scheme"
   * part of the URL string (Refer RFC 1738) is used to locate the Transport
   * protocol.
   * <p>
   * 
   * @param url
   ...
   * 
   * @param provider
   *          Transport Provider that will be instantiated
   ...
   * @exception NoSuchProviderException
   *              If provider for the Address type is not found
   */
   ...
   *          the provider to use
   * @param url
   *          urlname to use (can be null)
   * @return A Transport object
   * @exception NoSuchProviderException
   *              If no provider or the provider was the wrong class.
   */
  ...
  /**
   * Get a Service object. Needs a provider object, but will create a URLName if
   * needed. It attempts to instantiate the correct class.
   * 
   * @param provider
   *          which provider to use
   * @param url
   *          which URLName to use (can be null)
   * @exception NoSuchProviderException
   *              thrown when the class cannot be found or when it does not have
   *              the correct constructor (Session, URLName), or if it is not
  ...
  private Object getService(Provider provider, URLName url)
    ...
    ClassLoader cl;
    if(authenticator != null) cl=authenticator.getClass().getClassLoader();
    ...
    try{
      ...
      if(serviceClass == null) serviceClass=cl.loadClass(provider.getClassName());
    }
    catch(Exception ex1){
  ...
  /**
   * Save a PasswordAuthentication for this (store or transport) URLName. If pw
   * is null the entry corresponding to the URLName is removed.
   * <p>
  ...
  public void setPasswordAuthentication(URLName url,
    ...
    if(pw == null) authTable.remove(url);
    else authTable.put(url, pw);
  }
  ...
  /**
   * Call back to the application to get the needed user name and password. The
   * application should put up a dialog something like:
   * <p>
   * 
   * <pre>
   ...
   * 
   * @param addr
   *          InetAddress of the host. may be null.
   * @param protocol
   *          protocol scheme (e.g. imap, pop3, etc.)
   * @param prompt
   *          any additional String to show as part of the prompt; may be null.
   * @param defaultUserName
   *          the default username. may be null.
   * @return the authentication which was collected by the authenticator; may be
   *         null.
   */
  ...
  /**
   * Returns the value of the specified property. Returns null if this property
   * does not exist.
   * 
  ...
  private void loadProviders(Class cl){
    ...
    try{
      ...
      if(debug) pr("DEBUG: can't get java.home: " + sex);
    }
    ...
    if(providers.size() == 0){
      if(debug) pr("DEBUG: failed to load any providers, using defaults");
    ...
    if(is != null){
      ...
      while((currLine=lis.readLine()) != null){
        ...
        if(type == null || protocol == null || className == null
          ...
          if(debug) pr("DEBUG: Bad provider entry: " + currLine);
          continue;
        }
  ...
  public synchronized void addProvider(Provider provider){
    ...
    if(!providersByProtocol.containsKey(provider.getProtocol())) providersByProtocol.put(provider.getProtocol(), provider);
  }
  ...
  private void loadAddressMap(Class cl){
    ...
    if(addressMap.isEmpty()){
      if(debug) pr("DEBUG: failed to load address map, using defaults");
      addressMap.put("rfc822", "smtp");
    }
  }

  /**
   * Set the default transport protocol to use for addresses of the specified
   * type. Normally the default is set by the
  ...
  public synchronized void setProtocolForAddress(String addresstype,
      String protocol){
    if(protocol == null) addressMap.remove(addresstype);
    else addressMap.put(addresstype, protocol);
  }
  ...
  private void loadFile(String name, StreamLoader loader){
    ...
    try{
      ...
      loader.load(clis);
      if(debug) pr("DEBUG: successfully loaded file: " + name);
    }
    catch(IOException e){
      if(debug){
  ...
  private void loadResource(String name, Class cl, StreamLoader loader){
    ...
    try{
      ...
      if(clis != null){
        loader.load(clis);
        if(debug) pr("DEBUG: successfully loaded resource: " + name);
      }
      else{
        if(debug) pr("DEBUG: not loading resource: " + name);
      }
    }
    catch(IOException e){
      if(debug) pr("DEBUG: " + e);
  ...
  private void loadAllResources(String name, Class cl, StreamLoader loader){
    ...
    try{
      ...
      if(cld != null) urls=getResources(cld, name);
      else urls=getSystemResources(name);
      if(urls != null){
        for(int i=0; i < urls.length; i++){
          ...
          try{
            ...
            if(clis != null){
              loader.load(clis);
              anyLoaded=true;
              if(debug) pr("DEBUG: successfully loaded resource: " +
                  url);
            }
            else{
