...
public class URLName{
  ...
  /**
   * Creates a URLName object from the specified protocol, host, port number,
   * file, username, and password. Specifying a port number of -1 indicates that
  ...
  protected void parseString(String url){
    ...
    if(protocolEnd != -1) protocol=url.substring(0, protocolEnd);
    ...
    if(url.regionMatches(protocolEnd + 1, "//", 0, 2)){
      ...
      if(fileStart != -1){
        ...
        if(fileStart + 1 < len) file=url.substring(fileStart + 1);
        else file="";
      }
      else fullhost=url.substring(protocolEnd + 3);
      ...
      if(protocolEnd + 1 < len) file=url.substring(protocolEnd + 1);
    }
  ...
  /**
   * Returns the file name of this URLName. Returns null if this URLName has no
   * file name.
   */
  ...
  /**
   * Returns the reference of this URLName. Returns null if this URLName has no
   * reference.
   */
  ...
  /**
   * Returns the user name of this URLName. Returns null if this URLName has no
   * user name.
   */
   ...
   * the same host, the same port number on the host, the same username, and the
   * same file on the host. The fields (host, username, file) are also
   * considered the same if they are both null.
   * <p>
   * Hosts are considered equal if the names are equal (case independent) or if
   * host name lookups for them both succeed and they both reference the same IP
   * address.
   * <p>
   ...
   * equal.
   * <p>
   * Note also that the password field is not included in the comparison, nor is
   * any reference field appended to the filename.
   */
  public boolean equals(Object obj){
    if(!(obj instanceof URLName)) return false;
    URLName u2=(URLName)obj;

    // compare protocols
    if(u2.protocol == null || !u2.protocol.equals(protocol)) return false;
    ...
    if(a1 != null && a2 != null){
      ...
      if(!host.equalsIgnoreCase(u2.host)) return false;
    ...
    // compare usernames
    if(!(username == u2.username || (username != null && username.equals(u2.username)))) return false;
  ...
  public int hashCode(){
    ...
    if(addr != null) hashCode+=addr.hashCode();
    else if(host != null) hashCode+=host.toLowerCase(Locale.ENGLISH).hashCode();
    if(username != null) hashCode+=username.hashCode();
    if(file != null) hashCode+=file.hashCode();
    hashCode+=port;
    return hashCode;
  }
  ...
  private synchronized InetAddress getHostAddress(){
    if(hostAddressKnown) return hostAddress;
  ...
  static String encode(String s){
    ...
    for(int i=0; i < s.length(); i++){
      ...
      if(c == ' ' || !dontNeedEncoding.get(c)) return _encode(s);
    }
    return s;
  }
   ...
   * begin with the percent sign, "<code>%<i>xy</i></code>", where <i>xy</i> is
   * the two-digit hexadecimal representation of the lower 8-bits of the
   * character.
   * </ul>
   * 
  ...
  static String decode(String s){
    if(s == null) return null;
    if(indexOfAny(s, "+%") == -1) return s; // the common case
  ...
  /*
   * // Do not remove, this is needed when testing new URL cases public static
   * void main(String[] argv) { String [] testURLNames = {
   * "protocol://userid:password@host:119/file",
   * "http://funny/folder/file.html", "http://funny/folder/file.html#ref",
   * "http://funny/folder/file.html#", "http://funny/#ref",
   * "imap://jmr:secret@labyrinth//var/mail/jmr",
   * "nntp://fred@labyrinth:143/save/it/now.mbox", "imap://jmr@labyrinth/INBOX",
   * "imap://labryrinth", "imap://labryrinth/", "file:", "file:INBOX",
   * "file:/home/shannon/mail/foo", "/tmp/foo", "//host/tmp/foo", ":/tmp/foo",
   * "/really/weird:/tmp/foo#bar", "" }; URLName url = new URLName("protocol",
   * "host", 119, "file", "userid", "password"); System.out.println("Test URL: "
   * + url.toString()); if (argv.length == 0) { for (int i = 0; i <
   * testURLNames.length; i++) { print(testURLNames[i]); System.out.println(); }
   * } else { for (int i = 0; i < argv.length; i++) { print(argv[i]);
   * System.out.println(); } if (argv.length == 2) { URLName u1 = new
   * URLName(argv[0]); URLName u2 = new URLName(argv[1]);
   * System.out.println("URL1 hash code: " + u1.hashCode());
   * System.out.println("URL2 hash code: " + u2.hashCode()); if (u1.equals(u2))
   * System.out.println("success, equal"); else
   * System.out.println("fail, not equal"); if (u2.equals(u1))
   * System.out.println("success, equal"); else
   * System.out.println("fail, not equal"); if (u1.hashCode() == u2.hashCode())
   * System.out.println("success, hashCodes equal"); else
   * System.out.println("fail, hashCodes not equal"); } } } private static void
   * print(String name) { URLName url = new URLName(name);
   * System.out.println("Original URL: " + name);
   * System.out.println("The fullUrl : " + url.toString()); if
   * (!name.equals(url.toString()))
