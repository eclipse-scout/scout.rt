...
/**
 * A utility class that implements a DataSource out of a MimePart. This class is
 * primarily meant for service providers.
 * 
...
public class MimePartDataSource implements DataSource, MessageAware{
  ...
  public InputStream getInputStream() throws IOException{
    ...
    try{
      if(part instanceof MimeBodyPart) is=((MimeBodyPart)part).getContentStream();
      else if(part instanceof MimeMessage) is=((MimeMessage)part).getContentStream();
      else throw new MessagingException("Unknown part");

      String encoding=restrictEncoding(part.getEncoding(), part);
      if(encoding != null) return MimeUtility.decode(is, encoding);
      else return is;
    }
  ...
  private static String restrictEncoding(String encoding, MimePart part)
    ...
    if(encoding.equalsIgnoreCase("7bit") ||
        ...
        encoding.equalsIgnoreCase("binary")) return encoding; // these encodings
    // are always
    // valid
    ...
    try{
      /*
       * multipart and message types aren't allowed to have encodings except for
       * the three mentioned above. If it's one of these types, ignore the
       * encoding.
       */
   ...
   * <p>
   * This implementation just invokes the <code>getContentType</code> method on
   * the MimePart.
   */
