...
public class HeaderTokenizer{
  ...
  public static class Token{
    ...
    /**
     * Token type indicating a quoted string. The value field contains the
     * string without the quotes.
     */
    public static final int QUOTEDSTRING=-2;

    /**
     * Token type indicating a comment. The value field contains the comment
     * string without the comment start and end symbols.
     */
...
/**
    ...
    /**
     * Returns the value of the token just read. When the current token is a
     * quoted string, this field contains the body of the string, without the
     * quotes. When the current token is a comment, this field contains the body
     * of the comment.
     * 
     * @return token value
     */
   ...
   * 
   * @param header
   *          The rfc822 header to be tokenized
   * @param delimiters
   *          Set of delimiter characters to be used to delimit ATOMS. These are
   *          usually <code>RFC822</code> or <code>MIME</code>
   * @param skipComments
  ...
  /**
   * Constructor. The RFC822 defined delimiters - RFC822 - are used to delimit
   * ATOMS. Also comments are skipped and not returned as tokens
   */
   ...
   * <p>
   * Clients sit in a loop calling next() to parse successive tokens until an
  ...
  /**
   * Peek at the next token, without actually removing the token from the parse
   * stream. Invoking this method multiple times will return successive tokens,
   ...
   * 
   * @return String rest of header. null is returned if we are already at end of
   *         header
   */
  ...
  /*
   * Return the next token starting from 'currentPos'. After the parse,
   * 'currentPos' is updated to point to the start of the next token.
   */
  private Token getNext() throws ParseException{
    ...
    while(c == '('){
      ...
      if(!skipComments){
        ...
        else s=string.substring(start, currentPos - 1);
    ...
    if(c == '"'){
      for(start=++currentPos; currentPos < maxPos; currentPos++){
        ...
        if(c == '\\'){ // Escape sequence
          ...
          if(filter) s=filterToken(string, start, currentPos - 1);
          else s=string.substring(start, currentPos - 1);
    ...
    for(start=currentPos; currentPos < maxPos; currentPos++){
      ...
      if(c < 040 || c >= 0177 || c == '(' || c == ' ' ||
          c == '"' || delimiters.indexOf(c) >= 0) break;
    }
  ...
  /*
   * Process escape sequences and embedded LWSPs from a comment or quoted
   * string.
   */
