...
public class MimeBodyPart extends BodyPart implements MimePart{
  ...
  /**
   * An empty MimeBodyPart object is created. This body part maybe filled in by
   * a client constructing a multipart message.
   */
  public MimeBodyPart() {
    super();
   ...
   * specified input stream. The parser consumes data till the end of the given
   * input stream. The input stream must start at the beginning of a valid MIME
   * body part and must terminate at the end of that body part.
   * <p>
   * Note that the "boundary" string that delimits body parts must
   * <strong>not</strong> be included in the input stream. The intention is that
   * the MimeMultipart parser will extract each body part's bytes from a
   * multipart stream and feed them into this constructor, without the delimiter
   * strings.
   * 
   * @param is
  ...
  /**
   * Return the size of the content of this body part in bytes. Return -1 if the
  ...
  /**
   * Return the number of lines for the content of this Part. Return -1 if this
   ...
   *              if this body part is obtained from a READ_ONLY folder.
   * @exception MessagingException
   *              otherwise; an UnsupportedEncodingException may be included in
   *              the exception chain if the charset conversion fails.
   */
   ...
   * "Content-Disposition" header field of this body part. If its not available,
   * returns the value of the "name" parameter from the "Content-Type" header
   * field of this body part. Returns <code>null</code> if both are absent.
   * <p>
   ...
   * this body part. For compatibility with older mailers, the "name" parameter
   * of the "Content-Type" header is also set.
   * <p>
   * If the <code>mail.mime.encodefilename</code> System property is set to
   ...
   * it invokes getDataHandler().getInputStream();
   * 
   ...
   * DataHandler object for the content. Subclasses that can provide a separate
   ...
   * <p>
   * The implementation provided here works just like the the implementation in
   * MimeMessage.
   * 
  ...
  /**
   * Return the content as a Java object. The type of the object returned is of
   * course dependent on the content itself. For example, the native format of a
   * text/plain content is usually a String object. The native format for a
   * "multipart" content is always a Multipart subclass. For content types that
   * are unknown to the DataHandler system, an input stream is returned as the
   * content.
   * <p>
   ...
   * and returned in subsequent calls so that modifications to the content will
   * not be lost.
   * 
   * @return Object
   * @exception MessagingException
   ...
   * JavaMail implementation for this to work right. That is, to do
   ...
   * and if any edits have been made to either the content or message structure,
   * we might need to resync our headers. <br>
  ...
  static void setText(MimePart part, String text, String charset,
    ...
    if(charset == null){
      if(MimeUtility.checkAscii(text) != MimeUtility.ALL_ASCII) charset=MimeUtility.getDefaultMIMECharset();
  ...
  static void setDisposition(MimePart part, String disposition)
    ...
    if(disposition == null) part.removeHeader("Content-Disposition");
      ...
      if(s != null){
        /*
         * A Content-Disposition header already exists .. Override disposition,
         * but attempt to retain existing disposition parameters
         */
        ContentDisposition cd=new ContentDisposition(s);
  ...
  static void setFileName(MimePart part, String name)
    ...
    /*
     * Also attempt to set the Content-Type "name" parameter, to satisfy ancient
     * MUAs. XXX - This is not RFC compliant.
     */
  ...
  static String[] getContentLanguage(MimePart part)
    ...
    while(true){
      ...
      else if(tkType == HeaderTokenizer.Token.ATOM) v.addElement(tk.getValue());
  ...
  static String getEncoding(MimePart part) throws MessagingException{
    ...
    if(s.equalsIgnoreCase("7bit") || s.equalsIgnoreCase("8bit") ||
        ...
        s.equalsIgnoreCase("base64")) return s;
  ...
  static void updateHeaders(MimePart part) throws MessagingException{
    ...
    try{
      ...
      if(cType.match("multipart/*")){
        ...
        if(o instanceof MimeMultipart) ((MimeMultipart)o).updateHeaders();
        else throw new MessagingException("MIME part of type \"" +
      ...
      if(!composite){ // not allowed on composite parts
        if(part.getHeader("Content-Transfer-Encoding") == null) setEncoding(part, MimeUtility.getEncoding(dh));

        if(needCTHeader && setDefaultTextCharset &&
            cType.match("text/*") &&
            cType.getParameter("charset") == null){
          /*
           * Set a default charset for text parts. We really should examine the
           * data to determine whether or not it's all ASCII, but that's too
           * expensive so we make an assumption: If we chose 7bit encoding for
           * this data, it's probably ASCII. (MimeUtility.getEncoding will
           * choose 7bit only in this case, but someone might've set the
      ...
      if(needCTHeader){
        /*
         * Pull out "filename" from Content-Disposition, and use that to set the
         * "name" parameter. This is to satisfy older MUAs (DtMail, Roam and
