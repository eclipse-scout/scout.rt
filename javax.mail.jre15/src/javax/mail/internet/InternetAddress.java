...
public class InternetAddress extends Address implements Cloneable{
   ...
   * <code>parse</code> method for details of the parsing. The address is parsed
   * using "strict" parsing. This constructor does <b>not</b> perform the
   * additional syntax checks that the
  ...
  public InternetAddress(String address) throws AddressException {
    ...
    if(a.length != 1) throw new AddressException("Illegal address", address);

    /*
     * Now copy the contents of the single address we parsed into the current
     * object, which will be returned from the constructor. XXX - this sure is a
     * round-about way of getting this done.
     */
  ...
  public InternetAddress(String address, boolean strict)
    ...
    this(address);
    if(strict) checkAddress(this.address, true, true);
  }
  ...
  public void setPersonal(String name, String charset)
      throws UnsupportedEncodingException{
    personal=name;
    if(name != null) encodedPersonal=MimeUtility.encodeWord(name, charset, null);
    else encodedPersonal=null;
  }

  /**
   * Set the personal name. If the name contains non US-ASCII characters, then
   * the name will be encoded using the platform's default charset. If the name
   * contains only US-ASCII characters, no encoding is done and the name is used
   * as is.
   * <p>
   * 
   * @param name
  ...
  public void setPersonal(String name)
      throws UnsupportedEncodingException{
    personal=name;
    if(name != null) encodedPersonal=MimeUtility.encodeWord(name);
    else encodedPersonal=null;
  }
   ...
   * resulting string contains only US-ASCII characters, and hence is mail-safe.
   * 
   * @return possibly encoded address string
   */
  public String toString(){
    if(encodedPersonal == null && personal != null) try{
      encodedPersonal=MimeUtility.encodeWord(personal);
    }
    catch(UnsupportedEncodingException ex){
    }

    if(encodedPersonal != null) return quotePhrase(encodedPersonal) + " <" + address + ">";
    else if(isGroup() || isSimple()) return address;
    else return "<" + address + ">";
  }
  ...
  public String toUnicodeString(){
    ...
    if(p != null) return quotePhrase(p) + " <" + address + ">";
    else if(isGroup() || isSimple()) return address;
    else return "<" + address + ">";
  }

  /*
   * quotePhrase() quotes the words within a RFC822 phrase. This is tricky,
   * since a phrase is defined as 1 or more RFC822 words, separated by LWSP.
   * Now, a word that contains LWSP is supposed to be quoted, and this is
   * exactly what the MimeUtility.quote() method does. However, when dealing
   * with a phrase, any LWSP encountered can be construed to be the separator
   * between words, and not part of the words themselves. To deal with this
   * funkiness, we have the below variant of MimeUtility.quote(), which
   * essentially ignores LWSP when deciding whether to quote a word. It aint
  ...
  public boolean equals(Object a){
    if(!(a instanceof InternetAddress)) return false;

    String s=((InternetAddress)a).getAddress();
    if(s == address) return true;
    if(address != null && address.equalsIgnoreCase(s)) return true;
  ...
  public int hashCode(){
    ...
    else return address.toLowerCase(Locale.ENGLISH).hashCode();
  }
   ...
   *              InternetAddress object. Note that this is a RuntimeException.
  ...
  private static int lengthOfLastSegment(String s, int used){
    int pos;
    if((pos=s.lastIndexOf("\r\n")) != -1) return s.length() - pos - 2;
    else return s.length() + used;
  }

  /**
   * Return an InternetAddress object representing the current user. The entire
   * email address may be specified in the "mail.from" property. If not set, the
   * "mail.user" and "mail.host" properties are tried. If those are not set, the
   * "user.name" property and <code>InetAddress.getLocalHost</code> method are
   * tried. Security exceptions that may occur while accessing this information
   * are ignored. If it is not possible to determine an email address, null is
   * returned.
   * 
   * @param session
    ...
    try{
      if(session == null){
        ...
        if(address == null){
          ...
          if(user == null || user.length() == 0) user=session.getProperty("user.name");
          if(user == null || user.length() == 0) user=System.getProperty("user.name");
          host=session.getProperty("mail.host");
          if(host == null || host.length() == 0){
            InetAddress me=InetAddress.getLocalHost();
            if(me != null) host=me.getHostName();
          }
        }
      }

      if(address == null && user != null && user.length() != 0 &&
          host != null && host.length() != 0) address=user + "@" + host;

      if(address != null) return new InternetAddress(address);
    }
   ...
   * are also allowed. If <code>strict</code> is true, many (but not all) of the
   * RFC822 syntax rules are enforced. In particular, even if
   ...
   * this method enforces fewer syntax rules than the <code>parse</code> method
   * when the strict flag is false and enforces more rules when the strict flag
   * is true. If the strict flag is false and the parse is successful in
   * separating out an email address or addresses, the syntax of the addresses
   * themselves is not checked.
   * 
   * @param addresslist
   *          comma separated address strings
   * @param strict
  ...
  /*
   * RFC822 Address parser. XXX - This is complex enough that it ought to be a
   * real parser, not this ad-hoc mess, and because of that, this is not
   * perfect. XXX - Deal with encoded Headers too.
   */
  private static InternetAddress[] parse(String s, boolean strict,
    ...
    for(start=end=-1, index=0; index < length; index++){
      ...
      switch(c){
        case '(': // We are parsing a Comment. Ignore everything inside.
          ...
          rfc822=true;
          if(start >= 0 && end == -1) end=index;
          if(start_personal == -1) start_personal=index + 1;
          ...
          if(nesting > 0) throw new AddressException("Missing ')'", s, index);
          index--; // point to closing paren
          if(end_personal == -1) end_personal=index;
          break;
        ...
        case '<':
          rfc822=true;
          if(route_addr) throw new AddressException("Extra route-addr", s, index);
          if(!in_group){
            start_personal=start;
            if(start_personal >= 0) end_personal=index;
            start=index + 1;
          }
          ...
          if(index >= length){
            if(inquote) throw new AddressException("Missing '\"'", s, index);
            else throw new AddressException("Missing '>'", s, index);
          }
          route_addr=true;
          end=index;
          break;
        ...
        case '"': // parse quoted string
          ...
          if(index >= length) throw new AddressException("Missing '\"'", s, index);
          break;
        ...
        case ',': // end of an address, probably
          ...
          if(rfc822 || strict || parseHdr){
            if(strict || !parseHdr) checkAddress(addr, route_addr, false);
        ...
        case ':':
          rfc822=true;
          if(in_group) throw new AddressException("Nested group", s, index);
          in_group=true;
          if(start == -1) start=index;
          break;
        ...
        case ';':
          if(start == -1) start=index;
          if(!in_group) throw new AddressException(
    ...
    if(start >= 0){
      /*
       * The last token, add this to our InternetAddress vector. Note that this
       * block of code should be identical to the block above for "case ','".
       */
      if(end == -1) end=index;
      String addr=s.substring(start, end).trim();
      if(rfc822 || strict || parseHdr){
   ...
   * even though the syntax of the address may be correct, there's no guarantee
   * that a mailbox of that name exists.
   * 
   * @exception AddressException
   ...
   * simple names.) XXX - much more to check XXX - doesn't handle
  ...
  private static void checkAddress(String addr,
      ...
      throws AddressException{
    int i, start=0;
    if(addr.indexOf('"') >= 0) return; // quote in address, too hard to check
    if(routeAddr){
      /*
       * Check for a legal "route-addr": [@domain[,@domain ...]:]local@domain
       */
      for(start=0; (i=indexOfAny(addr, ",:", start)) >= 0; start=i + 1){
        if(addr.charAt(start) != '@') throw new AddressException("Illegal route-addr", addr);
        if(addr.charAt(i) == ':'){
          // end of route-addr
          start=i + 1;
          break;
    ...
    if((i=addr.indexOf('@', start)) >= 0){
      if(i == start) throw new AddressException("Missing local name", addr);
      if(i == addr.length() - 1) throw new AddressException("Missing domain", addr);
       ...
       */
      if(validate) throw new AddressException("Missing final '@domain'", addr);

      /*
       * No '@' so it's not *really* an RFC822 address, but still we allow some
       * simple forms.
       */
      local=addr;
      domain=null;
    ...
    if(indexOfAny(addr, " \t\n\r") >= 0) throw new AddressException("Illegal whitespace in address", addr);
    // local-part must follow RFC822, no specials except '.'
    if(indexOfAny(local, specialsNoDot) >= 0) throw new AddressException("Illegal character in local name", addr);
    // check for illegal chars in the domain, but ignore domain literals
    if(domain != null && domain.indexOf('[') < 0){
      if(indexOfAny(domain, specialsNoDot) >= 0) throw new AddressException("Illegal character in domain", addr);
    }
  }
  ...
  /**
   * Return the members of a group address. A group may have zero, one, or more
   * members. If this address is not a group, null is returned. The
   ...
   * < argv.length; i++) { InternetAddress[] a = InternetAddress.parse(argv[i]);
   * for (int j = 0; j < a.length; j++) { System.out.println("arg " + i +
   * " address " + j + ": " + a[j]); System.out.println("\tAddress: " +
   * a[j].getAddress() + "\tPersonal: " + a[j].getPersonal()); } if (a.length >
   * 1) { System.out.println("address 0 hash code: " + a[0].hashCode());
