 ...
 * <code>mail.mime.encodeparameters</code> and
 * <code>mail.mime.decodeparameters</code> System properties control whether
 * encoded parameters, as specified by <a
 * href="http://www.ietf.org/rfc/rfc2231.txt">RFC 2231</a>, are supported. By
 * default, such encoded parameters are not supported.
 * <p>
 * Also, in the current implementation, setting the System property
 * <code>mail.mime.decodeparameters.strict</code> to <code>"true"</code> will
 * cause a <code>ParseException</code> to be thrown for errors detected while
 * decoding encoded parameters. By default, if any decoding errors occur, the
...
public class ParameterList{
  ...
  /**
   * The map of name, value pairs. The value object is either a String, for
   * unencoded values, or a Value object, for encoded values, or a MultiValue
   * object, for multi-segment parameters. We use a LinkedHashMap so that
   * parameters are (as much as possible) kept in the original order. Note
   * however that multi-segment parameters (see below) will appear in the
   * position of the first seen segment and orphan segments will all move to the
   * end.
   */
   ...
   * Normally such names are accumulated during the inital parse and processed
   * at the end of the parse, but such names can also be set via the set method
   * when the IMAP provider accumulates pre-parsed pieces of a parameter list.
   * (A special call to the set method tells us when the IMAP provider is done
   * setting parameters.) A multi-segment parameter is defined by RFC 2231. For
   * example, "title*0=part1; title*1=part2", which represents a parameter named
   * "title" with value "part1part2". Note also that each segment of the value
   * might or might not be encoded, indicated by a trailing "*" on the parameter
   * name. If any segment is encoded, the first segment must be encoded. Only
   * the first segment contains the charset and language information needed to
   * decode any encoded segments. RFC 2231 introduces many possible failure
   * modes, which we try to handle as gracefully as possible. Generally, a
   * failure to decode a parameter value causes the non-decoded parameter value
   * to be used instead. Missing segments cause all later segments to be appear
   * as independent parameters with names that include the segment number. For
   * example, "title*0=part1; title*1=part2;
   ...
   * parameters. The map is indexed by "name*seg". The value object is either a
   * String or a Value object. The Value object is not decoded during the
  ...
  /**
   * MWB 3BView: The name of the last parameter added to the map. Used for the
   * AppleMail hack.
   */
  ...
  /**
   * A struct to hold an encoded value. A parsed encoded value is stored as both
   * the decoded value and the original encoded value (so that toString will
   * produce the same result). An encoded value that is set explicitly is stored
   * as the original value and the encoded value, to ensure that get will return
  ...
  /**
   * A struct for a multi-segment parameter. Each entry in the List is either a
   * String or a Value object. When all the segments are present and combined in
   * the combineMultisegmentNames method, the value field contains the combined
   * and decoded value. Until then the value field contains an empty string as a
   * placeholder.
   */
  ...
  /**
   * Constructor that takes a parameter-list string. The String is parsed and
   * the parameters are collected and stored internally. A ParseException is
   * thrown if the parse fails. Note that an empty parameter-list string is
   * valid and will be parsed into an empty ParameterList.
   * 
   * @param s
  ...
  public ParameterList(String s) throws ParseException {
    ...
    for(;;){
      ...
      if((char)type == ';'){
        ...
        if(tk.getType() == HeaderTokenizer.Token.EOF) break;
        // parameter name must be a MIME Atom
        if(tk.getType() != HeaderTokenizer.Token.ATOM) throw new ParseException("Expected parameter name, " +
            "got \"" + tk.getValue() + "\"");
        name=tk.getValue().toLowerCase(Locale.ENGLISH);

        // expect '='
        tk=h.next();
        if((char)tk.getType() != '=') throw new ParseException("Expected '=', " +
            "got \"" + tk.getValue() + "\"");
        ...
        if(type != HeaderTokenizer.Token.ATOM &&
            type != HeaderTokenizer.Token.QUOTEDSTRING) throw new ParseException("Expected parameter value, " +
            "got \"" + tk.getValue() + "\"");

        value=tk.getValue();
        lastName=name;
        if(decodeParameters) putEncodedName(name, value);
    ...
    if(decodeParameters){
      /*
       * After parsing all the parameters, combine all the multi-segment
   ...
   * string as a placeholder, replaced later in combineMultisegmentNames with a
   * MultiValue object. This causes all pieces of the multi-segment parameter to
   * appear in the position of the first seen segment of the parameter.
   */
   ...
   * value, and save all segments in a MultiValue object in the main list
  ...
  private void combineMultisegmentNames(boolean keepConsistentOnFailure)
    ...
    try{
      ...
      while(it.hasNext()){
        ...
        /*
         * Now find all the segments for this name and decode each segment as
         * needed.
         */
        String charset=null;
        int segment;
        for(segment=0;; segment++){
          ...
          if(v instanceof Value){
            try{
              ...
              if(decodeParametersStrict) throw new ParseException(nex.toString());
            }
            catch(UnsupportedEncodingException uex){
              if(decodeParametersStrict) throw new ParseException(uex.toString());
            }
            catch(StringIndexOutOfBoundsException ex){
              if(decodeParametersStrict) throw new ParseException(ex.toString());
            }
      ...
      /*
       * If we get here because of an exception that's going to be thrown
       * (success == false) from the constructor (keepConsistentOnFailure ==
       * false), this is all wasted effort.
       */
  ...
  /**
   * Returns the value of the specified parameter. Note that parameter names are
   * case-insensitive.
   * 
   * @param name
   *          parameter name.
   * @return Value of the parameter. Returns <code>null</code> if the parameter
   *         is not present.
   */
  public String get(String name){
    String value;
    Object v=list.get(name.trim().toLowerCase(Locale.ENGLISH));
    if(v instanceof MultiValue) value=((MultiValue)v).value;
    else if(v instanceof Value) value=((Value)v).value;
    else value=(String)v;
    return value;
  }

  /**
   * Set a parameter. If this parameter already exists, it is replaced by this
   * new value.
   * 
   * @param name
  ...
  public void set(String name, String value){
    ...
    if(name == null && value != null && value.equals("DONE")){
      /*
       * If we've accumulated any multi-segment names from calls to the set
       * method from the IMAP provider, combine the pieces. Ignore any parse
       * errors (e.g., from decoding the values) because it's too late to report
       * them.
       */
  ...
  /**
   * Set a parameter. If this parameter already exists, it is replaced by this
   * new value. If the <code>mail.mime.encodeparameters</code> System property
   * is true, and the parameter value is non-ASCII, it will be encoded with the
   * specified charset, as specified by RFC 2231.
   * 
   * @param name
   *          name of the parameter.
   * @param value
   *          value of the parameter.
   * @param charset
   *          charset of the parameter value.
   * @since JavaMail 1.4
   */
  public void set(String name, String value, String charset){
    if(encodeParameters){
      ...
      if(ev != null) list.put(name.trim().toLowerCase(Locale.ENGLISH), ev);
      else set(name, value);
    }
    else set(name, value);
  }
  ...
  /**
   * Removes the specified parameter from this ParameterList. This method does
  ...
  /**
   * Convert this ParameterList into a MIME String. If this is an empty list, an
  ...
  /**
   * Convert this ParameterList into a MIME String. If this is an empty list, an
   * empty string is returned. The 'used' parameter specifies the number of
   * character positions already taken up in the field into which the resulting
   * parameter list is to be inserted. It's used to determine where to fold the
    ...
    while(e.hasNext()){
      ...
      if(v instanceof MultiValue){
        ...
        for(int i=0; i < vv.size(); i++){
          ...
          else sb.addNV(ns + i, (String)va);
        }
      }
      else if(v instanceof Value) sb.addNV(name + "*", ((Value)v).encodedValue);
      else sb.addNV(name, (String)v);
    }
    return sb.toString();
  }

  /**
   * A special wrapper for a StringBuffer that keeps track of the number of
   * characters used in a line, wrapping to a new line as necessary; for use by
   * the toString method.
   */
  ...
  /**
   * Encode a parameter value, if necessary. If the value is encoded, a Value
   * object is returned. Otherwise, null is returned. XXX - Could return a
   * MultiValue object if parameter value is too long.
   */
  private static Value encodeValue(String value, String charset){
    if(MimeUtility.checkAscii(value) == MimeUtility.ALL_ASCII) return null; // no
    // need
    // to
    // encode
    // it
  ...
  private static Value decodeValue(String value) throws ParseException{
    ...
    try{
      int i=value.indexOf('\'');
      if(i <= 0){
        if(decodeParametersStrict) throw new ParseException(
  ...
  private static String decodeBytes(String value, String charset)
     ...
     * convert the bytes to a String using the specified charset. We'll never
     * need more bytes than encoded characters, so use that to size the array.
     */
