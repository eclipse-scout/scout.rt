 ...
 * A brief description on handling such headers is given below:
 * <p>
...
public class MimeUtility{
   ...
   * in its input stream are US-ASCII, then the encoding is "7bit". If more than
   * half of the bytes are non-US-ASCII, then the encoding is "base64". If less
   ...
   * bytes of its input stream are US-ASCII, the encoding is "7bit". If there is
   * even one non-US-ASCII character, the encoding is "base64".
   * </ul>
   * 
   * @param ds
   *          DataSource
   * @return the encoding. This is either "7bit", "quoted-printable" or "base64"
   */
  ...
  /**
   * Same as <code>getEncoding(DataSource)</code> except that instead of reading
   * the data from an <code>InputStream</code> it uses the <code>writeTo</code>
   * method to examine the data. This is more efficient in the common case of a
   * <code>DataHandler</code> created with an object and a MIME type (for
   * example, a "text/plain" String) because all the I/O is done in this thread.
   * In the case requiring an <code>InputStream</code> the
   * <code>DataHandler</code> uses a thread, a pair of pipe streams, and the
   * <code>writeTo</code> method to produce the data.
   * <p>
   * 
   * @since JavaMail 1.2
   */
  public static String getEncoding(DataHandler dh){
    ...
    /*
     * Try to pick the most efficient means of determining the encoding. If this
     * DataHandler was created using a DataSource, the getEncoding(DataSource)
     * method is typically faster. If the DataHandler was created with an
     * object, this method is much faster. To distinguish the two cases, we use
     * a heuristic. A DataHandler created with an object will always have a null
     * name. A DataHandler created with a DataSource will usually have a
     * non-null name. XXX - This is actually quite a disgusting hack, but it
     * makes a common case run over twice as fast.
     */
   ...
   * They include "base64", "quoted-printable", "7bit", "8bit", and "binary". In
   * addition, "uuencode" is also supported.
   * 
   * @param is
   *          input stream
   * @param encoding
  ...
  public static InputStream decode(InputStream is, String encoding)
    ...
    if(encoding.equalsIgnoreCase("base64")) return new BASE64DecoderStream(is);
    else if(encoding.equalsIgnoreCase("quoted-printable")) return new QPDecoderStream(is);
    else if(encoding.equalsIgnoreCase("uuencode") ||
        encoding.equalsIgnoreCase("x-uuencode") ||
        encoding.equalsIgnoreCase("x-uue")) return new UUDecoderStream(is);
    else if(encoding.equalsIgnoreCase("binary") ||
        encoding.equalsIgnoreCase("7bit") ||
        encoding.equalsIgnoreCase("8bit")) return is;
    else throw new MessagingException("Unknown encoding: " + encoding);
  }

  /**
   * Wrap an encoder around the given output stream. All the encodings defined
   * in RFC 2045 are supported here. They include "base64", "quoted-printable",
   * "7bit", "8bit" and "binary". In addition, "uuencode" is also supported.
   * 
   * @param os
   *          output stream
   * @param encoding
  ...
  public static OutputStream encode(OutputStream os, String encoding)
    ...
    else if(encoding.equalsIgnoreCase("base64")) return new BASE64EncoderStream(os);
    else if(encoding.equalsIgnoreCase("quoted-printable")) return new QPEncoderStream(os);
    else if(encoding.equalsIgnoreCase("uuencode") ||
        encoding.equalsIgnoreCase("x-uuencode") ||
        encoding.equalsIgnoreCase("x-uue")) return new UUEncoderStream(os);
    else if(encoding.equalsIgnoreCase("binary") ||
        encoding.equalsIgnoreCase("7bit") ||
        encoding.equalsIgnoreCase("8bit")) return os;
    else throw new MessagingException("Unknown encoding: " + encoding);
  }
   ...
   * Wrap an encoder around the given output stream. All the encodings defined
   * in RFC 2045 are supported here. They include "base64", "quoted-printable",
   * "7bit", "8bit" and "binary". In addition, "uuencode" is also supported. The
   * <code>filename</code> parameter is used with the "uuencode" encoding and is
  ...
  public static OutputStream encode(OutputStream os, String encoding,
    ...
    if(encoding == null) return os;
        ...
        encoding.equalsIgnoreCase("x-uuencode") ||
        encoding.equalsIgnoreCase("x-uue")) return new UUEncoderStream(os, filename);
    else if(encoding.equalsIgnoreCase("binary") ||
        encoding.equalsIgnoreCase("7bit") ||
        encoding.equalsIgnoreCase("8bit")) return os;
    else throw new MessagingException("Unknown encoding: " + encoding);
   ...
   * <p>
   * Note that this method should be used to encode only "unstructured" RFC 822
   * headers.
   * <p>
   * Example of usage:
   * <p>
   ...
   * string contains only US-ASCII characters, it is returned as-is. If the
   * string contains non US-ASCII characters, it is first character-encoded
   * using the specified charset, then transfer-encoded using either the B or Q
   * encoding. The resulting bytes are then returned as a Unicode string
   ...
   *          the header value
   * @param charset
   *          the charset. If this parameter is null, the platform's default
   *          chatset is used.
   * @param encoding
   *          the encoding to be used. Currently supported values are "B" and
   *          "Q". If this parameter is null, then the "Q" encoding is used if
   *          most of characters to be encoded are in the ASCII charset,
  ...
  public static String decodeText(String etext)
    ...
    /*
     * First, lets do a quick run thru the string and check whether the sequence
     * "=?" exists at all. If none exists, we know there are no encoded-words in
     * here and we can just return the string as-is, without suffering thru the
     * later decoding logic. This handles the most common case of unencoded
     * headers efficiently.
     */
  ...
  /*
   * Encode the given string. The parameter 'encodingWord' should be true if a
   * RFC 822 "word" token is being encoded and false if a RFC 822 "text" token
   * is being encoded. This is because the "Q" encoding defined in RFC 2047 has
   * more restrictions when encoding "word" tokens. (Sigh)
   */
  private static String encodeWord(String string, String charset,
    ...
    if(ascii == ALL_ASCII) return string;
    ...
    if(encoding == null){
      if(ascii != MOSTLY_NONASCII) encoding="Q";
      else encoding="B";
    }

    boolean b64;
    if(encoding.equalsIgnoreCase("B")) b64=true;
    else if(encoding.equalsIgnoreCase("Q")) b64=false;
  ...
  private static void doEncode(String string, boolean b64,
    ...
    if((len > avail) && ((size=string.length()) > 1)){
      ...
      if(foldEncodedWords) buf.append("\r\n "); // start a continuation line
      else buf.append(" "); // line will be folded later
   ...
   * "encoded-word". If the parse fails, a ParseException is thrown. Otherwise,
   * it is transfer-decoded, and then charset-converted into Unicode. If the
   ...
   *              if the string is not an encoded-word as per RFC 2047.
   * @exception UnsupportedEncodingException
   *              if the charset conversion failed.
   */
  public static String decodeWord(String eword)
    ...
    int start=2;
    int pos;
    if((pos=eword.indexOf('?', start)) == -1) throw new ParseException(
    ...
    /*
     * XXX - should include this, but leaving it out for compatibility... if
     * (decodeStrict && pos != eword.length() - 2) throw new ParseException(
     * "encoded word does not end with \"?=\": " + eword););
     */
    ...
    try{
      ...
      if(word.length() > 0){
        ...
        InputStream is;
        if(encoding.equalsIgnoreCase("B")) is=new BASE64DecoderStream(bis);
        else if(encoding.equalsIgnoreCase("Q")) is=new QDecoderStream(bis);
        else throw new UnsupportedEncodingException(
      ...
      if(pos + 2 < eword.length()){
        ...
        if(!decodeStrict) rest=decodeInnerWords(rest);
        decodedWord+=rest;
      }
      return decodedWord;
    }
      ...
      /*
       * An unknown charset of the form ISO-XXX-XXX, will cause the JDK to throw
       * an IllegalArgumentException ... Since the JDK will attempt to create a
       * classname using this string, but valid classnames must not contain the
       * character '-', and this results in an IllegalArgumentException, rather
       * than the expected UnsupportedEncodingException. Yikes
       */
  ...
  /**
   * Look for encoded words within a word. The MIME spec doesn't allow this, but
   * many broken mailers, especially Japanese mailers, produce such incorrect
   * encodings.
   */
  private static String decodeInnerWords(String word)
    ...
    if(start < word.length()) buf.append(word.substring(start));
    return buf.toString();
  }

  /**
   * A utility method to quote a word, if the word contains any characters from
   * the specified 'specials' list.
   * <p>
  ...
  public static String quote(String word, String specials){
    ...
    /*
     * Look for any "bad" characters, Escape and quote the entire string if
     * necessary.
     */
    ...
    for(int i=0; i < len; i++){
      ...
      if(c == '"' || c == '\\' || c == '\r' || c == '\n'){
        ...
        for(int j=i; j < len; j++){
          ...
          else sb.append('\\'); // Escape the character
          sb.append(cc);
          lastc=cc;
        }
        sb.append('"');
   ...
   * characters have been used in the current line; it is usually the length of
   * the header name.
   * <p>
  ...
  public static String fold(int used, String s){
    ...
    while(used + s.length() > 76){
      ...
      for(int i=0; i < s.length(); i++){
        if(lastspace != -1 && used + i > 76) break;
        c=s.charAt(i);
        if(c == ' ' || c == '\t') if(!(lastc == ' ' || lastc == '\t')) lastspace=i;
        lastc=c;
      }
      if(lastspace == -1){
   ...
   * none are found. This should be a method on String.
   */
   ...
   * @param charset
   *          the JDK charset
   * @return the MIME/IANA equivalent. If a mapping is not possible, the passed
   *         in charset itself is returned.
   * @since JavaMail 1.1
   */
  ...
  public static String getDefaultJavaCharset(){
    if(defaultJavaCharset == null){
      /*
       * If mail.mime.charset is set, it controls the default Java charset as
       * well.
       */
      String mimecs=null;
      try{
  ...
  static String getDefaultMIMECharset(){
    ...
    if(defaultMIMECharset == null) defaultMIMECharset=mimeCharset(getDefaultJavaCharset());
   ...
   *         charset. MOSTLY_ASCII if more than half of the available characters
  ...
  static int checkAscii(String s){
    ...
    if(non_ascii == 0) return ALL_ASCII;
    if(ascii > non_ascii) return MOSTLY_ASCII;
   ...
   * @return ALL_ASCII if all characters in the string belong to the US-ASCII
   *         charset. MOSTLY_ASCII if more than half of the available characters
   *         are US-ASCII characters. Else MOSTLY_NONASCII. XXX - this method is
   *         no longer used
   */
   ...
   * checked. If <code>breakOnNonAscii</code> is true the check terminates when
   * the first non-US-ASCII character is found and MOSTLY_NONASCII is returned.
   * Else, the check continues till <code>max</code> bytes or till the end of
   * stream.
   * 
   * @param is
   *          the input stream
   * @param max
   *          maximum bytes to check for. The special value ALL indicates that
   *          all the bytes in this input stream must be checked.
   * @param breakOnNonAscii
   *          if <code>true</code>, then terminate the the check when the first
   *          non-US-ASCII character is found.
   * @return ALL_ASCII if all characters in the string belong to the US-ASCII
   *         charset. MOSTLY_ASCII if more than half of the available characters
   *         are US-ASCII characters. Else MOSTLY_NONASCII.
   */
  static int checkAscii(InputStream is, int max, boolean breakOnNonAscii){
    ...
    while(max != 0){
      try{
        if((len=is.read(buf, 0, block)) == -1) break;
        int lastb=0;
        for(int i=0; i < len; i++){
...
class AsciiOutputStream extends OutputStream{
  ...
  public int getAscii(){
    ...
    // will be base64 encoded (since the quoted-printable encoder
    // doesn't encode this case properly).
    if(badEOL) return MimeUtility.MOSTLY_NONASCII;
    else if(non_ascii == 0){ // no non-us-ascii characters so far
      // if we've seen a long line, we degrade to mostly ascii
      if(longLine) return MimeUtility.MOSTLY_ASCII;
      else return MimeUtility.ALL_ASCII;
    }
    if(ascii > non_ascii) // mostly ascii
