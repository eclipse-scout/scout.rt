...
/**
 * InternetHeaders is a utility class that manages RFC822 style headers. Given
 * an RFC822 format message stream, it reads lines until the blank line that
 * indicates end of header. The input stream is positioned at the start of the
 * body. The lines are stored within the object and can be extracted as either
 * Strings or {@link javax.mail.Header} objects.
 * <p>
...
public class InternetHeaders{
  /**
   * An individual internet header. This class is only used by subclasses of
   * InternetHeaders.
   * <p>
   ...
   * InternetHeader object with a name of ":" marks the location in the list of
   * headers where new headers are added by default.
   * 
   * @since JavaMail 1.4
   */
  protected static final class InternetHeader extends Header{
    /*
     * Note that the value field from the superclass isn't used in this class.
     * We extract the value from the line field as needed. We store the line
     * rather than just the value to ensure that we can get back the exact
     * original line, with the original whitespace, etc.
     */
  ...
  /*
   * The enumeration object used to enumerate an InternetHeaders object. Can
   * return either a String or a Header object.
   */
  static class matchEnum implements Enumeration{
    ...
    /*
     * Constructor. Initialize the enumeration for the entire List of headers,
     * the set of headers, whether to return matching or non-matching headers,
     * and whether to return header lines or Header objects.
     */
    ...
    public boolean hasMoreElements(){
      ...
      // and remember it.
      if(next_header == null) next_header=nextMatch();
    ...
    public Object nextElement(){
      ...
      if(next_header == null) throw new NoSuchElementException("No more headers");

      InternetHeader h=next_header;
      next_header=null;
      if(want_line) return h.line;
      else return new Header(h.getName(), h.getValue());
    }

    /*
     * Return the next Header object according to the match criteria, or null if
     * none left.
     */
    private InternetHeader nextMatch(){
      next: while(e.hasNext()){
        ...
        if(names == null) return match ? null : h;
  ...
  /**
   * The actual list of Headers, including placeholder entries. Placeholder
   * entries are Headers with a null value and are never seen by clients of the
   * InternetHeaders class. Placeholder entries are used to keep track of the
   * preferred order of headers. Headers are never actually removed from the
   * list, they're converted into placeholder entries. New headers are added
   * after existing headers of the same name (or before in the case of
   * <code>Received</code> and <code>Return-Path</code> headers). If no existing
   * header or placeholder for the header is found, new headers are added after
   * the special placeholder with the name ":".
   * 
   * @since JavaMail 1.4
   */
  ...
  /**
   * Create an empty InternetHeaders object. Placeholder entries are inserted to
   * indicate the preferred order of headers.
   */
   ...
   * separating the header from the body. The input stream is left positioned at
   * the start of the body. The header lines are stored internally.
   * <p>
  ...
  /**
   * Read and parse the given RFC822 message stream till the blank line
   * separating the header from the body. Store the header lines inside this
   * InternetHeaders object. The order of header lines is preserved.
   * <p>
   * Note that the header lines are added into this InternetHeaders object, so
   * any existing headers in this object will not be affected. Headers are added
   * to the end of the existing list of headers, in order.
   * 
   * @param is
   *          RFC822 input stream
   */
    ...
    try{
      ...
      do{
        ...
        if(line != null &&
          ...
          // new header
          if(prevline != null) addHeaderLine(prevline);
  ...
  /**
   * Return all the values for the specified header. The values are String
   * objects. Returns <code>null</code> if no headers with the specified name
   * exist.
   * 
   * @param name
   *          header name
   ...
   * only the first header is returned. Returns <code>null</code> if no headers
  ...
  public String getHeader(String name, String delimiter){
    ...
    if((s.length == 1) || delimiter == null) return s[0];
  ...
  /**
   * Change the first header line that matches name to have value, adding a new
   ...
   * order (newest before oldest), and that they should appear at the beginning
   * of the headers, preceeded only by a possible <code>Return-Path</code>
   * header.
   * <p>
   * Note that RFC822 headers can only contain US-ASCII characters.
   * 
  ...
  public void addHeaderLine(String line){
    try{
      ...
      else headers.add(new InternetHeader(line));
    }
