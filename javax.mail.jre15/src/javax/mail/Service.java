 ...
 * <p>
 * A messaging service is created from a <code>Session</code> and is named using
 * a <code>URLName</code>. A service must be connected before it can be used.
...
public abstract class Service{
  ...
  /**
   * Debug flag for this service. Set from the session's debug flag when this
   * service is created.
   */
   ...
   * 
   * @param session
   *          Session object for this service
   * @param urlname
   ...
   * the appropriate authentication schemes. Subclasses that need additional
   * information might want to use some properties or might get it interactively
   * using a popup window.
   * <p>
   * If the connection is successful, an "open" <code>ConnectionEvent</code> is
   * delivered to any <code>ConnectionListeners</code> on this service.
   * <p>
   ...
   * It is an error to connect to an already connected service.
   * <p>
   * The implementation in the Service class will collect defaults for the host,
   * user, and password from the session, from the <code>URLName</code> for this
   ...
   * method returns <code>false</code>, the user will be prompted for any
   * missing information and the <code>protocolConnect</code> method will be
   * called again. The subclass should override the <code>protocolConnect</code>
   ...
   * described above. If the password passed in is null and this is the first
   * successful connection to this service, the user name and the password
   * collected from the user will be saved as defaults for subsequent connection
   * attempts to this same service when using other Service object instances
   * (the connection information is typically always saved within a particular
   * Service object instance). The password is saved using the Session method
   * <code>setPasswordAuthentication</code>. If the password passed in is not
  ...
  /**
   * Connect to the current host using the specified username and password. This
   * method is equivalent to calling the
   * <code>connect(host, user, password)</code> method with null for the host
   * name.
   * 
   * @param user
   *          the user name
  ...
  public synchronized void connect(String host, int port,
    ...
    if(user == null){
      try{
        ...
        if(debug) sex.printStackTrace(session.getDebugOut());
      }
    }
    ...
    if(save) session.setPasswordAuthentication(getURLName(),
   ...
   * actual protocol-specific connection attempt. The default implementation of
   * the <code>connect</code> method calls this method as needed.
   * <p>
   * The <code>protocolConnect</code> method should return <code>false</code> if
   * a user name or password is required for authentication but the
   * corresponding parameter is null; the <code>connect</code> method will
   * prompt the user when needed to supply missing information. This method may
   * also return <code>false</code> if authentication fails for the supplied
   * user name or password. Alternatively, this method may throw an
   * AuthenticationFailedException when authentication fails. This exception may
   * include a String message with more detail about the failure.
   * <p>
   * The <code>protocolConnect</code> method should throw an exception to report
   * failures not related to authentication, such as an invalid host name or
   * port number, loss of a connection during the authentication process,
   * unavailability of the server, etc.
   * 
   * @param host
  ...
  /**
   * Set the connection state of this service. The connection state will
   * automatically be set by the service implementation during the
   * <code>connect</code> and <code>close</code> methods. Subclasses will need
   * to call this method to set the state if the service was automatically
   * disconnected.
   * <p>
   * The implementation in this class merely sets the private field returned by
  ...
  /**
   * Close this service and terminate its connection. A close ConnectionEvent is
   * delivered to any ConnectionListeners. Any Messaging components (Folders,
   * Messages, etc.) belonging to this service are invalid after this service is
   * closed. Note that the service is closed even if this method terminates
   * abnormally by throwing a MessagingException.
   * <p>
   * This implementation uses <code>setConnected(false)</code> to set this
   * service's connected state to <code>false</code>. It will then send a close
   * ConnectionEvent to any registered ConnectionListeners. Subclasses
   * overriding this method to do implementation specific cleanup should call
   * this method as a last step to insure event notification, probably by
   * including a call to <code>super.close()</code> in a <code>finally</code>
   * clause.
   * 
   * @see javax.mail.event.ConnectionEvent
   * @throws MessagingException
  ...
  /**
   * Return a URLName representing this service. The returned URLName does
   * <em>not</em> include the password field.
   * <p>
   ...
   * <code>url</code> field with the password and file information stripped out.
   * 
   * @return the URLName representing this service
   * @see URLName
   */
  public synchronized URLName getURLName(){
    if(url != null && (url.getPassword() != null || url.getFile() != null)) return new URLName(url.getProtocol(), url.getHost(),
  ...
  /**
   * Set the URLName representing this service. Normally used to update the
   * <code>url</code> field after a service has successfully connected.
   * <p>
   * Subclasses should only override this method if their URL does not follow
   * the standard format. In particular, subclasses should override this method
   * if their URL does not require all the possible fields supported by
   * <code>URLName</code>; a new <code>URLName</code> should be constructed with
   * any unneeded fields removed.
   * <p>
   * The implementation in the Service class simply sets the <code>url</code>
   * field.
   * 
   * @see URLName
   */
   ...
   * <p>
   * The default implementation provided here adds this listener to an internal
  ...
  /**
   * Notify all ConnectionListeners. Service implementations are expected to use
   ...
   * dispatches them to the registered ConnectionListeners. Note that the event
   ...
   * EventQueue for this service. We can't synchronize on the service's lock
   * because that might violate the locking hierarchy in some cases.
   */
