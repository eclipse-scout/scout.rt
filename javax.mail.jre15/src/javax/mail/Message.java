...
public abstract class Message implements Part{
  ...
  /**
   * The number of this message within its folder, or zero if the message was
   * not retrieved from a folder.
   */
  ...
  /**
   * Constructor that takes a Folder and a message number. Used by Folder
   * implementations.
   * 
   * @param folder
   *          containing folder
  ...
  /**
   * Returns the "From" attribute. The "From" attribute contains the identity of
  ...
  /**
   * Set the "From" attribute in this Message. The value of this attribute is
   * obtained from the property "mail.user". If this property is absent, the
   * system property "user.name" is used.
   * 
   * @exception MessagingException
  ...
  /**
   * This inner class defines the types of recipients allowed by the Message
   * class. The currently defined types are TO, CC and BCC. Note that this class
   * only has a protected constructor, thereby restricting new Recipient types
   * to either this class or subclasses. This effectively implements an
   * enumeration of the allowed Recipient types. The following code sample shows
   * how to use this class to obtain the "TO" recipients from a message.
   * <blockquote>
   * 
   * <pre>
   * 
  ...
  public static class RecipientType implements Serializable{
    ...
    /**
     * When deserializing a RecipientType, we need to make sure to return only
     * one of the known static final instances defined in this class. Subclasses
     * must implement their own <code>readResolve</code> method that checks for
     * their known instances before calling this super method.
     */
    protected Object readResolve() throws ObjectStreamException{
      if(type.equals("To")) return TO;
      else if(type.equals("Cc")) return CC;
      else if(type.equals("Bcc")) return BCC;
   ...
   * are present in this message. It may return an empty array if the header is
   * present, but contains no addresses.
   * 
   * @param type
   *          the recipient type
   * @return array of Address objects
  ...
  /**
   * Get all the recipient addresses for the message. The default implementation
   * extracts the TO, CC, and BCC recipients using the
   * <code>getRecipients</code> method.
   * <p>
   * This method returns <code>null</code> if none of the recipient headers are
   * present in this message. It may Return an empty array if any recipient
   * header is present, but contains no addresses.
   * 
   * @return array of Address objects
   * @exception MessagingException
   * @see Message.RecipientType#TO
  ...
  public Address[] getAllRecipients() throws MessagingException{
    ...
    if(cc == null && bcc == null) return to; // a common case
  ...
  /**
   * Set the recipient addresses. All addresses of the specified type are
   * replaced by the addresses parameter.
   * 
   * @param type
   *          the recipient type
   * @param addresses
  ...
  /**
   * Set the recipient address. All addresses of the specified type are replaced
  ...
  /**
   * Get the addresses to which replies should be directed. This will usually be
   * the sender of the message, but some messages may direct replies to a
   * different address.
   * <p>
  ...
  /**
   * Set the addresses to which replies should be directed. (Normally only a
   * single address will be specified.) Not all message types allow this to be
   * specified separately from the sender of the message.
   * <p>
   ...
   *              if the underlying implementation does not support setting this
   *              attribute
   */
  ...
  /**
   * Set the specified flags on this message to the specified value. Note that
   * any flags in this message that are not specified in the given
   * <code>Flags</code> object are unaffected.
   * <p>
   * This will result in a <code>MessageChangedEvent</code> being delivered to
   * any MessageChangedListener registered on this Message's containing folder.
   * 
   * @param flag
  ...
  /**
   * Set the specified flag on this message to the specified value. This will
   * result in a <code>MessageChangedEvent</code> being delivered to any
   ...
   * 
   * @param flag
   *          Flags.Flag object containing the flag to be set
   * @param set
   *          the value to be set
   * @exception MessagingException
   * @exception IllegalWriteException
  ...
  /**
   * Get the Message number for this Message. A Message object's message number
   * is the relative position of this Message in its Folder. Note that the
   * message number for a particular Message can change during a session if
   * other messages in the Folder are deleted and expunged.
   * <p>
   * Valid message numbers start at 1. Messages that do not belong to any folder
   * (like newly composed or derived messages) have 0 as their message number.
   * 
  ...
  /**
   * Set the Message number for this Message. This method is invoked only by the
  ...
  /**
   * Get the folder from which this message was obtained. If this is a new
   * message or nested message, this method returns null.
   * 
   ...
   * <code>getMessageNumber()</code> are invalid on an expunged Message object.
   * <p>
   * Messages that are expunged due to an explict <code>expunge()</code> request
   * on the containing Folder are removed from the Folder immediately. Messages
   * that are externally expunged by another source are marked "expunged" and
   * return true for the isExpunged() method, but they are not removed from the
   * Folder until an explicit <code>expunge()</code> is done on the Folder.
   * <p>
  ...
  /**
   * Sets the expunged flag for this Message. This method is to be used only by
   ...
   * new message object will be empty, that is, it will <strong>not</strong>
   * have a "content". These will have to be suitably filled in by the client.
   * <p>
   ...
   * containing folder is closed, if the message is contained in a folder. (Some
   * implementations may save the changes immediately.) Update any header fields
   * to be consistent with the changed message contents. If any part of a
   * message's headers or contents are changed, saveChanges must be called to
   * ensure that those changes are permanent. If saveChanges is not called, any
   * such modifications may or may not be saved, depending on the message store
   ...
   * @return true if the Message matches this search criterion, false otherwise.
   * @exception MessagingException
