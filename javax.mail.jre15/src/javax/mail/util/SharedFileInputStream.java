...
/**
 * A <code>SharedFileInputStream</code> is a <code>BufferedInputStream</code>
 * that buffers data from the file and supports the <code>mark</code> and
 * <code>reset</code> methods. It also supports the <code>newStream</code>
 * method that allows you to create other streams that represent subsets of the
 * file. A <code>RandomAccessFile</code> object is used to access the file data.
 * <p>
 * Note that when the SharedFileInputStream is closed, all streams created with
 * the <code>newStream</code> method are also closed. This allows the creator of
 * the SharedFileInputStream object to control access to the underlying file and
 * ensure that it is closed when needed, to avoid leaking file descriptors. Note
 * also that this behavior contradicts the requirements of SharedInputStream and
...
public class SharedFileInputStream extends BufferedInputStream
  ...
  /**
   * True if this is a top level stream created directly by "new". False if this
   * is a derived stream created by newStream.
   */
  private boolean master=true;

  /**
   * A shared class that keeps track of the references to a particular file so
   * it can be closed when the last reference is gone.
   */
  ...
  /**
   * Creates a <code>SharedFileInputStream</code> for the file.
   * 
   * @param file
   *          the file
   */
  ...
  /**
   * Creates a <code>SharedFileInputStream</code> for the named file
   * 
   * @param file
   *          the file
   */
  ...
  /**
   * Creates a <code>SharedFileInputStream</code> with the specified buffer
   * size.
   * 
   * @param file
   *          the file
  ...
  /**
   * Reads bytes from this stream into the specified byte array, starting at the
   * given offset.
   * <p>
   * This method implements the general contract of the corresponding
   * <code>{@link java.io.InputStream#read(byte[], int, int) read}</code> method
   * of the <code>{@link java.io.InputStream}</code> class.
   * 
   * @param b
   *          destination buffer.
   * @param off
   ...
   * <code>SharedFileInputStream</code> returns <code>true</code>.
   * 
  ...
  public long getPosition(){
    ...
    if(in == null) throw new RuntimeException("Stream closed");
   ...
   * stream. The returned InputStream will also implement the SharedInputStream
   * interface.
   * 
   * @param start
   ...
   * java.util.Random r = new java.util.Random(); int b; while ((b = is.read())
   * >= 0) { System.out.write(b); if (r.nextDouble() < 0.3) { InputStream is2 =
   * is.newStream(is.getPosition(), -1); int b2; while ((b2 = is2.read()) >= 0)
   * ; } } }
   */
