...
public class TransportEvent extends MailEvent{
  ...
  /**
   * Message has been successfully delivered to all recipients by the transport
   * firing this event. validSent[] contains all the addresses this transport
   * sent to successfully. validUnsent[] and invalid[] should be null,
   */
  public static final int MESSAGE_DELIVERED=1;

  /**
   * Message was not sent for some reason. validSent[] should be null.
   * validUnsent[] may have addresses that are valid (but the message wasn't
   * sent to them). invalid[] should likely contain invalid addresses.
   */
   ...
   * 
   * @return Addresses that are valid but to which the message was not sent
   *         successfully or null
   */
  ...
  public void dispatch(Object listener){
    if(type == MESSAGE_DELIVERED) ((TransportListener)listener).messageDelivered(this);
    else if(type == MESSAGE_NOT_DELIVERED) ((TransportListener)listener).messageNotDelivered(this);
