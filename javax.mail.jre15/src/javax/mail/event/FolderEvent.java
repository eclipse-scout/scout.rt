 ...
 * delivered to FolderListeners registered on the affected Folder as well as the
 * containing Store.
 * <p>
 * Service providers vary widely in their ability to notify clients of these
 * events. At a minimum, service providers must notify listeners registered on
 * the same Store or Folder object on which the operation occurs. Service
 * providers may also notify listeners when changes are made through operations
 * on other objects in the same virtual machine, or by other clients in the same
 * or other hosts. Such notifications are not required and are typically not
 * supported by mail protocols (including IMAP).
 * 
 * @author John Mani
 * @author Bill Shannon
 */
...
public class FolderEvent extends MailEvent{
   ...
   *          The source of the event
   * @param oldFolder
   *          The folder that is renamed
   * @param newFolder
   *          The folder that represents the new name
   * @param type
   *          The event type
   * @since JavaMail 1.1
   */
  ...
  public void dispatch(Object listener){
    if(type == CREATED) ((FolderListener)listener).folderCreated(this);
    else if(type == DELETED) ((FolderListener)listener).folderDeleted(this);
    else if(type == RENAMED) ((FolderListener)listener).folderRenamed(this);
  }
}
