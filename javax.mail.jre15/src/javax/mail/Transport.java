...
/**
 * An abstract class that models a message transport. Subclasses provide actual
 * implementations.
 * <p>
...
public abstract class Transport extends Service{
  ...
  /**
   * Send a message. The message will be sent to all recipient addresses
   * specified in the message (as returned from the <code>Message</code> method
   ...
   * Whether or not the message is still sent succesfully to any valid addresses
   * depends on the Transport implementation. See SendFailedException for more
   * details. Note also that success does not imply that the message was
   * delivered to the ultimate recipient, as failures may occur in later stages
   * of delivery. Once a Transport accepts a message for delivery to a
   * recipient, failures that occur later should be reported to the user via
   * another mechanism, such as returning the undeliverable message.
   * <p>
   * Note that <code>send</code> is a static method that creates and manages its
   * own connection. Any connection associated with any Transport instance used
   * to invoke this method is ignored and not used. This method should only be
   * invoked using the form <code>Transport.send(msg);</code>, and should never
   ...
   * specified in the message itself. The <code>send</code> method calls the
   * <code>saveChanges</code> method on the message before sending it.
   * <p>
   * 
   * @param msg
  ...
  private static void send0(Message msg, Address[] addresses)
    ...
    if(addresses == null || addresses.length == 0) throw new SendFailedException("No recipient addresses");
    ...
    if(dsize == 0) throw new SendFailedException("No recipient addresses");
    ...
    /*
     * More than one protocol. Have to do them one at a time and collect
     * addresses and chain exceptions.
     */
    ...
    while(e.hasMoreElements()){
      ...
      try{
        ...
        if(chainedEx == null) chainedEx=sex;
        else chainedEx.setNextException(sex);
        ...
        if(c != null) for(int l=0; l < c.length; l++)
          validUnsent.addElement(c[l]);
      }
      catch(MessagingException mex){
        sendFailed=true;
        // chain the exception we're catching to any previous ones
        if(chainedEx == null) chainedEx=mex;
        else chainedEx.setNextException(mex);
      }
      finally{
        transport.close();
      }
   ...
   * TransportListener registered on this Transport. Also, if any of the
   * addresses is invalid, a SendFailedException is thrown. Whether or not the
   * message is still sent succesfully to any valid addresses depends on the
  ...
  public synchronized void addTransportListener(TransportListener l){
    if(transportListeners == null) transportListeners=new Vector();
  ...
  public synchronized void removeTransportListener(TransportListener l){
    if(transportListeners != null) transportListeners.removeElement(l);
  }
   ...
   * dispatches them to the registered TransportListeners. Note that the event
  ...
  protected void notifyTransportListeners(int type, Address[] validSent,
    ...
    if(transportListeners == null) return;
