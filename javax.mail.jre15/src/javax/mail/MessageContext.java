 ...
 * <code>getMessageContext</code> method of the <code>MessageAware</code>
 * interface. <code>MessageAware</code> is typically implemented by
 * <code>DataSources</code> to allow a <code>DataContentHandler</code> to pass
 * on information about the context in which a data content object is operating.
 * 
...
public class MessageContext{
  ...
  /**
   * Return the Message that contains the content. Follows the parent chain up
   * through containing Multipart objects until it comes to a Message object, or
   * null.
   * 
  ...
  /**
   * Return the Message containing an arbitrary Part. Follows the parent chain
   * up through containing Multipart objects until it comes to a Message object,
   * or null.
   * 
  ...
  private static Message getMessage(Part p) throws MessagingException{
    while(p != null){
      if(p instanceof Message) return (Message)p;
