...
public abstract class Folder{
   ...
   * <code>Folder.READ_ONLY</code>, <code>Folder.READ_WRITE</code>, or -1 if not
   * known.
   * 
   * @since JavaMail 1.1
   */
  ...
  /**
   * Returns the full name of this Folder. If the folder resides under the root
   * hierarchy of this Store, the returned name is relative to the root.
   * Otherwise an absolute name, starting with the hierarchy delimiter, is
   * returned.
   * <p>
   * This method can be invoked on a closed Folder.
   * 
  ...
  /**
   * Return a URLName representing this folder. The returned URLName does
   * <em>not</em> include the password used to access the store.
   * 
  ...
  public URLName getURLName() throws MessagingException{
    ...
    if(fullname != null){
      /*
       * // We need to encode each of the folder's names. StringTokenizer tok =
       * new StringTokenizer( fullname, new Character(separator).toString(),
       * true); while (tok.hasMoreTokens()) { String s = tok.nextToken(); if
       * (s.charAt(0) == separator) encodedName.append(separator); else // XXX -
       * should encode, but since there's no decoder...
    ...
    /*
     * Sure would be convenient if URLName had a constructor that took a base
     * URLName.
     */
  ...
  /**
   * Returns the Store that owns this Folder object. This method can be invoked
   * on a closed Folder.
   * 
   * @return the Store
   */
  ...
  /**
   * Returns the parent folder of this folder. This method can be invoked on a
   * closed Folder. If this folder is the top of a folder hierarchy, this method
   * returns null.
   * <p>
   * Note that since Folder objects are not cached, invoking this method returns
   * a new distinct Folder object.
   * 
  ...
  /**
   * Tests if this folder physically exists on the Store. This method can be
   * invoked on a closed Folder.
   * 
   * @return true if the folder exists, otherwise false
   * @see #create
   * @exception MessagingException
   *              typically if the connection to the server is lost.
   */
  public abstract boolean exists() throws MessagingException;

  /**
   * Returns a list of Folders belonging to this Folder's namespace that match
   * the specified pattern. Patterns may contain the wildcard characters
   * <code>"%"</code>, which matches any character except hierarchy delimiters,
   * and <code>"*"</code>, which matches any character.
   * <p>
   ...
   * "StockOptions".
   * <p>
   * Folder objects are not cached by the Store, so invoking this method on the
   * same pattern multiple times will return that many distinct Folder objects.
   * <p>
   * This method can be invoked on a closed Folder.
   * 
   * @param pattern
   *          the match pattern
   * @return array of matching Folder objects. An empty array is returned if no
   *         matching Folders exist.
   * @see #listSubscribed
   ...
   * that match the specified pattern. If the folder does not support
   * subscription, this method should resolve to <code>list</code>. (The default
   * implementation provided here, does just this). The pattern can contain
   * wildcards as for <code>list</code>.
   * <p>
   ...
   * folder lower in the hierarchy is subscribed. The <code>isSubscribed</code>
   * method on a folder will tell whether any particular folder is actually
   * subscribed.
   * <p>
   * Folder objects are not cached by the Store, so invoking this method on the
   * same pattern multiple times will return that many distinct Folder objects.
   ...
   * @param pattern
   *          the match pattern
   * @return array of matching subscribed Folder objects. An empty array is
   *         returned if no matching subscribed folders exist.
   * @see #list
   * @exception FolderNotFoundException
   *              if this folder does not exist.
   * @exception MessagingException
  ...
  /**
   * Convenience method that returns the list of folders under this Folder. This
   * method just calls the <code>list(String pattern)</code> method with
   * <code>"%"</code> as the match pattern. This method can be invoked on a
   * closed Folder.
   * 
   * @return array of Folder objects under this Folder. An empty array is
   *         returned if no subfolders exist.
   * @see #list
   * @exception FolderNotFoundException
   *              if this folder does not exist.
   * @exception MessagingException
  ...
  /**
   * Convenience method that returns the list of subscribed folders under this
   * Folder. This method just calls the
   * <code>listSubscribed(String pattern)</code> method with <code>"%"</code> as
   * the match pattern. This method can be invoked on a closed Folder.
   * 
   * @return array of subscribed Folder objects under this Folder. An empty
   *         array is returned if no subscribed subfolders exist.
   * @see #listSubscribed
   * @exception FolderNotFoundException
   *              if this folder does not exist.
   * @exception MessagingException
  ...
  /**
   * Return the delimiter character that separates this Folder's pathname from
   * the names of immediate subfolders. This method can be invoked on a closed
   * Folder.
   * 
   * @exception FolderNotFoundException
   *              if the implementation requires the folder to exist, but it
   *              does not
   ...
   * messages or subfolders or both. The returned value is an integer bitfield
   * with the appropriate bits set. This method can be invoked on a closed
   * folder.
   * 
  ...
  /**
   * Create this folder on the Store. When this folder is created, any folders
   * in its path that do not exist are also created.
   * <p>
   ...
   *              if this store does not support subscription
   * @exception MessagingException
   */
  ...
  /**
   * Returns true if this Folder has new messages since the last time this
   * indication was reset. When this indication is set or reset depends on the
   * Folder implementation (and in the case of IMAP, depends on the server).
   ...
   * indicate whether any messages in the Folder have the <code>RECENT</code>
   * flag set.
   * <p>
   * Note that this is not an incremental check for new mail, i.e., it cannot be
   * used to determine whether any new messages have arrived since the last time
   * this method was invoked. To implement incremental checks, the Folder needs
   * to be opened.
   * <p>
  ...
  /**
   * Return the Folder object corresponding to the given name. Note that this
   * folder does not physically have to exist in the Store. The
   * <code>exists()</code> method on a Folder indicates whether it really exists
   * on the Store.
   * <p>
   * In some Stores, name can be an absolute path if it starts with the
   * hierarchy delimiter. Otherwise, it is interpreted relative to this Folder.
   * <p>
   * Folder objects are not cached by the Store, so invoking this method on the
   * same name multiple times will return that many distinct Folder objects.
   * <p>
   * This method can be invoked on a closed Folder.
   * 
   * @param name
   ...
   * itself is deleted. If false, the behaviour is dependent on the folder type
   ...
   * <li>Any messages within the folder are removed. Subfolders are not removed.
   * The folder itself is not removed or affected in any manner. The delete()
   * method returns true. And the exists() method on this folder will return
   * true indicating that this folder still exists. <br>
   * An appropriate FolderEvent is generated by the Store and this folder.
   * <li>Any messages within the folder are removed. Subfolders are not removed.
   * The folder itself changes its type from HOLDS_FOLDERS | HOLDS_MESSAGES to
   * HOLDS_FOLDERS. Thus new messages cannot be added to this folder, but new
   * subfolders can be created underneath. The delete() method returns true
   * indicating success. The exists() method on this folder will return true
   ...
   *              if this folder is not in the closed state.
   * @exception MessagingException
   * @see javax.mail.event.FolderEvent
   */
  ...
  /**
   * The Folder is read only. The state and contents of this folder cannot be
   * modified.
   */
  ...
  /**
   * Open this Folder. This method is valid only on Folders that can contain
   * Messages and that are closed.
   * <p>
   ...
   * <p>
   * The effect of opening multiple connections to the same folder on a specifc
   * Store is implementation dependent. Some implementations allow multiple
   * readers, but only one writer. Others allow multiple writers as well as
   * readers.
   * 
   * @param mode
   ...
   * <p>
   * A CLOSED ConnectionEvent is delivered to any ConnectionListeners registered
   * on this Folder. Note that the folder is closed even if this method
  ...
  /**
   * Return the open mode of this folder. Returns <code>Folder.READ_ONLY</code>,
   * <code>Folder.READ_WRITE</code>, or -1 if the open mode is not known
   * (usually only because an older <code>Folder</code> provider has not been
   ...
   */
  public int getMode(){
    if(!isOpen()) throw new IllegalStateException("Folder not open");
    return mode;
  }

  /**
   * Get the permanent flags supported by this Folder. Returns a Flags object
   * that contains all the flags supported.
   * <p>
   ...
   * <p>
   * This method can be invoked on a closed folder. However, note that for some
   * folder implementations, getting the total message count can be an expensive
   * operation involving actually opening the folder. In such cases, a provider
   * can choose not to support this functionality in the closed state, in which
   * case this method must return -1.
   * <p>
   * Clients invoking this method on a closed folder must be aware that this is
   * a potentially expensive operation. Clients must also be prepared to handle
   * a return value of -1 in this case.
   * 
   ...
   * <p>
   * This method can be invoked on a closed folder. However, note that for some
   * folder implementations, getting the new message count can be an expensive
   * operation involving actually opening the folder. In such cases, a provider
   * can choose not to support this functionality in the closed state, in which
   * case this method must return -1.
   * <p>
  ...
  public synchronized int getNewMessageCount()
    ...
    for(int i=1; i <= total; i++){
      try{
        if(getMessage(i).isSet(Flags.Flag.RECENT)) newmsgs++;
      }
   ...
   * <p>
   * This method can be invoked on a closed folder. However, note that for some
   * folder implementations, getting the unread message count can be an
   * expensive operation involving actually opening the folder. In such cases, a
   * provider can choose not to support this functionality in the closed state,
   * in which case this method must return -1.
   * <p>
   * Clients invoking this method on a closed folder must be aware that this is
   * a potentially expensive operation. Clients must also be prepared to handle
   * a return value of -1 in this case.
   ...
   * This implementation returns -1 if this folder is closed. Else this
   * implementation gets each Message in the folder using
   * <code>getMessage(int)</code> and checks whether its <code>SEEN</code> flag
   * is set. The total number of messages that do not have this flag set is
   * returned.
   * 
   * @return total number of unread messages. -1 may be returned by certain
   *         implementations if this method is invoked on a closed folder.
   * @exception FolderNotFoundException
   *              if this folder does not exist.
   * @exception MessagingException
  ...
  public synchronized int getUnreadMessageCount()
    ...
    for(int i=1; i <= total; i++){
      try{
        if(!getMessage(i).isSet(Flags.Flag.SEEN)) unread++;
      }
      catch(MessageRemovedException me){
        // This is an expunged message, ignore it.
        continue;
   ...
   * <p>
   * This method can be invoked on a closed folder. However, note that for some
   * folder implementations, getting the deleted message count can be an
   * expensive operation involving actually opening the folder. In such cases, a
   * provider can choose not to support this functionality in the closed state,
   * in which case this method must return -1.
   * <p>
   ...
   * returned.
   * 
   * @return number of deleted messages. -1 may be returned by certain
   *         implementations if this method is invoked on a closed folder.
   * @exception FolderNotFoundException
   *              if this folder does not exist.
   * @exception MessagingException
  ...
  public synchronized int getDeletedMessageCount() throws MessagingException{
    ...
    for(int i=1; i <= total; i++){
      try{
        if(getMessage(i).isSet(Flags.Flag.DELETED)) deleted++;
      }
      catch(MessageRemovedException me){
        // This is an expunged message, ignore it.
        continue;
  ...
  /**
   * Get the Message object corresponding to the given message number. A Message
   * object's message number is the relative position of this Message in its
   * Folder. Messages are numbered starting at 1 through the total number of
   * message in the folder. Note that the message number for a particular
   * Message can change during a session if other messages in the Folder are
   * deleted and the Folder is expunged.
   * <p>
   * Message objects are light-weight references to the actual message that get
   * filled up on demand. Hence Folder implementations are expected to provide
   * light-weight Message objects.
   * <p>
  ...
  /**
   * Get the Message objects for message numbers ranging from start through end,
   ...
   *              if this folder is not opened.
   * @exception IndexOutOfBoundsException
   *              if the start or end message numbers are out of range.
   * @exception MessagingException
   */
   ...
   *              if this folder is not opened.
   * @exception IndexOutOfBoundsException
   *              if any message number in the given array is out of range.
   * @exception MessagingException
   */
  ...
  /**
   * Get all Message objects from this Folder. Returns an empty array if the
   * folder is empty. Clients can use Message objects (instead of sequence
   * numbers) as references to the messages within a folder; this method
   * supplies the Message objects to the client. Folder implementations are
   * expected to provide light-weight Message objects, which get filled on
   * demand.
   * <p>
  ...
  /**
   * Append given Messages to this folder. This method can be invoked on a
   * closed Folder. An appropriate MessageCountEvent is delivered to any
   * MessageCountListener registered on this folder when the messages arrive in
   * the folder.
   * <p>
   ...
   * Note that this method is just a hint to the implementation to prefetch the
   * desired items.
   * <p>
   ...
   * useful. Providers wanting to provide a real implementation for this method
  ...
  /**
   * Set the specified flags on the messages specified in the array. This will
   * result in appropriate MessageChangedEvents being delivered to any
   ...
   * Message.
   * <p>
   * This implementation degenerates to invoking <code>setFlags()</code> on each
   * Message object. Specific Folder implementations that can optimize this case
   * should do so. Also, an implementation must not abort the operation if a
   ...
   * @exception IllegalStateException
   *              if this folder is not opened or if it has been opened
   *              READ_ONLY.
   * @exception MessagingException
  ...
  /**
   * Set the specified flags on the messages numbered from start through end,
   * both start and end inclusive. Note that message numbers start at 1, not 0.
   * This will result in appropriate MessageChangedEvents being delivered to any
   * MessageChangedListener registered on this Message's containing folder.
   * <p>
   * Certain Folder implementations can optimize the operation of setting Flags
   * for a group of messages, so clients might want to use this method, rather
   * than invoking <code>Message.setFlags</code> for each Message.
   * <p>
   * The default implementation uses <code>getMessage(int)</code> to get each
   * <code>Message</code> object and then invokes <code>setFlags</code> on that
   * object to set the flags. Specific Folder implementations that can optimize
   * this case should do so. Also, an implementation must not abort the
   * operation if a message number refers to an expunged message.
   * 
   * @param start
   *          the number of the first message
   * @param end
  ...
  /**
   * Set the specified flags on the messages whose message numbers are in the
   * array. This will result in appropriate MessageChangedEvents being delivered
  ...
  /**
   * Copy the specified Messages from this Folder into another Folder. This
   * operation appends these Messages to the destination Folder. The destination
   * Folder does not have to be opened. An appropriate MessageCountEvent is
   * delivered to any MessageCountListener registered on the destination folder
   * when the messages arrive in the folder.
   * <p>
   * Note that the specified Message objects <strong>must</strong> belong to
   ...
   * implementation must not abort the operation if a Message in the array turns
   * out to be an expunged Message.
   * 
   * @param msgs
   *          the array of message objects
   * @param folder
  ...
  public void copyMessages(Message[] msgs, Folder folder)
      throws MessagingException{
    if(!folder.exists()) throw new FolderNotFoundException(
  ...
  /**
   * Expunge (permanently remove) messages marked DELETED. Returns an array
   * containing the expunged message objects. The <code>getMessageNumber</code>
   * method on each of these message objects returns that Message's original
   * (that is, prior to the expunge) sequence number. A MessageCountEvent
   * containing the expunged messages is delivered to any MessageCountListeners
   * registered on the folder.
   * <p>
   ...
   * use Message objects as references to messages, rather than message numbers.
   * Any expunged Messages objects still have to be pruned, but other Messages
   * in that folder are not affected by the expunge.
   * <p>
   * After a message is expunged, only the <code>isExpunged</code> and
   * <code>getMessageNumber</code> methods are still valid on the corresponding
   * Message object; other methods may throw
   ...
   *              if the search term is too complex for the implementation to
   *              handle.
   * @exception FolderNotFoundException
   *              if this folder does not exist.
   * @exception IllegalStateException
   ...
   * search criterion. Returns an array containing the matching messages.
   ...
   * <code>match()</code> method with the given term. The messages that succeed
   * in the match are returned. Providers that can implement server-side
   * searching might want to override this method to provide a more efficient
   * implementation. If the search term is too complex or contains user-defined
   * terms that cannot be executed on the server, providers may elect to either
   * throw a SearchException or degenerate to client-side searching by calling
  ...
  /*
   * The set of listeners are stored in Vectors appropriate to their type. We
   * mark all listener Vectors as "volatile" because, while we initialize them
   * inside this folder's synchronization lock, they are accessed (checked for
   * null) in the "notify" methods, which can't be synchronized due to lock
   * ordering constraints. Since the listener fields (the handles on the Vector
   * objects) are only ever set, and are never cleared, we believe this is safe.
   * The code that dispatches the notifications will either see the null and
   * assume there are no listeners or will see the Vector and will process the
   * listeners. There's an inherent race between adding a listener and notifying
   * the listeners; the lack of synchronization during notification does not
   * make the race condition significantly worse. If one thread is setting a
   * listener at the "same" time an event is being dispatched, the dispatch code
   * might not see the listener right away. The dispatch code doesn't have to
   * worry about the Vector handle being set to null, and thus using an
   * out-of-date set of listeners, because we never set the field to null.
   */
   ...
   * <p>
   * The implementation provided here adds this listener to an internal list of
   * ConnectionListeners.
   * 
   * @param l
  ...
  public synchronized void removeConnectionListener(ConnectionListener l){
    if(connectionListeners != null) connectionListeners.removeElement(l);
  }

  /**
   * Notify all ConnectionListeners. Folder implementations are expected to use
   * this method to broadcast connection events.
   * <p>
   * The provided implementation queues the event into an internal event queue.
   * An event dispatcher thread dequeues events from the queue and dispatches
   * them to the registered ConnectionListeners. Note that the event dispatching
   * occurs in a separate thread, thus avoiding potential deadlock problems.
   * 
   * @param type
  ...
  protected void notifyConnectionListeners(int type){
    ...
    /*
     * Fix for broken JDK1.1.x Garbage collector : The 'conservative' GC in
     * JDK1.1.x occasionally fails to garbage-collect Threads which are in the
     * wait state. This would result in thread (and consequently memory) leaks.
     * We attempt to fix this by sending a 'terminator' event to the queue,
     * after we've sent the CLOSED event. The terminator event causes the
     * event-dispatching thread to self destruct.
     */
    if(type == ConnectionEvent.CLOSED) terminateQueue();
  }
  ...
  public synchronized void addFolderListener(FolderListener l){
    if(folderListeners == null) folderListeners=new Vector();
  ...
  public synchronized void removeFolderListener(FolderListener l){
    if(folderListeners != null) folderListeners.removeElement(l);
  }

  /**
   * Notify all FolderListeners registered on this Folder and this folder's
   * Store. Folder implementations are expected to use this method to broadcast
   * Folder events.
   * <p>
   ...
   * this folder's Store to notify any FolderListeners registered on the store.
   * 
   * @param folder
  ...
  public synchronized void addMessageCountListener(MessageCountListener l){
    if(messageCountListeners == null) messageCountListeners=new Vector();
  ...
  public synchronized void removeMessageCountListener(MessageCountListener l){
    if(messageCountListeners != null) messageCountListeners.removeElement(l);
  }

  /**
   * Notify all MessageCountListeners about the addition of messages into this
   * folder. Folder implementations are expected to use this method to broadcast
   * MessageCount events for indicating arrival of new messages.
   * <p>
   * The provided implementation queues the event into an internal event queue.
   * An event dispatcher thread dequeues events from the queue and dispatches
   * them to the registered MessageCountListeners. Note that the event
   * dispatching occurs in a separate thread, thus avoiding potential deadlock
   * problems.
   */
  protected void notifyMessageAddedListeners(Message[] msgs){
    if(messageCountListeners == null) return;
   ...
   * Folder. Folder implementations are expected to use this method to broadcast
   * MessageCount events indicating removal of messages.
   * <p>
   * The provided implementation queues the event into an internal event queue.
   * An event dispatcher thread dequeues events from the queue and dispatches
   * them to the registered MessageCountListeners. Note that the event
  ...
  public synchronized void addMessageChangedListener(MessageChangedListener l){
    if(messageChangedListeners == null) messageChangedListeners=new Vector();
  ...
  public synchronized void removeMessageChangedListener(MessageChangedListener l){
    if(messageChangedListeners != null) messageChangedListeners.removeElement(l);
  }

  /**
   * Notify all MessageChangedListeners. Folder implementations are expected to
   * use this method to broadcast MessageChanged events.
   * <p>
   * The provided implementation queues the event into an internal event queue.
   * An event dispatcher thread dequeues events from the queue and dispatches
   * them to registered MessageChangedListeners. Note that the event dispatching
   * occurs in a separate thread, thus avoiding potential deadlock problems.
   */
  protected void notifyMessageChangedListeners(int type, Message msg){
    if(messageChangedListeners == null) return;
  ...
  /*
   * A lock for creating the EventQueue object. Only one thread should create an
   * EventQueue for this folder. We can't synchronize on the folder's lock
   * because that would violate the locking hierarchy in some cases. For
   * details, see the IMAP provider.
   */
  ...
  private void queueEvent(MailEvent event, Vector vector){
    ...
    synchronized(qLock){
      if(q == null) q=new EventQueue();
    }

    /*
     * Copy the vector in order to freeze the state of the set of EventListeners
     * the event should be delivered to prior to delivery. This ensures that any
     * changes made to the Vector from a target listener's method during the
     * delivery of this event will not take effect until after the event is
     * delivered.
     */
  ...
  /**
   * override the default toString(), it will return the String from
   * Folder.getFullName() or if that is null, it will use the default toString()
   * behavior.
   */
