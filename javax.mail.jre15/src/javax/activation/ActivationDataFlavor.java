 ...
 * <code>java.awt.datatransfer.DataFlavor</code>. It allows the JAF to set all
 * three values stored by the DataFlavor class via a new constructor. It also
 * contains improved MIME parsing in the <code>equals
 * </code> method. Except for the improved
 * parsing, its semantics are identical to that of the JDK's DataFlavor class.
 */

public class ActivationDataFlavor extends DataFlavor{

  /*
   * Raison d'etre: The DataFlavor class included in JDK 1.1 has several
   * limitations including piss poor MIME type parsing, and the limitation of
   * only supporting serialized objects and InputStreams as representation
   * objects. This class 'fixes' that.
   */
   ...
   * The returned DataFlavor will have the following characteristics:
   * <p>
   * If the mimeType is "application/x-java-serialized-object; class=", the
   * result is the same as calling new DataFlavor(Class.forName()) as above.
   * <p>
   * otherwise:
   * <p>
   ...
   * <code>isMimeTypeEqual</code> method.
   * 
   * @param dataFlavor
   *          the DataFlavor to compare with
  ...
  /**
   * Is the string representation of the MIME type passed in equivalent to the
   * MIME type of this DataFlavor.
   * <p>
   * ActivationDataFlavor delegates the comparison of MIME types to the MimeType
   * class included as part of the JavaBeans Activation Framework. This provides
   * a more robust comparison than is normally available in the DataFlavor
   * class.
   * 
   * @param mimeType
   *          the MIME type
  ...
  public boolean isMimeTypeEqual(String mimeType){
    MimeType mt=null;
    try{
      if(mimeObject == null) mimeObject=new MimeType(this.mimeType);
   ...
   * parameters, whose values are case insensitive. (MIME type parameter values
   * are supposed to be case sensitive).
   * <p>
   * This method is called for each parameter name/value pair and should return
   * the normalized representation of the parameterValue. This method is never
   ...
   * where none are present in the MIME type string passed in. This method is
   * never invoked by this implementation.
   * 
   * @param mimeType
   *          the MIME type
