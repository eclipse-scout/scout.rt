...
public class POP3Message extends MimeMessage{
   ...
   * POP3Message so we have to be careful to drop our lock before calling back
   * to the folder to close it and notify of connection lost events.
   */
  ...
  public void setFlags(Flags newFlags, boolean set)
    ...
    if(!flags.equals(oldFlags)) folder.notifyMessageChangedListeners(
        MessageChangedEvent.FLAGS_CHANGED, this);
  }

  /**
   * Return the size of the content of this message in bytes. Returns -1 if the
  ...
  public int getSize() throws MessagingException{
    try{
      synchronized(this){
        ...
        if(msgSize < 0){
          /*
           * Use LIST to determine the entire message size and subtract out the
           * header size (which may involve loading the headers, which may load
           * the content as a side effect). If the content is loaded as a side
           * effect of loading the headers, get the size directly.
           */
          if(headers == null) loadHeaders();
          if(contentStream != null) msgSize=contentStream.available();
          else msgSize=folder.getProtocol().list(msgnum) - hdrSize;
        }
        return msgSize;
      }
    }
  ...
  /**
   * Produce the raw bytes of the content. The data is fetched using the POP3
   * RETR command.
   * 
   * @see #contentStream
   */
      ...
      synchronized(this){
        if(contentStream == null){
          ...
          if(headers == null ||
            ...
            /*
             * Already have the headers, have to skip the headers in the content
             * array and return the body. XXX - It seems that some mail servers
             * return slightly different headers in the RETR results than were
             * returned in the TOP results, so we can't depend on remembering
             * the size of the headers from the TOP command and just skipping
             * that many bytes. Instead, we have to process the content,
             * skipping over the header until we come to the empty line that
   ...
   * fetched again from the server the next time it is needed. If
   * <code>invalidateHeaders</code> is true, invalidate the headers as well.
   * 
  ...
  /**
   * Fetch the header of the message and the first <code>n</code> lines of the
   * raw content of the message. The headers and data are available in the
  ...
  /**
   * Set the value for this header_name. Throws IllegalWriteException because
   ...
   *              because the underlying implementation does not support
   *              modification
  ...
  /**
   * Add this value to the existing values for this header_name. Throws
   * IllegalWriteException because POP3 messages are read-only.
   * 
   * @param name
   *          header name
   * @param value
  ...
  /**
   * Remove all headers with this name. Throws IllegalWriteException because
   * POP3 messages are read-only.
   * 
   * @exception IllegalWriteException
   *              because the underlying implementation does not support
  ...
  /**
   * Add a raw RFC822 header-line. Throws IllegalWriteException because POP3
   ...
   * raw RFC822 header-line, containing both the "name" and "value" field.
   * 
   * @exception MessagingException
   */
   ...
   * is a raw RFC822 header-line, containing both the "name" and "value" field.
   * 
   * @exception MessagingException
   */
