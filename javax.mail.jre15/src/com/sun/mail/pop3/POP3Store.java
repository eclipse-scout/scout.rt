...
/**
 * A POP3 Message Store. Contains only one folder, "INBOX". See the <a
...
public class POP3Store extends Store{
  ...
  public POP3Store(Session session, URLName url,
    ...
    if(s != null && s.equalsIgnoreCase("true")) rsetBeforeQuit=true;

    s=session.getProperty("mail." + name + ".disabletop");
    if(s != null && s.equalsIgnoreCase("true")) disableTop=true;

    s=session.getProperty("mail." + name + ".forgettopheaders");
    if(s != null && s.equalsIgnoreCase("true")) forgetTopHeaders=true;

    s=session.getProperty("mail." + name + ".message.class");
    if(s != null){
      if(session.getDebug()) session.getDebugOut().println(
  ...
  protected synchronized boolean protocolConnect(String host, int portNum,
    ...
    if(host == null || passwd == null || user == null) return false;
    ...
    if(portNum == -1){
      ...
      if(portstring != null) portNum=Integer.parseInt(portstring);
    }

    if(portNum == -1) portNum=defaultPort;
   ...
   */
  /*
   * Note that we maintain somewhat of an illusion of being connected even if
   * we're not really connected. This is because a Folder can use the connection
   * and close it when it's done. If we then ask whether the Store's connected
   * we want the answer to be true, as long as we can reconnect at that point.
   * This means that we need to be able to reconnect the Store on demand.
   */
  ...
  synchronized Protocol getPort(POP3Folder owner) throws IOException{
    ...
    /*
     * If a Folder closes the port, and then a Folder is opened, the Store won't
     * have a port. In that case, the getPort call will come from Folder.open,
     * but we need to keep track of the port in the Store so that a later call
     * to Folder.isOpen, which calls Store.isConnected, will use the same port.
     */
    if(port == null && owner != null){
      port=p;
      portOwner=owner;
    }
    if(portOwner == null) portOwner=owner;
    return p;
  }
