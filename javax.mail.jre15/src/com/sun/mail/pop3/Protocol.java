 ...
 * requests. APOP support courtesy of "chamness".
 * 
 * @author Bill Shannon
 */
class Protocol{
  ...
  Protocol(String host, int port, boolean debug, PrintStream out,
    ...
    try{
      ...
      if(debug) out.println("DEBUG POP3: connecting to host \"" + host +
    ...
    if(enableAPOP){
      ...
      if(challStart != -1 && challEnd != -1) apopChallenge=r.data.substring(challStart, challEnd + 1);
      if(debug) out.println("DEBUG POP3: APOP challenge: " + apopChallenge);
    }
  }
  ...
  synchronized String login(String user, String password)
    ...
    Response r;
    String dpw=null;
    if(apopChallenge != null) dpw=getDigest(password);
    if(apopChallenge != null && dpw != null){
      r=simpleCommand("APOP " + user + " " + dpw);
    }
    else{
      r=simpleCommand("USER " + user);
      if(!r.ok) return r.data != null ? r.data : "USER command failed";
      r=simpleCommand("PASS " + password);
    }
    if(!r.ok) return r.data != null ? r.data : "login failed";
    return null;
  }

  /**
   * Gets the APOP message digest. From RFC 1939: The 'digest' parameter is
   * calculated by applying the MD5 algorithm [RFC1321] to a string consisting
   * of the timestamp (including angle-brackets) followed by a shared secret.
  ...
  /**
   * Retrieve the specified message. Given an estimate of the message's size we
   * can be more efficient, preallocating the array and returning a
  ...
  synchronized String uidl(int msg) throws IOException{
    ...
    if(i > 0) return r.data.substring(i + 1);
    else return null;
  }

  /**
   * Return the UIDL strings for all messages. The UID for msg #N is returned in
   * uids[N-1].
   */
  synchronized boolean uidl(String[] uids) throws IOException{
    ...
    while((line=lis.readLine()) != null){
      int i=line.indexOf(' ');
      if(i < 1 || i >= line.length()) continue;
      int n=Integer.parseInt(line.substring(0, i));
      if(n > 0 && n <= uids.length) uids[n - 1]=line.substring(i + 1);
    }
    return true;
  }
  ...
  private Response simpleCommand(String cmd) throws IOException{
    if(socket == null) throw new IOException("Folder is closed"); // XXX
    if(cmd != null){
      if(debug) out.println("C: " + cmd);
      cmd+=CRLF;
    ...
    if(line == null){
      if(debug) out.println("S: EOF");
      throw new EOFException("EOF on socket");
    }
    if(debug) out.println("S: " + line);
    Response r=new Response();
    if(line.startsWith("+OK")) r.ok=true;
    else if(line.startsWith("-ERR")) r.ok=false;
    else throw new IOException("Unexpected response: " + line);
    int i;
    if((i=line.indexOf(' ')) >= 0) r.data=line.substring(i + 1);
    return r;
  }

  /**
   * Issue a POP3 command that expects a multi-line response. <code>size</code>
  ...
  private Response multilineCommand(String cmd, int size) throws IOException{
    ...
    if(b < 0) throw new EOFException("EOF on socket");
