...
public class ResponseInputStream{
  ...
  public ByteArray readResponse(ByteArray ba) throws IOException{
    if(ba == null) ba=new ByteArray(new byte[128], 0, 128);
    ...
    for(;;){ // read until CRLF with no preceeding literal
      ...
      while(!gotCRLF &&
        ...
        switch(b){
          case '\n':
            if((idx > 0) && buffer[idx - 1] == '\r') gotCRLF=true;
          default:
      ...
      if(count > 0){
        ...
        /*
         * read() might not return all the bytes in one shot, so call repeatedly
         * till we are done
         */
        int actual;
        while(count > 0){
