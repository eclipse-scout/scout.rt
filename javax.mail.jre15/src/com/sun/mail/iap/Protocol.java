...
public class Protocol{
  ...
  public synchronized void removeResponseHandler(ResponseHandler h){
    if(handlers != null) handlers.removeElement(h);
  }
  ...
  protected void processGreeting(Response r) throws ProtocolException{
    if(r.isBYE()) throw new ConnectionException(this, r);
  }
  ...
  /**
   * Returns whether this Protocol supports non-synchronizing literals Default
   * is false. Subclasses should override this if required
   */
  ...
  /**
   * Return a buffer to be used to read a response. The default implementation
   * returns null, which causes a new buffer to be allocated for every response.
   * 
  ...
  /**
   * Send a command to the server. Collect all responses until either the
   * corresponding command completion response or a BYE response (indicating
   * server failure). Return all the collected responses.
   * 
   * @param command
   *          the command
   * @param args
  ...
  public synchronized Response[] command(String command, Argument args){
    ...
    while(!done){
      ...
      if(r.isTagged() && r.getTag().equals(tag)) done=true;
    }
  ...
  /**
   * Convenience routine to handle simple IAP commands that do not have
   * responses specific to that command.
   */
   ...
   * issue to start TLS negotiation. If the command succeeds, we begin TLS
   * negotiation.
   */
