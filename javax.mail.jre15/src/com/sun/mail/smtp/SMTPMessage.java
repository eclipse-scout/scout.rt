...
/**
 * This class is a specialization of the MimeMessage class that allows you to
 * specify various SMTP options and parameters that will be used when this
 * message is sent over SMTP. Simply use this class instead of MimeMessage and
 * set SMTP options using the methods on this class.
 * <p>
...
public class SMTPMessage extends MimeMessage{
   ...
   * specified MIME InputStream. The InputStream will be left positioned at the
   * end of the data for the message. Note that the input stream parse is done
   * within this constructor itself.
   * 
   * @param session
  ...
  /**
   * Set the From address to appear in the SMTP envelope. Note that this is
   * different than the From address that appears in the message itself. The
   * envelope From address is typically used when reporting errors. See <A
   * HREF="http://www.ietf.org/rfc/rfc821.txt">RFC 821</A> for details.
   * <p>
   ...
   * Notification (<A HREF="http://www.ietf.org/rfc/rfc1891.txt">RFC 1891</A>).
  ...
  public void setNotifyOptions(int options){
    if(options < -1 || options >= 8) throw new IllegalArgumentException("Bad return option");
  ...
  /**
   * Return notification options as an RFC 1891 string. Returns null if no
   * options set.
   */
  String getDSNNotify(){
    if(notifyOptions == 0) return null;
    if(notifyOptions == NOTIFY_NEVER) return "NEVER";
    StringBuffer sb=new StringBuffer();
    if((notifyOptions & NOTIFY_SUCCESS) != 0) sb.append("SUCCESS");
  ...
  /**
   * Set return option to be used if server supports Delivery Status
   * Notification (<A HREF="http://www.ietf.org/rfc/rfc1891.txt">RFC 1891</A>).
  ...
  public void setReturnOption(int option){
    if(option < 0 || option > RETURN_HDRS) throw new IllegalArgumentException("Bad return option");
    returnOption=option;
  }
   ...
   * of this message that use the "quoted-printable" or "base64" encodings are
   * converted to use "8bit" encoding if they follow the RFC 2045 rules for 8bit
   * text.
   * <p>
  ...
  /**
   * If set to true, and this message has some valid and some invalid addresses,
  ...
  /**
   * Gets the submitter to be used for the RFC 2554 AUTH= value in the MAIL FROM
   * command.
   * 
  ...
  /**
   * Sets the submitter to be used for the RFC 2554 AUTH= value in the MAIL FROM
   * command. Normally only used by a server that's relaying a message. Clients
   * will typically not set a submitter. See <A
   * HREF="http://www.ietf.org/rfc/rfc2554.txt">RFC 2554</A> for details.
   * 
   * @param submitter
  ...
  /**
   * Set the extension string to use with the MAIL command. The extension string
   ...
   * {@link com.sun.mail.smtp.SMTPTransport SMTPTransport} method
   * {@link com.sun.mail.smtp.SMTPTransport#supportsExtension supportsExtension}
   * to verify that the server supports the desired service extension. See <A
   * HREF="http://www.ietf.org/rfc/rfc1869.txt">RFC 1869</A> and other RFCs that
