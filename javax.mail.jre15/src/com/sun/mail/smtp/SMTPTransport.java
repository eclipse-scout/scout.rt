 ...
 *   if(supportsExtension(&quot;XACCOUNTING&quot;)) issueCommand(&quot;XACT&quot;, 25);
 *   return super.data();
 * }
 * </pre>
 * 
...
public class SMTPTransport extends Transport{
  ...
  protected SMTPTransport(Session session, URLName urlname,
    ...
    if(urlname != null) name=urlname.getProtocol();
    this.name=name;
  ...
  public synchronized String getLocalHost(){
    try{
      ...
      if(localHostName == null || localHostName.length() <= 0) localHostName=
   ...
   * the caller. Useful for implementing the SMTP ATRN command (RFC 2645) where
   * an existing connection is used when the server reverses roles and becomes
   * the client.
   * 
  ...
  /**
   * Should we use the RSET command instead of the NOOP command in the @{link
   * #isConnected isConnected} method?
   * 
   ...
   * the @{link #isConnected isConnected} method.
   * 
   * @param useRset
  ...
  private synchronized DigestMD5 getMD5(){
    if(md5support == null) md5support=new DigestMD5(debug ? out : null);
    return md5support;
  }
   ...
   * using the ESMTP command EHLO. If mail.smtp.auth is set to true, we insist
   * on having a username and password, and will try to authenticate ourselves
   * if the server supports the AUTH extension (RFC 2554).
   * 
   * @param host
  ...
  protected boolean protocolConnect(String host, int port, String user,
    ...
    DigestMD5 md5;
    if(debug) out.println("DEBUG SMTP: useEhlo " + useEhlo +
        ", useAuth " + useAuth);

    /*
     * If mail.smtp.auth is set, make sure we have a valid username and
     * password, even if we might not end up using it (e.g., because the server
     * doesn't support ESMTP or doesn't support the AUTH extension).
     */
    if(useAuth && (user == null || passwd == null)) return false;

    /*
     * If port is not specified, set it to value of mail.smtp.port property if
     * it exists, otherwise default to 25.
     */
    if(port == -1){
    ...
    if(serverSocket != null) openServer(); // only happens from connect(socket)
    else openServer(host, port);

    if(useEhlo) succeed=ehlo(getLocalHost());
    if(!succeed) helo(getLocalHost());

    if(useStartTLS && supportsExtension("STARTTLS")){
      startTLS();
      /*
       * Have to issue another EHLO to update list of extensions supported,
       * especially authentication mechanisms. Don't know if this could ever
       * fail, but we ignore failure.
       */
      ehlo(getLocalHost());
    }
    ...
    if((useAuth || (user != null && passwd != null)) &&
      ...
      if(debug){
        ...
        if(!supportsAuthentication("LOGIN") &&
            supportsExtension("AUTH=LOGIN")) out.println("DEBUG SMTP: use AUTH=LOGIN hack");
      }
      ...
      if(supportsAuthentication("LOGIN") ||
        ...
        /*
         * A 530 response indicates that the server wants us to issue a STARTTLS
         * command first. Do that and try again.
         */
        if(resp == 530){
          startTLS();
         ...
         * encoded username and password strings. Also note that unlike the B64
         * definition in MIME, CRLFs should *not* be inserted during the
         * encoding process. So I use Integer.MAX_VALUE (0x7fffffff (> 1G)) as
         * the bytesPerLine, which should be sufficiently large!
         */
        try{
        ...
        try{
          ...
          if(debug) out.println("DEBUG SMTP: DIGEST-MD5: " + ex);
        }
        finally{
          if(resp != 235){
            closeConnection();
  ...
  public synchronized void sendMessage(Message message, Address[] addresses)
    ...
    if(!(message instanceof MimeMessage)){
      if(debug) out.println("DEBUG SMTP: Can only send RFC822 msgs");
    ...
    if(message instanceof SMTPMessage) use8bit=((SMTPMessage)message).getAllow8bitMIME();
    if(!use8bit){
      String ebStr=
          session.getProperty("mail." + name + ".allow8bitmime");
      use8bit=ebStr != null && ebStr.equalsIgnoreCase("true");
    }
    if(debug) out.println("DEBUG SMTP: use8bit " + use8bit);
    ...
    try{
      ...
      if(sendPartiallyFailed){
        ...
        if(debug) out.println("DEBUG SMTP: Sending partially failed " +
      ...
      if(debug) mex.printStackTrace(out);
    ...
    try{
      if(serverSocket != null){
        ...
        if(quitWait){
          ...
          if(resp != 221 && resp != -1) out.println("DEBUG SMTP: QUIT failed with " + resp);
        }
      }
    }
    finally{
  ...
  public synchronized boolean isConnected(){
    ...
    try{
      ...
      if(useRset) sendCommand("RSET");
      else sendCommand("NOOP");
      int resp=readServerResponse();
  ...
  private void expandGroups(){
    ...
    for(int i=0; i < addresses.length; i++){
      ...
      if(a.isGroup()){
        ...
        if(groups != null) groups.addElement(a);
      }
    }
   ...
   * 
   * @return true if any changes were made XXX - This is really quite a hack.
   */
    ...
    try{
      if(part.isMimeType("text/*")){
        ...
        if(enc != null && (enc.equalsIgnoreCase("quoted-printable") ||
          ...
          if(is8Bit(is)){
            /*
             * If the message was created using an InputStream then we have to
             * extract the content as an object and set it back as an object so
             * that the content will be re-encoded. If the message was not
             * created using an InputStream, the following should have no
             * effect.
             */
        ...
        for(int i=0; i < count; i++){
          if(convertTo8Bit((MimePart)mp.getBodyPart(i))) changed=true;
        }
      }
    }
   ...
   * text. Lines have to be 998 characters or less and no NULs are allowed. CR
   * and LF must occur in pairs but we don't check that because we assume this
   * is text and we convert all CR/LF combinations into canonical CRLF later.
   */
  private boolean is8Bit(InputStream is){
    ...
    try{
      while((b=is.read()) >= 0){
        ...
        if(b > 0x7f) need8bit=true;
      }
    }
    catch(IOException ex){
      return false;
    }
    if(debug && need8bit) out.println("DEBUG SMTP: found an 8bit part");
    return need8bit;
  }
  ...
  protected void helo(String domain) throws MessagingException{
    if(domain != null) issueCommand("HELO " + domain, 250);
    else issueCommand("HELO", 250);
  }
  ...
  protected boolean ehlo(String domain) throws MessagingException{
    String cmd;
    if(domain != null) cmd="EHLO " + domain;
    else cmd="EHLO";
    sendCommand(cmd);
    int resp=readServerResponse();
    if(resp == 250){
      ...
      try{
        ...
        while((line=rd.readLine()) != null){
          ...
          if(line.length() < 5) continue; // shouldn't happen
          ...
          if(debug) out.println("DEBUG SMTP: Found extension \"" +
              line + "\", arg \"" + arg + "\"");
  ...
  protected void mailFrom() throws MessagingException{
    String from=null;
    if(message instanceof SMTPMessage) from=((SMTPMessage)message).getEnvelopeFrom();
    if(from == null || from.length() <= 0) from=session.getProperty("mail." + name + ".from");
    if(from == null || from.length() <= 0){
      Address[] fa;
      Address me;
      if(message != null && (fa=message.getFrom()) != null &&
          fa.length > 0) me=fa[0];
      else me=InternetAddress.getLocalAddress(session);

      if(me != null) from=((InternetAddress)me).getAddress();
    ...
    if(supportsExtension("DSN")){
      String ret=null;
      if(message instanceof SMTPMessage) ret=((SMTPMessage)message).getDSNRet();
      if(ret == null) ret=session.getProperty("mail." + name + ".dsn.ret");
      // XXX - check for legal syntax?
      if(ret != null) cmd+=" RET=" + ret;
    }

    /*
     * If an RFC 2554 submitter has been specified, and the server supports the
     * AUTH extension, include the AUTH= element on the MAIL FROM command.
     */
    if(supportsExtension("AUTH")){
      String submitter=null;
      if(message instanceof SMTPMessage) submitter=((SMTPMessage)message).getSubmitter();
      if(submitter == null) submitter=session.getProperty("mail." + name + ".submitter");
      // XXX - check for legal syntax?
      if(submitter != null){
        try{
          ...
          if(debug) out.println("DEBUG SMTP: ignoring invalid submitter: " +
     ...
     */
    String ext=null;
    if(message instanceof SMTPMessage) ext=((SMTPMessage)message).getMailExtension();
    if(ext == null) ext=session.getProperty("mail." + name + ".mailextension");
    if(ext != null && ext.length() > 0) cmd+=" " + ext;
   ...
   * 503, 421 and how we map the above error/failure conditions to valid/invalid
   * address vectors that are reported in the thrown exception: invalid addr:
   * 550, 501, 503, 551, 553 valid addr: 552 (quota), 450, 451, 452 (quota), 421
   * (srvr abort)
   */
  protected void rcptTo() throws MessagingException{
    ...
    if(message instanceof SMTPMessage) sendPartial=((SMTPMessage)message).getSendPartial();
    if(!sendPartial){
      String sp=session.getProperty("mail." + name + ".sendpartial");
      sendPartial=sp != null && sp.equalsIgnoreCase("true");
    }
    if(debug && sendPartial) out.println("DEBUG SMTP: sendPartial set");

    boolean dsn=false;
    String notify=null;
    if(supportsExtension("DSN")){
      if(message instanceof SMTPMessage) notify=((SMTPMessage)message).getDSNNotify();
      if(notify == null) notify=session.getProperty("mail." + name + ".dsn.notify");
      // XXX - check for legal syntax?
      if(notify != null) dsn=true;
    }
      ...
      switch(retCode){
        ...
        case 251:
          ...
          sfex=new SMTPAddressSucceededException(ia, cmd, retCode,
              lastServerResponse);
          if(mex == null) mex=sfex;
          else mex.setNextException(sfex);
          break;
        ...
        case 501:
          ...
          if(!sendPartial) sendFailed=true;
        ...
        default:
          ...
          if(retCode >= 400 && retCode <= 499){
            ...
            if(debug) out.println("DEBUG SMTP: got response code " + retCode +
    ...
    if(sendPartial && valid.size() == 0) sendFailed=true;
    ...
    if(sendFailed){
      if(debug) out.println("DEBUG SMTP: Sending failed " +
          "because of invalid destination addresses");
      notifyTransportListeners(TransportEvent.MESSAGE_NOT_DELIVERED,
          validSentAddr, validUnsentAddr,
          invalidAddr, this.message);
      ...
      try{
        if(serverSocket != null) issueCommand("RSET", 250);
      }
        ...
        try{
          ...
          if(debug) ex2.printStackTrace(out);
        }
      }
      finally{
  ...
  private void openServer(String server, int port)
    ...
    if(debug) out.println("DEBUG SMTP: trying to connect to host \"" + server +
    ...
    try{
      ...
      if((r=readServerResponse()) != 220){
        ...
        serverInput=null;
        lineInputStream=null;
        if(debug) out.println("DEBUG SMTP: could not connect to host \"" +
            server + "\", port: " + port +
            ", response: " + r + "\n");
        ...
        if(debug) out.println("DEBUG SMTP: connected to host \"" +
            server + "\", port: " + port + "\n");
      }
    }
    catch(UnknownHostException uhex){
  ...
  private void openServer() throws MessagingException{
    ...
    try{
      ...
      if(debug) out.println("DEBUG SMTP: starting protocol to host \"" +
          server + "\", port " + port);
      ...
      if((r=readServerResponse()) != 220){
        ...
        serverInput=null;
        lineInputStream=null;
        if(debug) out.println("DEBUG SMTP: got bad greeting from host \"" +
            server + "\", port: " + port +
            ", response: " + r + "\n");
        throw new MessagingException(
        ...
        if(debug) out.println("DEBUG SMTP: protocol started to host \"" +
            server + "\", port: " + port + "\n");
      }
    }
    catch(IOException ioe){
  ...
  public synchronized void issueCommand(String cmd, int expect)
    ...
    if(readServerResponse() != expect) throw new MessagingException(lastServerResponse);
  }
  ...
  private void issueSendCommand(String cmd, int expect)
    ...
    if((ret=readServerResponse()) != expect){
      ...
      if(vsl > 0) System.arraycopy(validSentAddr, 0, valid, 0, vsl);
      if(vul > 0) System.arraycopy(validUnsentAddr, 0, valid, vsl, vul);
      validSentAddr=null;
      validUnsentAddr=valid;
      if(debug) out.println("DEBUG SMTP: got response code " + ret +
          ", with response: " + lastServerResponse);
      String _lsr=lastServerResponse; // else rset will nuke it
      int _lrc=lastReturnCode;
      if(serverSocket != null) // hasn't already been closed
  ...
  protected int readServerResponse() throws MessagingException{
    ...
    try{
      ...
      do{
        ...
        if(line == null){
          ...
          if(serverResponse.length() == 0) serverResponse="[EOF]";
          lastServerResponse=serverResponse;
          lastReturnCode=-1;
          if(debug) out.println("DEBUG SMTP: EOF: " + serverResponse);
          return -1;
        }
        buf.append(line);
        buf.append("\n");
      ...
      if(debug) out.println("DEBUG SMTP: exception reading response: " + ioex);
    ...
    if(returnCode == -1 && debug) out.println("DEBUG SMTP: bad server response: " + serverResponse);
  ...
  private String normalizeAddress(String addr){
    if((!addr.startsWith("<")) && (!addr.endsWith(">"))) return "<" + addr + ">";
    else return addr;
  }
  ...
  protected boolean supportsAuthentication(String auth){
    assert Thread.holdsLock(this);
    if(extMap == null) return false;
    String a=(String)extMap.get("AUTH");
    if(a == null) return false;
    StringTokenizer st=new StringTokenizer(a);
    while(st.hasMoreTokens()){
      String tok=st.nextToken();
      if(tok.equalsIgnoreCase(auth)) return true;
    }
    return false;
  }
