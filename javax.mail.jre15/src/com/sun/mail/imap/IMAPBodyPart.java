...
public class IMAPBodyPart extends MimeBodyPart{
  ...
  /*
   * Override this method to make it a no-op, rather than throw an
   * IllegalWriteException. This will permit IMAPBodyParts to be inserted in
   * newly crafted MimeMessages, especially when forwarding or replying to
   * messages.
   */
  ...
  protected InputStream getContentStream() throws MessagingException{
    ...
    synchronized(message.getMessageCacheLock()){
      try{
        ...
        if(p.isREV1() && (message.getFetchBlockSize() != -1)) return new IMAPInputStream(message, sectionId, bs.size, pk);

        // Else, vanila IMAP4, no partial fetch

        int seqnum=message.getSequenceNumber();
        BODY b;
        if(pk) b=p.peekBody(seqnum, sectionId);
        else b=p.fetchBody(seqnum, sectionId);
  ...
  public synchronized DataHandler getDataHandler()
    ...
    if(dh == null){
      ...
      else if(bs.isNested() && message.isREV1()) dh=new DataHandler(
    ...
    synchronized(message.getMessageCacheLock()){
      try{
        ...
        if(p.isREV1()){
          ...
          if(b == null) throw new MessagingException("Failed to fetch headers");

          ByteArrayInputStream bis=b.getByteArrayInputStream();
          if(bis == null) throw new MessagingException("Failed to fetch headers");
          ...
          // Content-Description
          if(bs.description != null) headers.addHeader("Content-Description",
              bs.description);
          // Content-ID
          if(bs.id != null) headers.addHeader("Content-ID", bs.id);
          // Content-MD5
          if(bs.md5 != null) headers.addHeader("Content-MD5", bs.md5);
        }
      }
      catch(ConnectionException cex){
        throw new FolderClosedException(
