 ...
 * <code>Store</code> object to an <code>IMAPStore</code> object and use the
 ...
 * implements the protocol-level commands. <p> A connected IMAPStore maintains a
 * pool of IMAP protocol objects for use in communicating with the IMAP server.
 * The IMAPStore will create the initial AUTHENTICATED connection and seed the
 * pool with this connection. As folders are opened and new IMAP protocol
 * objects are needed, the IMAPStore will provide them from the connection pool,
 * or create them if none are available. When a folder is closed, its IMAP
 * protocol object is returned to the connection pool if the pool is not over
 * capacity. The pool size can be configured by setting the
 * mail.imap.connectionpoolsize property. <p> A mechanism is provided for timing
 * out idle connection pool IMAP protocol objects. Timed out connections are
 * closed and removed (pruned) from the connection pool. The time out interval
 * can be configured via the mail.imap.connectionpooltimeout property. <p> The
 * connected IMAPStore object may or may not maintain a separate IMAP protocol
 * object that provides the store a dedicated connection to the IMAP server.
 * This is provided mainly for compatibility with previous implementations of
 * JavaMail and is determined by the value of the
 * mail.imap.separatestoreconnection property. <p> An IMAPStore object provides
 * closed IMAPFolder objects thru its list() and listSubscribed() methods. A
 * closed IMAPFolder object acquires an IMAP protocol object from the store to
 * communicate with the server. When the folder is opened, it gets its own
 * protocol object and thus its own, separate connection to the server. The
 * store maintains references to all 'open' folders. When a folder is/gets
 * closed, the store removes it from its list. When the store is/gets closed, it
 * closes all open folders in its list, thus cleaning up all open connections to
 * the server. <p> A mutex is used to control access to the connection pool
 * resources. Any time any of these resources need to be accessed, the following
 * convention should be followed: synchronized (pool) { // ACQUIRE LOCK //
 * access connection pool resources } // RELEASE LOCK <p> The locking
 * relationship between the store and folders is that the store lock must be
 * acquired before a folder lock. This is currently only applicable in the
 * store's cleanup method. It's important that the connection pool lock is not
 * held when calling into folder objects. The locking hierarchy is that a folder
 * lock must be acquired before any connection pool operations are performed.
 * <p> The IMAPStore implements the ResponseHandler interface and listens to BYE
 * or untagged OK-notification events from the server. <p>
 */
...
public class IMAPStore extends Store
  ...
  /**
   * A special event type for a StoreEvent to indicate an IMAP response, if the
   * mail.imap.enableimapevents property is set.
   */
  ...
  /*
   * Track our connected state. Set on successful return from protocolConnect
   * and reset in cleanup. Field is volatile so that it can be tested in
   * handleResponse without holding any locks.
   */
  ...
  static class ConnectionPool{
     ...
     * IMAP command we hold the store's lock. While executing the IDLE command
     * we can't hold the lock or it would prevent other threads from entering
     * Store methods even far enough to check whether an IDLE command is in
     * progress. We need to check before issuing another command so that we can
     * abort the IDLE command. The idleState field is protected by the store's
     * lock. The RUNNING state is the normal state and means no IDLE command is
     * in progress. The IDLE state means we've issued an IDLE command and are
     * reading responses. The ABORTING state means we've sent the DONE
     * continuation command and are waiting for the thread running the IDLE
     * command to break out of its read loop. When an IDLE command is in
     * progress, the thread calling the idle method will be reading from the
     * IMAP connection while not holding the sotre's lock. It's obviously
     * critical that no other thread try to send a command or read from the
     * connection while in this state. However, other threads can send the DONE
     * continuation command that will cause the server to break out of the IDLE
     * loop and send the ending tag response to the IDLE command. The thread in
     * the idle method that's reading the responses from the IDLE command will
     * see this ending response and complete the idle method, setting the
     * idleState field back to RUNNING, and notifying any threads waiting to use
     * the connection. All uses of the IMAP connection (IMAPProtocol object)
     * must be preceeded by a check to make sure an IDLE command is not running,
     * and abort the IDLE command if necessary. This check is made while holding
     * the connection pool lock. While waiting for the IDLE command to complete,
     * these other threads will give up the connection pool lock. This check is
     * done by the getStoreProtocol() method.
     */
  ...
  protected IMAPStore(Session session, URLName url,
    ...
    if(url != null) name=url.getProtocol();
    this.name=name;
    ...
    if(s != null && s.equalsIgnoreCase("true")) pool.debug=true;
    ...
    if(s != null && s.equalsIgnoreCase("false")){
      ...
      if(debug) out.println("DEBUG: mail.imap.partialfetch: false");
    }
    ...
    if(s != null){
      try{
        ...
        if(size > 0) pool.poolSize=size;
      }
      catch(NumberFormatException nfe){
      }
      if(pool.debug) out.println("DEBUG: mail.imap.connectionpoolsize: " +
          pool.poolSize);
    }
    ...
    if(s != null){
      try{
        ...
        if(connectionPoolTimeout > 0) pool.clientTimeoutInterval=connectionPoolTimeout;
      }
      catch(NumberFormatException nfe){
      }
      if(pool.debug) out.println("DEBUG: mail.imap.connectionpooltimeout: " +
    ...
    if(s != null){
      try{
        ...
        if(serverTimeout > 0) pool.serverTimeoutInterval=serverTimeout;
      }
      catch(NumberFormatException nfe){
      }
      if(pool.debug) out.println("DEBUG: mail.imap.servertimeout: " +
    ...
    if(s != null && s.equalsIgnoreCase("true")){
      if(pool.debug) out.println("DEBUG: dedicate a store connection");
    ...
    if(s != null){
      proxyAuthUser=s;
      if(debug) out.println("DEBUG: mail.imap.proxyauth.user: " +
          proxyAuthUser);
    }
    ...
    if(s != null && s.equalsIgnoreCase("true")){
      if(debug) out.println("DEBUG: disable AUTH=LOGIN");
    ...
    if(s != null && s.equalsIgnoreCase("true")){
      if(debug) out.println("DEBUG: disable AUTH=PLAIN");
    ...
    if(s != null && s.equalsIgnoreCase("true")){
      if(debug) out.println("DEBUG: enable STARTTLS");
      enableStartTLS=true;
    }
    ...
    if(s != null && s.equalsIgnoreCase("true")){
      if(debug) out.println("DEBUG: enable SASL");
      enableSASL=true;
    }
    ...
    if(enableSASL){
      ...
      if(s != null && s.length() > 0){
        if(debug) out.println("DEBUG: SASL mechanisms allowed: " + s);
        Vector v=new Vector(5);
        StringTokenizer st=new StringTokenizer(s, " ,");
        while(st.hasMoreTokens()){
          String m=st.nextToken();
          if(m.length() > 0) v.addElement(m);
        }
    ...
    if(s != null){
      authorizationID=s;
      if(debug) out.println("DEBUG: mail.imap.sasl.authorizationid: " +
          authorizationID);
    }
    ...
    if(s != null){
      saslRealm=s;
      if(debug) out.println("DEBUG: mail.imap.sasl.realm: " + saslRealm);
    }

    // check if forcePasswordRefresh is enabled
    s=session.getProperty("mail." + name + ".forcepasswordrefresh");
    if(s != null && s.equalsIgnoreCase("true")){
      if(debug) out.println("DEBUG: enable forcePasswordRefresh");
    ...
    if(s != null && s.equalsIgnoreCase("true")){
      if(debug) out.println("DEBUG: enable IMAP events");
      enableImapEvents=true;
    }
  }

  /**
   * Implementation of protocolConnect(). Will create a connection to the server
   * and authenticate the user using the mechanisms specified by various
   * properties.
   * <p>
   * The <code>host</code>, <code>user</code>, and <code>password</code>
   * parameters must all be non-null. If the authentication mechanism being used
   * does not require a password, an empty string or other suitable dummy
  ...
  protected synchronized boolean protocolConnect(String host, int pport, String user, String password)
    ...
    if(host == null || password == null || user == null){
      if(debug) out.println("DEBUG: protocolConnect returning false" +
          ", host=" + host +
          ", user=" + user +
    ...
    try{
      ...
      if(poolEmpty){
        ...
        if(debug) out.println("DEBUG: protocolConnect login" +
            ", host=" + host +
            ", user=" + user +
            ", password=<non-null>");
  ...
  private void login(IMAPProtocol p, String u, String pw)
    ...
    if(p.isAuthenticated()) return; // no need to login

    /*
     * Put a special "marker" in the capabilities list so we can detect if the
     * server refreshed the capabilities in the OK response.
     */
    p.getCapabilities().put("__PRELOGIN__", "");
    String authzid;
    if(authorizationID != null) authzid=authorizationID;
    else if(proxyAuthUser != null) authzid=proxyAuthUser;
    else authzid=u;

    if(enableSASL) p.sasllogin(saslMechanisms, saslRealm, authzid, u, pw);

    if(p.isAuthenticated())
    ; // SASL login succeeded, go to bottom
    else if(p.hasCapability("AUTH=PLAIN") && !disableAuthPlain) p.authplain(authzid, u, pw);
    else if((p.hasCapability("AUTH-LOGIN") ||
        p.hasCapability("AUTH=LOGIN")) && !disableAuthLogin) p.authlogin(u, pw);
    else if(!p.hasCapability("LOGINDISABLED")) p.login(u, pw);
    else throw new ProtocolException("No login methods supported!");

    if(proxyAuthUser != null) p.proxyauth(proxyAuthUser);

    /*
     * If marker is still there, capabilities haven't been refreshed, refresh
     * them now.
     */
  ...
  /**
   * Set the user name that will be used for subsequent connections after this
   * Store is first connected (for example, when creating a connection to open a
   * Folder). This value is overridden by any call to the Store's connect
   * method.
   * <p>
   * Some IMAP servers may provide an authentication ID that can be used for
   * more efficient authentication for future connections. This authentication
  ...
  /**
   * Set the password that will be used for subsequent connections after this
   * Store is first connected (for example, when creating a connection to open a
   * Folder). This value is overridden by any call to the Store's connect
   * method.
   * <p>
  ...
  /*
   * Get a new authenticated protocol object for this Folder. Also store a
   * reference to this folder in our list of open folders.
   */
    ...
    while(p == null){
      ...
      synchronized(pool){
        ...
        if(pool.authenticatedConnections.isEmpty() ||
          ...
          if(debug) out.println("DEBUG: no connections in the pool, " +
              "creating a new one");
          try{
            /*
             * Some authentication systems use one time passwords or tokens, so
             * each authentication request requires a new password. This
             * "kludge" allows a callback to application code to get a new
             * password. XXX - remove this when SASL support is added
             */
          ...
          if(p == null) throw new MessagingException("connection failure");
        }
        else{
          if(debug) out.println("DEBUG: connection available -- size: " +
        ...
        if(folder != null){
          if(pool.folders == null) pool.folders=new Vector();
  ...
  /**
   * Get this Store's protocol connection. When acquiring a store protocol
   * object, it is important to use the following steps: IMAPProtocol p = null;
   * try { p = getStoreProtocol(); // perform the command } catch
   * (ConnectionException cex) { throw new StoreClosedException(this,
   * cex.getMessage()); } catch (WhateverException ex) { // handle it } finally
   * { releaseStoreProtocol(p); if (p == null) { // failed to get a Store
   * connection // have to force Store to be closed cleanup(); } }
   */
  IMAPProtocol getStoreProtocol() throws ProtocolException{
    ...
    while(p == null){
      synchronized(pool){
        ...
        if(pool.authenticatedConnections.isEmpty()){
          if(pool.debug) out.println("DEBUG: getStoreProtocol() - no connections " +
          ...
          if(pool.debug) out.println("DEBUG: getStoreProtocol() - " +
  ...
  /**
   * If a SELECT succeeds, but indicates that the folder is READ-ONLY, and the
   * user asked to open the folder READ_WRITE, do we allow the open to succeed?
   */
  ...
  boolean isConnectionPoolFull(){
    ...
    synchronized(pool){
      if(pool.debug) out.println("DEBUG: current size: " +
          pool.authenticatedConnections.size() +
    ...
    synchronized(pool){
      if(protocol != null){
        ...
        if(!isConnectionPoolFull()){
          ...
          if(debug) out.println("DEBUG: added an " +
      ...
      if(pool.folders != null) pool.folders.removeElement(folder);
  ...
  void releaseStoreProtocol(IMAPProtocol protocol){
    ...
    if(protocol == null) return; // nothing to release
    synchronized(pool){
      pool.storeConnectionInUse=false;
      pool.notifyAll(); // in case anyone waiting

      if(pool.debug) out.println("DEBUG: releaseStoreProtocol()");
  ...
  private void emptyConnectionPool(boolean force){
    ...
    if(pool.debug) out.println("DEBUG: removed all authenticated connections");
  }
  ...
  /**
   * Check whether this store is connected. Override superclass method, to
  ...
  public synchronized boolean isConnected(){
    ...
    /*
     * The below noop() request can: (1) succeed - in which case all is fine.
     * (2) fail because the server returns NO or BAD, in which case we ignore it
     * since we can't really do anything. (2) fail because a BYE response is
     * obtained from the server (3) fail because the socket.write() to the
     * server fails, in which case the iap.protocol() code converts the
     * IOException into a BYE response. Thus, our BYE handler will take care of
     * closing the Store in case our connection is really gone.
     */
  ...
  public synchronized void close() throws MessagingException{
    ...
    try{
      ...
      /*
       * Have to drop the lock before calling cleanup. Yes, there's a potential
       * race here. The pool could become empty after we check, in which case
       * we'll just waste time getting a new connection and closing it. Or, the
       * pool could be empty now and not empty by the time we get into cleanup,
       * but that's ok because cleanup will just close the connection.
       */
      if(isEmpty){
        if(pool.debug) out.println("DEBUG: close() - no connections ");
        cleanup();
        return;
      }
      ...
      /*
       * We have to remove the protocol from the pool so that, when our response
       * handler processes the BYE response and calls cleanup, which calls
       * emptyConnection, that we don't try to log out this connection twice.
       */
      synchronized(pool){
      ...
      /*
       * LOGOUT. Note that protocol.logout() closes the server socket
       * connection, regardless of what happens .. Also note that
       * protocol.logout() results in a BYE response (As per rfc 2060, BYE is a
       * *required* response to LOGOUT). In fact, even if protocol.logout()
       * fails with an IOException (if the server connection is dead),
       * iap.Protocol.command() converts that exception into a BYE response. So,
       * I depend on my BYE handler to do the Store cleanup.
       */
      protocol.logout();
    }
    catch(ProtocolException pex){
  ...
  /**
   * Cleanup before dying. If force is true, we force the folders to close
   * abruptly without waiting for the server. Used when the store connection
   * times out. Not synchronized so that it can be safely called from
   * handleResponse.
   */
  private void cleanup(boolean force){
    if(debug) out.println("DEBUG: IMAPStore cleanup, force " + force);
    ...
    for(;;){
      ...
      for(int i=0, fsize=foldersCopy.size(); i < fsize; i++){
        ...
        try{
          if(force){
            if(debug) out.println("DEBUG: force folder to close");
            ...
            if(debug) out.println("DEBUG: close folder");
            f.close(false);
          }
        }
    ...
    if(debug) out.println("DEBUG: IMAPStore cleanup done");
  }

  /**
   * Get the default folder, representing the root of this user's namespace.
   * Returns a closed DefaultFolder object.
   */
  ...
  /**
   * Using the IMAP NAMESPACE command (RFC 2342), return a set of folders
   * representing the Personal namespaces.
   */
  public Folder[] getPersonalNamespaces() throws MessagingException{
    Namespaces ns=getNamespaces();
    if(ns == null || ns.personal == null) return super.getPersonalNamespaces();
    return namespaceToFolders(ns.personal, null);
  }

  /**
   * Using the IMAP NAMESPACE command (RFC 2342), return a set of folders
   * representing the User's namespaces.
   */
  public Folder[] getUserNamespaces(String user)
      throws MessagingException{
    Namespaces ns=getNamespaces();
    if(ns == null || ns.otherUsers == null) return super.getUserNamespaces(user);
    return namespaceToFolders(ns.otherUsers, user);
  }

  /**
   * Using the IMAP NAMESPACE command (RFC 2342), return a set of folders
   * representing the Shared namespaces.
   */
  public Folder[] getSharedNamespaces() throws MessagingException{
    Namespaces ns=getNamespaces();
    if(ns == null || ns.shared == null) return super.getSharedNamespaces();
  ...
  private Folder[] namespaceToFolders(Namespaces.Namespace[] ns,
    ...
    for(int i=0; i < fa.length; i++){
      ...
      if(user == null){
        ...
        int len=name.length();
        if(len > 0 && name.charAt(len - 1) == ns[i].delimiter) name=name.substring(0, len - 1);
      }
      else{
        // add user
        name+=user;
  ...
  /**
   * Get the quotas for the named quota root. Quotas are controlled on the basis
   * of a quota root, not (necessarily) a folder. The relationship between
   * folders and quota roots depends on the IMAP server. Some servers might
   * implement a single quota root for all folders owned by a user. Other
   * servers might implement a separate quota root for each folder. A single
   * folder can even have multiple quota roots, perhaps controlling quotas for
   * different resources.
   * 
   * @param root
  ...
  public void handleResponse(Response r){
    ...
    if(r.isOK() || r.isNO() || r.isBAD() || r.isBYE()) handleResponseCode(r);
    if(r.isBYE()){
      if(debug) out.println("DEBUG: IMAPStore connection dead");
   ...
   * notifications without the need for the client to constantly poll the
   * server. Use a <code>ConnectionListener</code> to be notified of events.
   * When another thread (e.g., the listener thread) needs to issue an IMAP
   * comand for this Store, the idle mode will be terminated and this method
   * will return. Typically the caller will invoke this method in a loop.
   * <p>
   * If the mail.imap.enableimapevents property is set, notifications received
   * while the IDLE command is active will be delivered to
   * <code>ConnectionListener</code>s as events with a type of
   * <code>IMAPStore.RESPONSE</code>. The event's message will be the raw IMAP
   * response string. Note that most IMAP servers will not deliver any events
   * when using the IDLE command on a connection with no mailbox selected (i.e.,
   * this method). In most cases you'll want to use the <code>idle</code> method
   * on <code>IMAPFolder</code>.
   * <p>
  ...
  public void idle() throws MessagingException{
    ...
    try{
      ...
      /*
       * We gave up the pool lock so that other threads can get into the pool
       ...
       * notifications from the server. We don't hold the pool lock while
       * reading because it protects the idleState and other threads need to be
       * able to examine the state. We hold the pool lock while processing the
       * responses.
       */
      for(;;){
      ...
      /*
       * Enforce a minimum delay to give time to threads processing the
       * responses that came in while we were idle.
       */
   ...
   * completes. ASSERT: Must be called with the pool's lock held.
   */
  ...
  /**
   * Handle notifications and alerts. Response must be an OK, NO, BAD, or BYE
   * response.
   */
  void handleResponseCode(Response r){
    ...
    if(s.startsWith("[")){ // a response code
      ...
      if(i > 0 && s.substring(0, i + 1).equalsIgnoreCase("[ALERT]")) isAlert=true;
      // strip off the response code in any event
      s=s.substring(i + 1).trim();
    }
    if(isAlert) notifyStoreListeners(StoreEvent.ALERT, s);
