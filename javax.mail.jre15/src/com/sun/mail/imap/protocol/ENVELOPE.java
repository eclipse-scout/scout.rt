  ...
  private InternetAddress[] parseAddressList(Response r)
    ...
    if(b == '('){
      ...
      do{
        ...
        if(!a.isEndOfGroup()) v.addElement(a);
      }
      while(r.peekByte() != ')');
    ...
    else throw new ParsingException("ADDRESS parse error");
  }
}
...
class IMAPAddress extends InternetAddress{
  ...
  IMAPAddress(Response r) throws ParsingException {
    ...
    if(r.readByte() != '(') throw new ParsingException("ADDRESS parse error");
    ...
    if(host == null){
      ...
      if(mb == null || mb.length() == 0) address=host;
      else if(host.length() == 0) address=mb;
      else address=mb + "@" + host;
    }
