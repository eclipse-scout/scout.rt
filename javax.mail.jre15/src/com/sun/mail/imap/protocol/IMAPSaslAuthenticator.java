  ...
  public boolean authenticate(String[] mechs, String realm, String authzid,
    ...
    synchronized(pr){ // authenticate method should be synchronized
      ...
      try{
        ...
        if(debug) out.println("IMAP SASL DEBUG: Failed to create SASL client: " +
            sex);
        return false;
      }
      if(sc == null){
      ...
      if(debug) out.println("IMAP SASL DEBUG: SASL client " +
          sc.getMechanismName());
       ...
       * encoded username and password strings Note that the encoded bytes
       * should be sent "as-is" to the server, *not* as literals or
       * quoted-strings. Also note that unlike the B64 definition in MIME, CRLFs
       * should *not* be inserted during the encoding process. So, I use
       * Integer.MAX_VALUE (0x7fffffff (> 1G)) as the bytesPerLine, which should
        ...
        try{
          ...
          if(r.isContinuation()){
            ...
            if(!sc.isComplete()){
              ...
              if(debug) out.println("IMAP SASL DEBUG: challenge: " +
            ...
            if(ba == null){
              ...
              if(debug) out.println("IMAP SASL DEBUG: response: " +
                  ASCIIUtility.toString(ba, 0, ba.length) + " :");
      ...
      if(sc.isComplete() /* && res.status == SUCCESS */){
        ...
        if(qop != null && (qop.equalsIgnoreCase("auth-int") ||
          ...
          // XXX - NOT SUPPORTED!!!
          if(debug) out.println("IMAP SASL DEBUG: " +
       ...
       * the connection has been authenticated. So, for now, the below code
       * really ends up being just a no-op.
       */
