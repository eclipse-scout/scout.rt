...
/**
 * This class extends the iap.Protocol object and implements IMAP semantics. In
 * general, there is a method corresponding to each IMAP protocol command. The
 * typical implementation issues the appropriate protocol command, collects all
 * responses, processes those responses that are specific to this command and
 * then dispatches the rest (the unsolicited ones) to the dispatcher using the
 * <code>notifyResponseHandlers(r)</code>.
 * 
...
public class IMAPProtocol extends Protocol{
  ...
  public IMAPProtocol(String name, String host, int port,
    ...
    try{
      ...
      if(capabilities == null) capability();

      if(hasCapability("IMAP4rev1")) rev1=true;
      ...
      /*
       * If we get here because an exception was thrown, we need to disconnect
       * to avoid leaving a connected socket that no one will be able to use
       * because this object was never completely constructed.
       */
  ...
  public void capability() throws ProtocolException{
    ...
    for(int i=0, len=r.length; i < len; i++){
      ...
      if(ir.keyEquals("CAPABILITY")) parseCapabilities(ir);
    }
  }

  /**
   * If the response contains a CAPABILITY response code, extract it and save
   * the capabilities.
   */
  protected void setCapabilities(Response r){
    ...
    String s;
    s=r.readAtom();
    if(!s.equalsIgnoreCase("CAPABILITY")) return;
    capabilities=new HashMap(10);
    authmechs=new ArrayList(5);
    parseCapabilities(r);
  }
  ...
  /**
   * Parse the capabilities from a CAPABILITY response or from a CAPABILITY
   * response code attached to (e.g.) an OK response.
   */
  protected void parseCapabilities(Response r){
    String s;
    while((s=r.readAtom(']')) != null){
      if(s.length() == 0){
        if(r.peekByte() == (byte)']') break;
        /*
         * Probably found something here that's not an atom. Rather than loop
         * forever or fail completely, we'll try to skip this bogus capability.
         * This is known to happen with: Netscape Messaging Server 4.03 (built
         * Apr 27 1999) that returns: * CAPABILITY * CAPABILITY IMAP4 IMAP4rev1
         * ... The "*" in the middle of the capability list causes us to loop
         * forever here.
         */
        r.skipToken();
      }
        ...
        if(s.regionMatches(true, 0, "AUTH=", 0, 5)){
          ...
          if(debug) out.println("IMAP DEBUG: AUTH: " + s.substring(5));
        }
      }
    }
  }
  ...
  protected void processGreeting(Response r) throws ProtocolException{
    ...
    else throw new ConnectionException(this, r);
  }

  /**
   * Returns <code>true</code> if the connection has been authenticated, either
   * due to a successful login, or due to a PREAUTH greeting response.
   */
  ...
  /**
   * Check whether the given capability is supported by this server. Returns
   * <code>true</code> if so, otherwise returns false.
   */
  ...
  /**
   * Close socket connection. This method just makes the Protocol.disconnect()
   * method public.
   */
  ...
  public void noop() throws ProtocolException{
    if(debug) out.println("IMAP DEBUG: IMAPProtocol noop");
  ...
  public synchronized void authlogin(String u, String p)
     ...
     * username and password strings Note that the encoded bytes should be sent
     * "as-is" to the server, *not* as literals or quoted-strings. Also note
     * that unlike the B64 definition in MIME, CRLFs should *not* be inserted
     * during the encoding process. So, I use Integer.MAX_VALUE (0x7fffffff (>
     * 1G)) as the bytesPerLine, which should be sufficiently large ! Finally,
     * format the line in a buffer so it can be sent as a single packet, to
     * avoid triggering a bug in SUN's SIMS 2.0 server caused by patch 105346.
     */
    ...
    /*
     * Dispatch untagged responses. NOTE: in our current upper level IMAP
     * classes, we add the responseHandler to the Protocol object only *after*
     * the connection has been authenticated. So, for now, the below code really
     * ends up being just a no-op.
     */
  ...
  /**
   * The AUTHENTICATE command with AUTH=PLAIN authentication scheme. This is
   * based heavly on the {@link #authlogin} method.
   * 
   * @param authzid
   ...
   *          the password
   * @throws ProtocolException
   *           as thrown by {@link Protocol#handleResult}.
  ...
  public void sasllogin(String[] allowed, String realm, String authzid,
    ...
    if(saslAuthenticator == null){
      try{
        ...
        if(debug) out.println("IMAP DEBUG: Can't load SASL authenticator: " +
            ex);
    ...
    if(saslAuthenticator.authenticate(mechs, realm, authzid, u, p)) authenticated=true;
  }
  ...
  public MailboxInfo select(String mbox) throws ProtocolException{
    ...
    if(response.isOK()){ // command succesful
      if(response.toString().indexOf("READ-ONLY") != -1) minfo.mode=Folder.READ_ONLY;
      else minfo.mode=Folder.READ_WRITE;
    }
  ...
  public Status status(String mbox, String[] items)
    ...
    if(items == null) items=Status.standardItems;
    ...
    if(response.isOK()){ // command succesful
      for(int i=0, len=r.length; i < len; i++){
        ...
        if(ir.keyEquals("STATUS")){
          if(status == null) status=new Status(ir);
          else // collect 'em all
  ...
  public AppendUID appenduid(String mbox, Flags f, Date d,
    ...
    if(f != null){ // set Flags in appended message
      ...
      /*
       * HACK ALERT: We want the flag_list to be written out without any
       * checking/processing of the bytes in it. If I use writeString(), the
       * flag_list will end up being quoted since it contains "illegal"
       * characters. So I am depending on implementation knowledge that
       * writeAtom() does not do any checking/processing - it just writes out
       * the bytes. What we really need is a writeFoo() that just dumps out its
       * argument.
       */
    ...
    if(uid) return getAppendUID(r[r.length - 1]);
    else return null;
  }

  /**
   * If the response contains an APPENDUID response code, extract it and return
   * an AppendUID object with the information.
   */
  private AppendUID getAppendUID(Response r){
    ...
    String s;
    s=r.readAtom();
    if(!s.equalsIgnoreCase("APPENDUID")) return null;
  ...
  public void uidexpunge(UIDSet[] set) throws ProtocolException{
    if(!hasCapability("UIDPLUS")) throw new BadCommandException("UID EXPUNGE not supported");
  ...
  public BODYSTRUCTURE fetchBodyStructure(int msgno)
    ...
    Response response=r[r.length - 1];
    if(response.isOK()) return (BODYSTRUCTURE)FetchResponse.getItem(r, msgno,
        BODYSTRUCTURE.class);
    else if(response.isNO()) return null;
    else{
      handleResult(response);
      return null;
    }
  ...
  protected BODY fetchBody(int msgno, String section, boolean peek)
    ...
    Response response=r[r.length - 1];
    if(response.isOK()) return (BODY)FetchResponse.getItem(r, msgno, BODY.class);
    else if(response.isNO()) return null;
    else{
      handleResult(response);
      return null;
  ...
  /**
   * Return a buffer to read a response into. The buffer is provided by
   * fetchBody and is used only once.
   */
  ...
  /**
   * Fetch the specified RFC822 Data item. 'what' names the item to be fetched.
  ...
  public RFC822DATA fetchRFC822(int msgno, String what)
    ...
    Response response=r[r.length - 1];
    if(response.isOK()) return (RFC822DATA)FetchResponse.getItem(r, msgno,
        RFC822DATA.class);
    else if(response.isNO()) return null;
    else{
      handleResult(response);
  ...
  public UID fetchUID(int msgno) throws ProtocolException{
    ...
    Response response=r[r.length - 1];
    if(response.isOK()) return (UID)FetchResponse.getItem(r, msgno, UID.class);
  ...
  /**
   * Get the sequence number for the given UID. A UID object containing the
   * sequence number is returned. If the given UID is invalid, <code>null</code>
   * is returned.
   */
  ...
  /**
   * Get the sequence numbers for UIDs ranging from start till end. UID objects
   * that contain the sequence numbers are returned. If no UIDs in the given
   * range are found, an empty array is returned.
   */
  public UID[] fetchSequenceNumbers(long start, long end)
    ...
    for(int i=0, len=r.length; i < len; i++){
      ...
      FetchResponse fr=(FetchResponse)r[i];
      if((u=(UID)fr.getItem(UID.class)) != null) v.addElement(u);
    }
  ...
  private Response[] fetch(String msgSequence, String what, boolean uid)
      throws ProtocolException{
    if(uid) return command("UID FETCH " + msgSequence + " (" + what + ")", null);
    else return command("FETCH " + msgSequence + " (" + what + ")", null);
  }
  ...
  private void storeFlags(String msgset, Flags flags, boolean set)
      throws ProtocolException{
    Response[] r;
    if(set) r=command("STORE " + msgset + " +FLAGS " +
        createFlagList(flags), null);
    else r=command("STORE " + msgset + " -FLAGS " +
        createFlagList(flags), null);
  ...
  /**
   * Issue the given search criterion on the specified message sets. Returns
   * array of matching sequence numbers. An empty array is returned if no
   * matches are found.
   * 
   * @param msgsets
   *          array of MessageSets
   * @param term
   *          SearchTerm
  ...
  /**
   * Issue the given search criterion on all messages in this folder. Returns
   * array of matching sequence numbers. An empty array is returned if no
   * matches are found.
   * 
   * @param term
  ...
  /*
   * Apply the given SearchTerm on the specified sequence. Returns array of
   * matching sequence numbers. Note that an empty array is returned for no
   * matches.
   */
  private int[] search(String msgSequence, SearchTerm term)
    ...
    /*
     * The search "text" terms do contain non-ASCII chars. We need to use SEARCH
     * CHARSET <charset> ... The charsets we try to use are UTF-8 and the
     * locale's default charset. If the server supports UTF-8, great, always use
     * it. Else we try to use the default charset.
     */

    // Cycle thru the list of charsets
    for(int i=0; i < searchCharsets.length; i++){
      if(searchCharsets[i] == null) continue;
      ...
      try{
        ...
        /*
         * Server returned NO. For now, I'll just assume that this indicates
         * that this charset is unsupported. We can check the BADCHARSET
         * response code once that's spec'd into the IMAP RFC ..
         */
  ...
  /*
   * Apply the given SearchTerm on the specified sequence, using the given
   * charset. <p> Returns array of matching sequence numbers. Note that an empty
  ...
  private int[] issueSearch(String msgSequence, SearchTerm term,
    ...
    else r=command("SEARCH CHARSET " + charset, args);
  ...
  public Namespaces namespace() throws ProtocolException{
    if(!hasCapability("NAMESPACE")) throw new BadCommandException("NAMESPACE not supported");
    ...
    if(response.isOK()){ // command succesful
      for(int i=0, len=r.length; i < len; i++){
        ...
        if(ir.keyEquals("NAMESPACE")){
          if(namespace == null) namespace=new Namespaces(ir);
          r[i]=null;
        }
      }
    }
  ...
  /**
   * GETQUOTAROOT Command. Returns an array of Quota objects, representing the
   * quotas for this mailbox and, indirectly, the quotaroots for this mailbox.
   * 
   * @see "RFC2087"
   */
  public Quota[] getQuotaRoot(String mbox) throws ProtocolException{
    if(!hasCapability("QUOTA")) throw new BadCommandException("GETQUOTAROOT not supported");
  ...
  /**
   * GETQUOTA Command. Returns an array of Quota objects, representing the
   * quotas for this quotaroot.
   * 
   * @see "RFC2087"
   */
  public Quota[] getQuota(String root) throws ProtocolException{
    if(!hasCapability("QUOTA")) throw new BadCommandException("QUOTA not supported");
  ...
  public void setQuota(Quota quota) throws ProtocolException{
    ...
    /*
     * Quota quota = null; Vector v = new Vector(); // Grab all QUOTA responses
     * if (response.isOK()) { // command succesful for (int i = 0, len =
     * r.length; i < len; i++) { if (!(r[i] instanceof IMAPResponse)) continue;
     * IMAPResponse ir = (IMAPResponse)r[i]; if (ir.keyEquals("QUOTA")) { quota
     * = parseQuota(ir); v.addElement(quota); r[i] = null; } } }
     */
    ...
    handleResult(response);
    /*
     * Quota[] qa = new Quota[v.size()]; v.copyInto(qa); return qa;
     */
  }
  ...
  private Quota parseQuota(Response r) throws ParsingException{
    ...
    if(r.readByte() != '(') throw new ParsingException("parse error in QUOTA");
  ...
  public void setACL(String mbox, char modifier, ACL acl)
      throws ProtocolException{
    if(!hasCapability("ACL")) throw new BadCommandException("ACL not supported");
    ...
    if(modifier == '+' || modifier == '-') rights=modifier + rights;
    ...
    if(response.isOK()){ // command succesful
      for(int i=0, len=r.length; i < len; i++){
        ...
        if(ir.keyEquals("MYRIGHTS")){
          ...
          if(rights == null) rights=new Rights(rs);
          r[i]=null;
        }
      }
    }
  ...
  /*
   * The tag used on the IDLE command. Set by idleStart() and used in
   * processIdleResponse() to determine if the response is the matching end tag.
   */
   ...
   * IDLE Command.
   * <p>
   * If the server supports the IDLE command extension, the IDLE command is
   * issued and this method blocks until a response has been received. Once the
   * first response has been received, the IDLE command is terminated and all
   * responses are collected and handled and this method returns.
   * <p>
   * Note that while this method is blocked waiting for a response, no other
   * threads may issue any commands to the server that would use this same
   * connection.
   * 
   * @see "RFC2177"
   * @since JavaMail 1.4.1
  ...
  public synchronized void idleStart() throws ProtocolException{
    if(!hasCapability("IDLE")) throw new BadCommandException("IDLE not supported");
    ...
    if(!r.isContinuation()) handleResult(r);
  }

  /**
   * While an IDLE command is in progress, read a response sent from the server.
   * The response is read with no locks held so that when the read blocks
   * waiting for the response from the server it's not holding locks that would
   * prevent other threads from interrupting the IDLE command.
   * 
   * @since JavaMail 1.4.1
   */
  public synchronized Response readIdleResponse(){
    if(idleTag == null) return null; // IDLE not in progress
    Response r;
    try{
      r=readResponse();
    }
  ...
  /**
   * Process a response returned by readIdleResponse(). This method will be
   * called with appropriate locks held so that the processing of the response
   * is safe.
   * 
   * @since JavaMail 1.4.1
   */
  public boolean processIdleResponse(Response r) throws ProtocolException{
    ...
    if(r.isTagged() && r.getTag().equals(idleTag)) done=true;

    if(done) idleTag=null; // no longer in IDLE
   ...
   * another thread will use this method to abort the IDLE command, which will
   * cause the server to send the closing tag for the IDLE command, which
   * readIdleResponse() and processIdleResponse() will see and terminate the
   * IDLE state.
   * 
   * @since JavaMail 1.4.1
   */
