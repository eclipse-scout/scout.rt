...
public class FetchResponse extends IMAPResponse{
  ...
  public Item getItem(Class c){
    for(int i=0; i < items.length; i++){
      if(c.isInstance(items[i])) return items[i];
    }
  ...
  public static Item getItem(Response[] r, int msgno, Class c){
    ...
    for(int i=0; i < r.length; i++){
      ...
      for(int j=0; j < f.items.length; j++){
        if(c.isInstance(f.items[j])) return f.items[j];
      }
    }
  ...
  private void parse() throws ParsingException{
    skipSpaces();
    if(buffer[index] != '(') throw new ParsingException(
    ...
    do{
      ...
      if(index >= size) throw new ParsingException(
        ...
        case 'B':
          if(match(BODY.name)){
            if(buffer[index + 4] == '['){
              ...
              if(match(BODYSTRUCTURE.name)) index+=BODYSTRUCTURE.name.length;
              // skip "BODYSTRUCTURE"
              else index+=BODY.name.length; // skip "BODY"
        ...
        case 'R':
          if(match(RFC822SIZE.name)){
            ...
            if(match(RFC822DATA.name)){
              ...
              if(match(HEADER)) index+=HEADER.length; // skip ".HEADER"
              else if(match(TEXT)) index+=TEXT.length; // skip ".TEXT"
  ...
  /*
   * itemName is the name of the IMAP item to compare against. NOTE that
   * itemName *must* be all uppercase.
   */
  private boolean match(char[] itemName){
    ...
    for(int i=0, j=index; i < len;)
      ...
      if(Character.toUpperCase((char)buffer[j++]) != itemName[i++]) return false;
    return true;
  }
}
