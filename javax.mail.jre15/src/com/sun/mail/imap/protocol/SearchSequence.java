...
/**
 * This class traverses a search-tree and generates the corresponding IMAP
 * search sequence.
 * 
 * @author John Mani
 */
class SearchSequence{
  ...
  static Argument generateSequence(SearchTerm term, String charset)
    ...
    /*
     * Call the appropriate handler depending on the type of the search-term ...
     */
    ...
    else throw new SearchException("Search too complex");
  }

  /*
   * Check if the "text" terms in the given SearchTerm contain non US-ASCII
   * characters.
   */
  static boolean isAscii(SearchTerm term){
    if(term instanceof AndTerm || term instanceof OrTerm){
      SearchTerm[] terms;
      if(term instanceof AndTerm) terms=((AndTerm)term).getTerms();
      else terms=((OrTerm)term).getTerms();

      for(int i=0; i < terms.length; i++)
        if(!isAscii(terms[i])) // outta here !
        return false;
    }
    else if(term instanceof NotTerm) return isAscii(((NotTerm)term).getTerm());
    else if(term instanceof StringTerm) return isAscii(((StringTerm)term).getPattern());
    else if(term instanceof AddressTerm) return isAscii(((AddressTerm)term).getAddress().toString());
  ...
  private static Argument or(OrTerm term, String charset)
    ...
    /*
     * The IMAP OR operator takes only two operands. So if we have more than 2
     * operands, group them into 2-operand OR Terms.
     */
    if(terms.length > 2){
    ...
    if(terms.length > 1) result.writeAtom("OR");

    /*
     * If this term is an AND expression, we need to enclose it within
     * paranthesis. AND expressions are either AndTerms or FlagTerms
     */
    if(terms[0] instanceof AndTerm || terms[0] instanceof FlagTerm) result.writeArgument(generateSequence(terms[0], charset));
    else result.append(generateSequence(terms[0], charset));

    // Repeat the above for the second term, if there is one
    if(terms.length > 1){
      if(terms[1] instanceof AndTerm || terms[1] instanceof FlagTerm) result.writeArgument(generateSequence(terms[1], charset));
      else result.append(generateSequence(terms[1], charset));
    }
  ...
  private static Argument not(NotTerm term, String charset)
    ...
    if(nterm instanceof AndTerm || nterm instanceof FlagTerm) result.writeArgument(generateSequence(nterm, charset));
    else result.append(generateSequence(nterm, charset));
  ...
  private static Argument flag(FlagTerm term) throws SearchException{
    ...
    if(sf.length == 0 && uf.length == 0) throw new SearchException("Invalid FlagTerm");

    for(int i=0; i < sf.length; i++){
      if(sf[i] == Flags.Flag.DELETED) result.writeAtom(set ? "DELETED" : "UNDELETED");
      else if(sf[i] == Flags.Flag.ANSWERED) result.writeAtom(set ? "ANSWERED" : "UNANSWERED");
      else if(sf[i] == Flags.Flag.DRAFT) result.writeAtom(set ? "DRAFT" : "UNDRAFT");
      else if(sf[i] == Flags.Flag.FLAGGED) result.writeAtom(set ? "FLAGGED" : "UNFLAGGED");
      else if(sf[i] == Flags.Flag.RECENT) result.writeAtom(set ? "RECENT" : "OLD");
      else if(sf[i] == Flags.Flag.SEEN) result.writeAtom(set ? "SEEN" : "UNSEEN");
    }
  ...
  private static Argument recipient(Message.RecipientType type,
    ...
    if(type == Message.RecipientType.TO) result.writeAtom("TO");
    else if(type == Message.RecipientType.CC) result.writeAtom("CC");
    else if(type == Message.RecipientType.BCC) result.writeAtom("BCC");
    else throw new SearchException("Illegal Recipient type");
  ...
  /**
   * Print an IMAP Date string, that is suitable for the Date SEARCH commands.
   * The IMAP Date string is : date ::= date_day "-" date_month "-" date_year
