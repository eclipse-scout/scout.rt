 ...
 * <code>Folder</code> object to an <code>IMAPFolder</code> object and use the
 * methods on this class. The {@link #getQuota getQuota} and {@link #setQuota
 * setQuota} methods support the IMAP QUOTA extension. Refer to <A
 * HREF="http://www.ietf.org/rfc/rfc2087.txt">RFC 2087</A> for more information.
 * <p>
 * The {@link #getACL getACL}, {@link #addACL addACL}, {@link #removeACL
 * removeACL}, {@link #addRights addRights}, {@link #removeRights removeRights},
 * {@link #listRights listRights}, and {@link #myRights myRights} methods
 * support the IMAP ACL extension. Refer to <A
 * HREF="http://www.ietf.org/rfc/rfc2086.txt">RFC 2086</A> for more information.
 * <p>
 * The {@link #doCommand doCommand} method and
 * {@link IMAPFolder.ProtocolCommand IMAPFolder.ProtocolCommand} interface
 * support use of arbitrary IMAP protocol commands.
 * <p>
 ...
 * <p>
 * <strong>WARNING:</strong> The APIs unique to this class should be considered
 * <strong>EXPERIMENTAL</strong>. They may be changed in the future in ways that
 * are incompatible with applications using the current APIs.
 * 
...
/*
 * The folder object itself serves as a lock for the folder's state EXCEPT for
 * the message cache (see below), typically by using synchronized methods. When
 * checking that a folder is open or closed, the folder's lock must be held.
 * It's important that the folder's lock is acquired before the messageCacheLock
 * (see below). Thus, the locking hierarchy is that the folder lock, while
 * optional, must be acquired before the messageCacheLock, if it's acquired at
 * all. Be especially careful of callbacks that occur while holding the
 * messageCacheLock into (e.g.) superclass Folder methods that are synchronized.
 * Note that methods in IMAPMessage will acquire the messageCacheLock without
 * acquiring the folder lock. <p> When a folder is opened, it creates a
 * messageCache (a Vector) of empty IMAPMessage objects. Each Message has a
 * messageNumber - which is its index into the messageCache, and a
 * sequenceNumber - which is its IMAP sequence-number. All operations on a
 * Message which involve communication with the server, use the message's
 * sequenceNumber. <p> The most important thing to note here is that the server
 * can send unsolicited EXPUNGE notifications as part of the responses for
 * "most" commands. Refer RFC2060, sections 5.3 & 5.5 for gory details. Also,
 * the server sends these notifications AFTER the message has been expunged. And
 * once a message is expunged, the sequence-numbers of those messages after the
 * expunged one are renumbered. This essentially means that the mapping between
 * *any* Message and its sequence-number can change in the period when a IMAP
 * command is issued and its responses are processed. Hence we impose a strict
 * locking model as follows: <p> We define one mutex per folder - this is just a
 * Java Object (named messageCacheLock). Any time a command is to be issued to
 * the IMAP server (i.e., anytime the corresponding IMAPProtocol method is
 * invoked), follow the below style: synchronized (messageCacheLock) { //
 * ACQUIRE LOCK issue command () // The response processing is typically done
 * within // the handleResponse() callback. A few commands (Fetch, // Expunge)
 * return *all* responses and hence their // processing is done here itself.
 * Now, as part of the // processing unsolicited EXPUNGE responses, we renumber
 * // the necessary sequence-numbers. Thus the renumbering // happens within
 * this critical-region, surrounded by // locks. process responses () } //
 * RELEASE LOCK This technique is used both by methods in IMAPFolder and by
 * methods in IMAPMessage and other classes that operate on data in the folder.
 * Note that holding the messageCacheLock has the side effect of preventing the
 * folder from being closed, and thus ensuring that the folder's protocol object
 * is still valid. The protocol object should only be accessed while holding the
 * messageCacheLock (except for calls to IMAPProtocol.isREV1(), which don't need
 * to be protected because it doesn't access the server). Note that interactions
 * with the Store's protocol connection do not have to be protected as above,
 * since the Store's protocol is never in a "meaningful" SELECT-ed state.
 */
...
public class IMAPFolder extends Folder implements UIDFolder, ResponseHandler{
  ...
  /*
   * An IMAP delimiter is a 7bit US-ASCII character. (except NUL). We use
   * '\uffff' (a non 7bit character) to indicate that we havent yet determined
   * what the separator character is. We use '\u0000' (NUL) to indicate that no
   * separator character exists, i.e., a flat hierarchy
   */
  ...
  /*
   * This field tracks the state of this folder. If the folder is closed due to
   * external causes (i.e, not thru the close() method), then this field will
   * remain false. If the folder is closed thru the close() method, then this
   * field is set to true. If reallyClosed is false, then a
   * FolderClosedException is generated when a method is invoked on any
   * Messaging object owned by this folder. If reallyClosed is true, then the
   * IllegalStateException runtime exception is thrown.
   */
  private boolean reallyClosed=true;

  /*
   * The idleState field supports the IDLE command. Normally when executing an
   * IMAP command we hold the messageCacheLock and often the folder lock (see
   * above). While executing the IDLE command we can't hold either of these
   * locks or it would prevent other threads from entering Folder methods even
   * far enough to check whether an IDLE command is in progress. We need to
   * check before issuing another command so that we can abort the IDLE command.
   * The idleState field is protected by the messageCacheLock. The RUNNING state
   * is the normal state and means no IDLE command is in progress. The IDLE
   * state means we've issued an IDLE command and are reading responses. The
   * ABORTING state means we've sent the DONE continuation command and are
   * waiting for the thread running the IDLE command to break out of its read
   * loop. When an IDLE command is in progress, the thread calling the idle
   * method will be reading from the IMAP connection while holding neither the
   * folder lock nor the messageCacheLock. It's obviously critical that no other
   * thread try to send a command or read from the connection while in this
   * state. However, other threads can send the DONE continuation command that
   * will cause the server to break out of the IDLE loop and send the ending tag
   * response to the IDLE command. The thread in the idle method that's reading
   * the responses from the IDLE command will see this ending response and
   * complete the idle method, setting the idleState field back to RUNNING, and
   * notifying any threads waiting to use the connection. All uses of the IMAP
   * connection (IMAPProtocol object) must be done while holding the
   * messageCacheLock and must be preceeded by a check to make sure an IDLE
   * command is not running, and abort the IDLE command if necessary. While
   * waiting for the IDLE command to complete, these other threads will give up
   * the messageCacheLock, but might still be holding the folder lock. This
   * check is done by the getProtocol() method, resulting in a typical usage
   * pattern of: synchronized (messageCacheLock) { IMAPProtocol p =
   * getProtocol(); // may block waiting for IDLE // ... use protocol }
   */
  ...
  /**
   * A fetch profile item for fetching headers. This inner class extends the
   * <code>FetchProfile.Item</code> class to add new FetchProfile item types,
   * specific to IMAPFolders.
   * 
   * @see FetchProfile
   */
  public static class FetchProfileItem extends FetchProfile.Item{
     ...
     * <code>FetchProfile</code> during a fetch request to a Folder. This item
     * indicates that the headers for messages in the specified range are
     * desired to be prefetched.
     * <p>
     ...
     * <code>FetchProfile</code> during a fetch request to a Folder. This item
     * indicates that the sizes of the messages in the specified range are
     * desired to be prefetched.
     * <p>
   ...
   * 
   * @param fullName
   *          fullname of this folder
   * @param separator
   *          the default separator character for this folder's namespace
   * @param store
   *          the Store
   */
  protected IMAPFolder(String fullName, char separator, IMAPStore store) {
    super(store);
    if(fullName == null) throw new NullPointerException("Folder name is null");
    ...
    /*
     * Work around apparent bug in Exchange. Exchange will return a name of
     * "Public Folders/" from LIST "%". If name has one separator, and it's at
     * the end, assume this is a namespace name and treat it accordingly.
     * Usually this will happen as a result of the list method, but this also
     * allows getFolder to work with namespace names.
     */
  ...
  protected IMAPFolder(ListInfo li, IMAPStore store) {
    ...
    if(li.hasInferiors) type|=HOLDS_FOLDERS;
    if(li.canOpen) type|=HOLDS_MESSAGES;
    exists=true;
    attributes=li.attrs;
  }

  /*
   * Ensure that this folder exists. If 'exists' has been set to true, we don't
   * attempt to validate it with the server again. Note that this can result in
   * a possible loss of sync with the server.
   */
  private void checkExists() throws MessagingException{
    ...
    if(!exists && !exists()) throw new FolderNotFoundException(
        this, fullName + " not found");
  }

  /*
   * Ensure the folder is closed. ASSERT: Must be called with this folder's
  ...
  /*
   * Ensure the folder is open. ASSERT: Must be called with this folder's
   * synchronization lock held.
   */
  private void checkOpened() throws FolderClosedException{
    assert Thread.holdsLock(this);
    if(!opened){
      if(reallyClosed) throw new IllegalStateException(
  ...
  /*
   * Check that the given message number is within the range of messages present
   * in this folder. If the message number is out of range, we ping the server
   * to obtain any pending new message notifications from the server.
   */
  private void checkRange(int msgno) throws MessagingException{
    if(msgno < 1) // message-numbers start at 1
    throw new IndexOutOfBoundsException();

    if(msgno <= total) return;
  ...
  /*
   * Check whether the given flags are supported by this server, and also verify
   * that the folder allows setting flags.
   */
  private void checkFlags(Flags flags) throws MessagingException{
    assert Thread.holdsLock(this);
    if(mode != READ_WRITE) throw new IllegalStateException(
        "Cannot change flags on READ_ONLY folder: " + fullName);
    /*
     * if (!availableFlags.contains(flags)) throw new MessagingException(
  ...
  public synchronized String getName(){
    /*
     * Return the last component of this Folder's full name. Folder components
     * are delimited by the separator character.
     */
    if(name == null){
      try{
  ...
  public synchronized Folder getParent() throws MessagingException{
    char c=getSeparator();
    int index;
    if((index=fullName.lastIndexOf(c)) != -1) return new IMAPFolder(fullName.substring(0, index),
        c, (IMAPStore)store);
  ...
  public synchronized boolean exists() throws MessagingException{
    ...
    ListInfo[] li=null;
    final String lname;
    if(isNamespace && separator != '\0') lname=fullName + separator;
    else lname=fullName;
    ...
    if(li != null){
      ...
      type=0;
      if(li[i].hasInferiors) type|=HOLDS_FOLDERS;
      if(li[i].canOpen) type|=HOLDS_MESSAGES;
      exists=true;
  ...
  /**
   * Which entry in <code>li</code> matches <code>lname</code>? If the name
   * contains wildcards, more than one entry may be returned.
   */
  private int findName(ListInfo[] li, String lname){
    int i;
    // if the name contains a wildcard, there might be more than one
    for(i=0; i < li.length; i++){
      if(li[i].name.equals(lname)) break;
    }
  ...
  private synchronized Folder[] doList(final String pattern,
    ...
    if(li == null) return new Folder[0];

    /*
     * The UW based IMAP4 servers (e.g. SIMS2.0) include current folder
     * (terminated with the separator), when the LIST pattern is '%' or '*'.
     * i.e, <LIST "" mail/%> returns "mail/" as the first LIST response. Doesn't
     * make sense to include the current folder in this case, so we filter it
     * out. Note that I'm assuming that the offending response is the *first*
     * one, my experiments with the UW & SIMS2.0 servers indicate that ..
     */
    int start=0;
    // Check the first LIST response.
    if(li.length > 0 && li[0].name.equals(fullName + c)) start=1; // start from
    // index = 1
  ...
  public synchronized char getSeparator() throws MessagingException{
    if(separator == UNKNOWN_SEPARATOR){
      ...
      if(li != null) separator=li[0].separator;
      else separator='/'; // punt !
    }
    return separator;
  }
  ...
  public synchronized int getType() throws MessagingException{
    if(opened){
      ...
      if(attributes == null) exists(); // try to fetch attributes
    }
    else{
      checkExists();
    }
  ...
  public synchronized void setSubscribed(final boolean subscribe)
    ...
    doCommandIgnoreFailure(new ProtocolCommand(){
      public Object doCommand(IMAPProtocol p) throws ProtocolException{
        if(subscribe) p.subscribe(fullName);
        else p.unsubscribe(fullName);
        return null;
      }
    });
  }
  ...
  public synchronized boolean create(final int type)
    ...
    if(ret == null) return false; // CREATE failure, maybe this
  ...
  public Folder getFolder(String name) throws MessagingException{
    ...
    if(attributes != null && !isDirectory()) throw new MessagingException("Cannot contain subfolders");
  ...
  public synchronized boolean renameTo(final Folder f)
    ...
    checkExists();
    if(f.getStore() != store) throw new MessagingException("Can't rename across Stores");
  ...
  public synchronized void open(int mode) throws MessagingException{
    ...
    lock: synchronized(messageCacheLock){ // Acquire messageCacheLock

      /*
       * Add response handler right away so we get any alerts or notifications
       * that occur during the SELECT or EXAMINE. Have to be sure to remove it
       * if we fail to open the folder.
       */
      protocol.addResponseHandler(this);

      try{
        if(mode == READ_ONLY) mi=protocol.examine(fullName);
        else mi=protocol.select(fullName);
      }
    ...
    /*
     * Handle SELECT or EXAMINE failure after lock is released. Try to figure
     * out why the operation failed so we can report a more reasonable
     * exception.
     */
    if(exc != null){
      checkExists(); // throw exception if folder doesn't exist

      if((type & HOLDS_MESSAGES) == 0) throw new MessagingException("folder cannot contain messages");
  ...
  public synchronized void setFlags(Message[] msgs, Flags flag, boolean value)
    ...
    synchronized(messageCacheLock){
      try{
        ...
        if(ms == null) throw new MessageRemovedException(
  ...
  private void close(boolean expunge, boolean force)
    ...
    synchronized(messageCacheLock){
      /*
       * If we already know we're closed, this is illegal. Can't use
       * checkOpened() because if we were forcibly closed asynchronously we just
       * want to complete the closing here.
       */
      if(!opened && reallyClosed) throw new IllegalStateException(
          "This operation is not allowed on a closed folder");
      ...
      try{
        waitIfIdle();
        if(force){
          if(debug) out.println("DEBUG: forcing folder " + fullName +
              " to close");
          ...
          if(debug) out.println("DEBUG: pool is full, not adding " +
              "an Authenticated connection");

          // If the expunge flag is set, close the folder first.
          if(expunge) protocol.close();

          if(protocol != null) protocol.logout();
        }
        else{
  ...
  private Status getStatus() throws ProtocolException{
    ...
    if(statusCacheTimeout > 0 && cachedStatus != null &&
        System.currentTimeMillis() - cachedStatusTime < statusCacheTimeout) return cachedStatus;
  ...
  /**
   * Append the given messages into this folder. Return array of AppendUID
   * objects containing UIDs of these messages in the destination folder. Each
   * element of the returned array corresponds to an element of the
   * <code>msgs</code> array. A null element means the server didn't return UID
   * information for the appended message.
   * <p>
   * Depends on the APPENDUID response code defined by the UIDPLUS extension -
  ...
  /**
   * Append the given messages into this folder. Return array of Message objects
   * representing the messages in the destination folder. Note that the folder
   * must be open. Each element of the returned array corresponds to an element
   * of the <code>msgs</code> array. A null element means the server didn't
   * return UID information for the appended message.
   * <p>
   * Depends on the APPENDUID response code defined by the UIDPLUS extension -
   * <A HREF="http://www.ietf.org/rfc/rfc2359.txt">RFC 2359</A>.
   * 
    ...
    if(folder.getStore() == store){
      synchronized(messageCacheLock){
        try{
          ...
          if(cfx.getMessage().indexOf("TRYCREATE") != -1) throw new FolderNotFoundException(
              folder,
              folder.getFullName() + " does not exist");
          else throw new MessagingException(cfx.getMessage(), cfx);
        }
        catch(ConnectionException cex){
          throw new FolderClosedException(this, cex.getMessage());
        }
  ...
  public synchronized Message[] expunge(Message[] msgs)
    ...
    synchronized(messageCacheLock){
      ...
      try{
        IMAPProtocol p=getProtocol();
        if(msgs != null) p.uidexpunge(Utility.toUIDSet(msgs));
        else p.expunge();
      }
      catch(CommandFailedException cfx){
      ...
      for(int i=0; i < messageCache.size();){
        ...
        if(m.isExpunged()){
          ...
          /*
           * remove this message from the messageCache. Note that this also
           * causes all succeeding messages in the cache to shifted downward in
           * the vector, therby decrementing the vector's size. (and hence we
           * need to do messageCache.size() at the top of this loop.
           */
          ...
          if(uidTable != null){
            long uid=m.getUID();
            if(uid != -1) uidTable.remove(new Long(uid));
          }
        }
        else{
          /*
    ...
    v.copyInto(rmsgs);
    if(rmsgs.length > 0) notifyMessageRemovedListeners(true, rmsgs);
    return rmsgs;
  }
  ...
  /**
   * Search the folder for messages matching the given term. Returns array of
   * matching messages. Returns an empty array if no matching messages are
   * found.
   */
  ...
  /**
   * Returns the predicted UID that will be assigned to the next message that is
   * appended to this folder. If the folder is closed, the STATUS command is
   * used to retrieve this value. If the folder is open, the value returned from
   * the SELECT or EXAMINE command is returned. Note that messages may have been
   * appended to the folder while it was open and thus this value may be out of
   * date.
   * <p>
   * Servers implementing RFC2060 likely won't return this value when a folder
   * is opened. Servers implementing RFC3501 should return this value when a
   * folder is opened.
   * <p>
   * 
   ...
   * <p>
   * Returns Message objects for all valid messages in this range. Returns an
   * empty array if no messages are found.
   */
  public synchronized Message[] getMessagesByUID(long start, long end)
    ...
    try{
      synchronized(messageCacheLock){
        if(uidTable == null) uidTable=new Hashtable();
   ...
   * <p>
   * <code>uids.length()</code> elements are returned. If any UID in the array
   * is invalid, a <code>null</code> entry is returned for that element.
   */
  ...
  public synchronized long getUID(Message message)
      throws MessagingException{
    if(message.getFolder() != this) throw new NoSuchElementException(
  ...
  /**
   * Get the quotas for the quotaroot associated with this folder. Note that
   * many folders may have the same quotaroot. Quotas are controlled on the
   * basis of a quotaroot, not (necessarily) a folder. The relationship between
   * folders and quotaroots depends on the IMAP server. Some servers might
   * implement a single quotaroot for all folders owned by a user. Other servers
   * might implement a separate quotaroot for each folder. A single folder can
   * even have multiple quotaroots, perhaps controlling quotas for different
   * resources.
   * 
   * @return array of Quota objects for the quotaroots associated with this
   *         folder
  ...
  /**
   * Set the quotas for the quotaroot specified in the quota argument. Typically
   * this will be one of the quotaroots associated with this folder, as obtained
   * from the <code>getQuota</code> method, but it need not be.
   * 
   * @param quota
   *          the quota to set
   * @exception MessagingException
  ...
  /**
   * Add an access control list entry to the access control list for this
   * folder.
   * 
   * @param acl
  ...
  /**
   * Remove any access control list entry for the given identifier from the
   * access control list for this folder.
   * 
   * @param name
  ...
  /**
   * Add the rights specified in the ACL to the entry for the identifier
   * specified in the ACL. If an entry for the identifier doesn't already exist,
   * add one.
   * 
   * @param acl
  ...
  /**
   * Remove the rights specified in the ACL from the entry for the identifier
  ...
  /**
   * Get all the rights that may be allowed to the given identifier. Rights are
   * grouped per RFC 2086 and each group is returned as an element of the array.
   * The first element of the array is the set of rights that are always granted
   * to the identifier. Later elements are rights that may be optionally granted
   * to the identifier.
   * <p>
   * Note that this method lists the rights that it is possible to assign to the
   * given identifier, <em>not</em> the rights that are actually granted to the
   * given identifier. For the latter, see the <code>getACL</code> method.
   * 
   * @param name
   *          the identifier to list rights for
   * @return array of Rights objects representing possible rights for the
   *         identifier
   * @exception MessagingException
   *              if the server doesn't support the ACL extension
   */
  ...
  public String[] getAttributes() throws MessagingException{
    if(attributes == null) exists(); // do a LIST to set the attributes
    return (String[])(attributes.clone());
  }

  /**
   * Use the IMAP IDLE command (see <A
   * HREF="http://www.ietf.org/rfc/rfc2177.txt">RFC 2177</A>), if supported by
   * the server, to enter idle mode so that the server can send unsolicited
   * notifications of new messages arriving, etc. without the need for the
   * client to constantly poll the server. Use an appropriate listener to be
   * notified of new messages or other events. When another thread (e.g., the
   * listener thread) needs to issue an IMAP comand for this folder, the idle
   * mode will be terminated and this method will return. Typically the caller
   ...
   * issue commands before the caller invokes this method again. The default
   * delay is 10 milliseconds.
   * 
   * @exception MessagingException
   *              if the server doesn't support the IDLE extension
   * @exception IllegalStateException
   *              if the folder isn't open
  ...
  public void idle() throws MessagingException{
    ...
    synchronized(this){
      ...
      if(!started.booleanValue()) return;
    }

    /*
     * We gave up the folder lock so that other threads can get into the folder
     * far enough to see that we're in IDLE and abort the IDLE. Now we read
     * responses from the IDLE command, especially including unsolicited
     * notifications from the server. We don't hold the messageCacheLock while
     * reading because it protects the idleState and other threads need to be
     * able to examine the state. We hold the messageCacheLock while processing
     * the responses so that we can update the number of messages in the folder
     * (for example).
     */
    for(;;){
    ...
    /*
     * Enforce a minimum delay to give time to threads processing the responses
     * that came in while we were idle.
     */
  ...
  /*
   * If an IDLE command is in progress, abort it if necessary, and wait until it
   * completes. ASSERT: Must be called with the message cache lock held.
   */
  ...
  /**
   * The response handler. This is the callback routine that is invoked by the
   * protocol layer.
   */
  /*
   * ASSERT: This method must be called only when holding the messageCacheLock.
   * ASSERT: This method must *not* invoke any other method that might grab the
   * 'folder' lock or 'message' lock (i.e., any synchronized methods on
   * IMAPFolder or IMAPMessage) since that will result in violating the locking
   * hierarchy.
   */
  public void handleResponse(Response r){
     ...
     */
    if(r.isOK() || r.isNO() || r.isBAD() || r.isBYE()) ((IMAPStore)store).handleResponseCode(r);

    /*
     * Now check whether this is a BYE or OK response and handle appropriately.
     */
    if(r.isBYE()){
  ...
  /**
   * Handle the given array of Responses. ASSERT: This method must be called
   * only when holding the messageCacheLock
   */
  void handleResponses(Response[] r){
    for(int i=0; i < r.length; i++){
      if(r[i] != null) handleResponse(r[i]);
    }
  }

  /**
   * Get this folder's Store's protocol connection. When acquiring a store
   * protocol object, it is important to use the following steps: IMAPProtocol p
   * = null; try { p = getStoreProtocol(); // perform the command } catch
   * (WhateverException ex) { // handle it } finally { releaseStoreProtocol(p);
   * }
   */
  ...
  private synchronized void throwClosedException(ConnectionException cex)
    ...
    if((protocol != null && cex.getProtocol() == protocol) ||
        (protocol == null && !reallyClosed)) throw new FolderClosedException(this, cex.getMessage());
    else throw new StoreClosedException(store, cex.getMessage());
  }
  ...
  /**
   * Execute a user-supplied IMAP command. The command is executed in the
   * appropriate context with the necessary locks held and using the appropriate
   * <code>IMAPProtocol</code> object.
   * <p>
   * This method returns whatever the <code>ProtocolCommand</code> object's
   * <code>doCommand</code> method returns. If the <code>doCommand</code> method
   * throws a <code>ConnectionException</code> it is translated into a
   * <code>StoreClosedException</code> or <code>FolderClosedException</code> as
   * appropriate. If the <code>doCommand</code> method throws a
   * <code>ProtocolException</code> it is translated into a
   * <code>MessagingException</code>.
   * <p>
   * The following example shows how to execute the IMAP NOOP command. Executing
   * more complex IMAP commands requires intimate knowledge of the
   * <code>com.sun.mail.iap</code> and <code>com.sun.mail.imap.protocol</code>
   * packages, best acquired by reading the source code.
   * <p>
   * <blockquote>
   * 
   * <pre>
   ...
   * </blockquote>
   * <p>
   * Here's a more complex example showing how to use the proposed IMAP SORT
   * extension:
   * <p>
   * 
   * <pre>
  ...
  /**
   * Release the store protocol object. If we borrowed a protocol object from
   * the connection pool, give it back. If we used our own protocol object,
   * nothing to do.
   */
  protected synchronized void releaseStoreProtocol(IMAPProtocol p){
    if(p != protocol) ((IMAPStore)store).releaseStoreProtocol(p);
  }

  /**
   * Release the protocol object. ASSERT: This method must be called only when
   * holding the messageCacheLock
   */
  private void releaseProtocol(boolean returnToPool){
    if(protocol != null){
      protocol.removeResponseHandler(this);

      if(returnToPool) ((IMAPStore)store).releaseProtocol(this, protocol);
      else ((IMAPStore)store).releaseProtocol(this, null);
    }
  }

  /**
   * Issue a noop command for the connection if the connection has not been used
   * in more than a second. If <code>keepStoreAlive</code> is true, also issue a
   * noop over the store's connection. ASSERT: This method must be called only
   * when holding the messageCacheLock
   */
  private void keepConnectionAlive(boolean keepStoreAlive)
    ...
    if(keepStoreAlive && ((IMAPStore)store).hasSeparateStoreConnection()){
      ...
      try{
        ...
        if(System.currentTimeMillis() - p.getTimestamp() > 1000) p.noop();
      }
      finally{
  ...
  /**
   * Get the message object for the given sequence number. If none found, null
   * is returned. ASSERT: This method must be called only when holding the
   * messageCacheLock
   */
  IMAPMessage getMessageBySeqNumber(int seqnum){
    /*
     * Check messageCache for message matching the given sequence number. We
     * start the search from position (seqnum-1) and continue down the vector
     * till we get a match.
     */
    for(int i=seqnum - 1; i < total; i++){
      IMAPMessage msg=(IMAPMessage)messageCache.elementAt(i);
      if(msg.getSequenceNumber() == seqnum) return msg;
    }
    return null;
  }
...
/**
 * An object that holds a Message object and reports its size and writes it to
 * another OutputStream on demand. Used by appendMessages to avoid the need to
 * buffer the entire message in memory in a single byte array before sending it
 * to the server.
 */
class MessageLiteral implements Literal{
  ...
  public void writeTo(OutputStream os) throws IOException{
    ...
    try{
      if(buf != null) os.write(buf, 0, msgSize);
      else{
...
/**
 * Count the number of bytes written to the stream. Also, save a copy of small
 * messages to avoid having to process the data again.
 */
