...
public class IMAPInputStream extends InputStream{
  ...
  private void forceCheckExpunged()
    ...
    if(msg.isExpunged()) throw new MessageRemovedIOException();
  }

  /**
   * Fetch more data from the server. This method assumes that all data has
   * already been read in, hence bufpos > bufcount.
   */
  private void fill() throws IOException{
    /*
     * If we know the total number of bytes available from this section, let's
     * check if we have consumed that many bytes.
     */
    if(max != -1 && pos >= max){
      if(pos == 0) checkSeen();
    ...
    BODY b=null;
    if(readbuf == null) readbuf=new ByteArray(blksize + slop);
    ...
    synchronized(msg.getMessageCacheLock()){
      try{
        ...
        int cnt=blksize;
        if(max != -1 && pos + blksize > max) cnt=max - pos;
        if(peek) b=p.peekBody(seqnum, section, pos, cnt, readbuf);
        else b=p.fetchBody(seqnum, section, pos, cnt, readbuf);
      }
      catch(ProtocolException pex){
        forceCheckExpunged();
  ...
  /**
   * Reads the next byte of data from this buffered input stream. If no byte is
   * available, the value <code>-1</code> is returned.
   */
  public synchronized int read() throws IOException{
    if(bufpos >= bufcount){
      fill();
      if(bufpos >= bufcount) return -1; // EOF
    }
    return buf[bufpos++] & 0xff;
  }

  /**
   * Reads up to <code>len</code> bytes of data from this input stream into the
   * given buffer.
   * <p>
   * Returns the total number of bytes read into the buffer, or <code>-1</code>
   * if there is no more data.
   * <p>
   * Note that this method mimics the "weird !" semantics of BufferedInputStream
   * in that the number of bytes actually returned may be less that the
   * requested value. So callers of this routine should be aware of this and
   * must check the return value to insure that they have obtained the requisite
   * number of bytes.
   */
  ...
  /**
   * Normally the SEEN flag will have been set by now, but if not, force it to
   * be set (as long as the folder isn't open read-only and we're not peeking).
   * And of course, if there's no folder (e.g., a nested message) don't do
   * anything.
   */
  private void checkSeen(){
    ...
    try{
      ...
      if(f != null && f.getMode() != Folder.READ_ONLY &&
          !msg.isSet(Flags.Flag.SEEN)) msg.setFlag(Flags.Flag.SEEN, true);
    }
