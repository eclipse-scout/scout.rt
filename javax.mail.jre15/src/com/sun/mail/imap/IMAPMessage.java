 ...
 */
/*
 * The lock hierarchy is that the lock on the IMAPMessage object, if it's
 * acquired at all, must be acquired before the message cache lock. The
 * IMAPMessage lock protects the message flags, sort of. XXX - I'm not convinced
 * that all fields of IMAPMessage are properly protected by locks.
 */
...
public class IMAPMessage extends MimeMessage{
  ...
  /*
   * Hashtable of names of headers we've loaded from the server. Used in
   * isHeaderLoaded() and setHeaderLoaded() to keep track of those headers we've
   * attempted to load from the server. We need this table of names to avoid
   * multiple attempts at loading headers that don't exist for a particular
   * message. Could this somehow be included in the InternetHeaders object ??
   */
  ...
  /**
   * Get this message's folder's protocol connection. Throws
   * FolderClosedException, if the protocol connection is not available. ASSERT:
   * Must hold the messageCacheLock.
   */
  protected IMAPProtocol getProtocol()
    ...
    if(p == null) throw new FolderClosedException(folder);
    else return p;
  }
  ...
  /**
   * Get this message's IMAP sequence number. ASSERT: This method must be called
  ...
  /**
   * Set this message's IMAP sequence number. ASSERT: This method must be called
   * only when holding the messageCacheLock.
   */
  ...
  /**
   * Wrapper around the protected method Message.setMessageNumber() to make that
  ...
  /**
   * Do a NOOP to force any untagged EXPUNGE responses and then check if this
   * message is expunged.
   */
  ...
  public Address getSender() throws MessagingException{
    checkExpunged();
    loadEnvelope();
    if(envelope.sender != null) return (envelope.sender)[0]; // there can be
    // only one sender
    else return null;
  }
  ...
  public Address[] getRecipients(Message.RecipientType type)
    ...
    if(type == Message.RecipientType.TO) return aaclone(envelope.to);
    else if(type == Message.RecipientType.CC) return aaclone(envelope.cc);
    else if(type == Message.RecipientType.BCC) return aaclone(envelope.bcc);
    else return super.getRecipients(type);
  }
  ...
  public Date getSentDate() throws MessagingException{
    checkExpunged();
    loadEnvelope();
    if(envelope.date == null) return null;
    else return new Date(envelope.date.getTime());
  }
  ...
  public Date getReceivedDate() throws MessagingException{
    ...
    else return new Date(receivedDate.getTime());
  }
  ...
  public int getSize() throws MessagingException{
    checkExpunged();
    if(size == -1) loadEnvelope(); // XXX - could just fetch the size
    return size;
  }
  ...
  public String[] getContentLanguage() throws MessagingException{
    checkExpunged();
    loadBODYSTRUCTURE();
    if(bs.language != null) return (String[])(bs.language).clone();
    else return null;
  }
  ...
  /**
   * Get the Content-Type. Generate this header from the BODYSTRUCTURE. Append
   * parameters as well.
   */
  ...
  /**
   * Get the "filename" Disposition parameter. (Only available in IMAP4rev1). If
   * thats not available, get the "name" ContentType parameter.
   */
  ...
  /**
   * Get all the bytes for this message. Overrides getContentStream() in
   * MimeMessage. This method is ultimately used by the DataHandler to obtain
   * the input stream for this message.
   * 
  ...
  protected InputStream getContentStream() throws MessagingException{
    ...
    synchronized(getMessageCacheLock()){
      try{
        ...
        if(p.isREV1()){
          BODY b;
          if(pk) b=p.peekBody(getSequenceNumber(), toSection("TEXT"));
          else b=p.fetchBody(getSequenceNumber(), toSection("TEXT"));
  ...
  public synchronized DataHandler getDataHandler()
    ...
    if(dh == null){
      ...
      /*
       * Special-case Multipart and Nested content. All other cases are handled
       * by the superclass.
       */
      if(bs.isMulti()) dh=new DataHandler(
          new IMAPMultipartDataSource(this, bs.bodies,
          sectionId, this)
          );
      else if(bs.isNested() && isREV1())
      /*
       * Nested messages are handled specially only for IMAP4rev1. IMAP4 doesn't
       * provide enough support to FETCH the components of nested messages
       */
      dh=new DataHandler(
  ...
  public void writeTo(OutputStream os)
    ...
    synchronized(getMessageCacheLock()){
      try{
        ...
        if(p.isREV1()){
          BODY b;
          if(pk) b=p.peekBody(getSequenceNumber(), sectionId);
          else b=p.fetchBody(getSequenceNumber(), sectionId);
          if(b != null) is=b.getByteArrayInputStream();
        }
        else{
  ...
  /**
   * Set whether or not to use the PEEK variant of FETCH when fetching message
   * content.
   * 
   * @since JavaMail 1.3.3
   */
  ...
  /**
   * Get whether or not to use the PEEK variant of FETCH when fetching message
   * content.
   * 
   * @since JavaMail 1.3.3
   */
   ...
   * Subsequent accesses of this information will cause it to be fetched from
   * the server.
   * 
   * @since JavaMail 1.3.3
   */
  ...
  static void fetch(IMAPFolder folder, Message[] msgs,
     ...
     * The test is to check whether the message has already cached all the items
     * requested in the FetchProfile. If any item is missing, the test succeeds
     * and breaks out.
     */
    class FetchProfileCondition implements Utility.Condition{
      ...
      public boolean test(IMAPMessage m){
        if(needEnvelope && m._getEnvelope() == null) return true; // no envelope
        if(needFlags && m._getFlags() == null) return true; // no flags
        if(needBodyStructure && m._getBodyStructure() == null) return true; // no
        // BODYSTRUCTURE
        ...
        for(int i=0; i < hdrs.length; i++){
          if(!m.isHeaderLoaded(hdrs[i])) return true; // Nope, return
        }
    ...
    if(fp.contains(IMAPFolder.FetchProfileItem.HEADERS)){
      allHeaders=true;
      if(folder.protocol.isREV1()) command.append(first ?
          "BODY.PEEK[HEADER]" : " BODY.PEEK[HEADER]");
      else command.append(first ? "RFC822.HEADER" : " RFC822.HEADER");
      first=false;
    }
      ...
      for(int i=0; i < r.length; i++){
        ...
        for(int j=0; j < count; j++){
          ...
          if(item instanceof Flags){
            ...
            else msg.flags=(Flags)item;
          }

          // Check for ENVELOPE items
          else if(item instanceof ENVELOPE) msg.envelope=(ENVELOPE)item;
          else if(item instanceof INTERNALDATE) msg.receivedDate=((INTERNALDATE)item).getDate();
          else if(item instanceof RFC822SIZE) msg.size=((RFC822SIZE)item).size;

          // Check for the BODYSTRUCTURE item
          else if(item instanceof BODYSTRUCTURE) msg.bs=(BODYSTRUCTURE)item;
          ...
          // Check for header items
            ...
            h.load(headerStream);
            if(msg.headers == null || allHeaders) msg.headers=h;
            else{
              /*
               * This is really painful. A second fetch of the same headers
               * (which might occur because a new header was added to the set
               * requested) will return headers we already know about. In this
               * case, only load the headers we haven't seen before to avoid
               * adding duplicates of headers we already have.
               */
              Enumeration e=h.getAllHeaders();
              while(e.hasMoreElements()){
                Header he=(Header)e.nextElement();
                if(!msg.isHeaderLoaded(he.getName())) msg.headers.addHeader(
                    he.getName(), he.getValue());
              }
            }

            // if we asked for all headers, assume we got them
            if(allHeaders) msg.setHeadersLoaded(true);
            else{
      ...
      try{
        ...
        for(int i=0; i < r.length; i++){
          ...
          if(r[i] == null ||
              ...
              ((FetchResponse)r[i]).getNumber() != seqnum) continue;
          ...
          for(int j=0; j < count; j++){
            ...
            if(item instanceof ENVELOPE) envelope=(ENVELOPE)item;
            else if(item instanceof INTERNALDATE) receivedDate=((INTERNALDATE)item).getDate();
            else if(item instanceof RFC822SIZE) size=((RFC822SIZE)item).size;
          }
        }
  ...
  private static String craftHeaderCmd(IMAPProtocol p, String[] hdrs){
    ...
    if(p.isREV1()) sb=new StringBuffer("BODY.PEEK[HEADER.FIELDS (");
    else sb=new StringBuffer("RFC822.HEADER.LINES (");
  ...
  /*
   * Convert the given FETCH item identifier to the approriate section-string
   * for this message.
   */
  private String toSection(String what){
    if(sectionId == null) return what;
    else return sectionId + "." + what;
  }
  ...
  private InternetAddress[] aaclone(InternetAddress[] aa){
    ...
    else return (InternetAddress[])aa.clone();
  }
  ...
  /***********************************************************
   * accessor routines to make available certain private/protected fields to
   * other classes in this package.
   ***********************************************************/

  /*
   * Called by IMAPFolder. Must not be synchronized.
   */
