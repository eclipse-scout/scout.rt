...
public class message_deliverystatus implements DataContentHandler{
  ...
  public Object getTransferData(DataFlavor df, DataSource ds)
    ...
    if(ourDataFlavor.equals(df)) return getContent(ds);
    else return null;
  }
  ...
  public Object getContent(DataSource ds) throws IOException{
    ...
    try{
      /*
       * Session session; if (ds instanceof MessageAware) {
       * javax.mail.MessageContext mc = ((MessageAware)ds).getMessageContext();
       * session = mc.getSession(); } else { // Hopefully a rare case. Also
       * hopefully the application // has created a default Session that can
       * just be returned // here. If not, the one we create here is better than
       * // nothing, but overall not a really good answer. session =
       * Session.getDefaultInstance(new Properties(), null); } return new
       * DeliveryStatus(session, ds.getInputStream());
       */
