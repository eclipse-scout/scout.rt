...
/**
 * A message/delivery-status message content, as defined in <A
 * HREF="http://www.ietf.org/rfc/rfc3464.txt">RFC 3464</A>.
 */
public class DeliveryStatus{
  ...
  /**
   * Construct a delivery status notification by parsing the supplied input
   * stream.
   */
  public DeliveryStatus(InputStream is)
      throws MessagingException, IOException {
    messageDSN=new InternetHeaders(is);
    if(debug) System.out.println("DSN: got messageDSN");
    Vector v=new Vector();
    try{
      while(is.available() > 0){
        InternetHeaders h=new InternetHeaders(is);
        if(debug) System.out.println("DSN: got recipientDSN");
        v.addElement(h);
      }
    }
    catch(EOFException ex){
      if(debug) System.out.println("DSN: got EOFException");
    }
    if(debug) System.out.println("DSN: recipientDSN size " + v.size());
    recipientDSN=new InternetHeaders[v.size()];
    v.copyInto(recipientDSN);
  }

  /**
   * Return all the per-message fields in the delivery status notification. The
  ...
  /**
   * Return the number of recipients for which we have per-recipient delivery
  ...
  /**
   * Return the delivery status notification information for the specified
   * recipient.
   */
  ...
  private static void writeInternetHeaders(InternetHeaders h,
    ...
    try{
      ...
      else throw new IOException("Exception writing headers: " + mex);
    }
  }
