...
public class SocketFetcher{
  ...
  public static Socket getSocket(String host, int port, Properties props,
    ...
    if(prefix == null) prefix="socket";
    if(props == null) props=new Properties(); // empty
    ...
    if(localaddrstr != null) localaddr=InetAddress.getByName(localaddrstr);
    ...
    if(socket == null) socket=createSocket(localaddr, localport,
    ...
    if(to >= 0) socket.setSoTimeout(to);
  ...
  private static Socket createSocket(InetAddress localaddr, int localport,
    ...
    if(sf != null) socket=sf.createSocket();
    else if(useSSL) socket=SSLSocketFactory.getDefault().createSocket();
    else socket=new Socket();
    if(localaddr != null) socket.bind(new InetSocketAddress(localaddr, localport));
    if(cto >= 0) socket.connect(new InetSocketAddress(host, port), cto);
  ...
  private static SocketFactory getSocketFactory(String sfClass)
    ...
    if(sfClass == null || sfClass.length() == 0) return null;
    ...
    if(clsSockFact == null) clsSockFact=Class.forName(sfClass);
  ...
  public static Socket startTLS(Socket socket, Properties props,
    ...
    try{
      ...
      String sfClass=
          props.getProperty(prefix + ".socketFactory.class", null);
      SocketFactory sf=getSocketFactory(sfClass);
      if(sf != null && sf instanceof SSLSocketFactory) ssf=(SSLSocketFactory)sf;
      else ssf=(SSLSocketFactory)SSLSocketFactory.getDefault();
  ...
  private static void configureSSLSocket(Socket socket, Properties props,
    ...
    if(protocols != null) sslsocket.setEnabledProtocols(stringArray(protocols));
    else{
      /*
       * At least the UW IMAP server insists on only the TLSv1 protocol for
       * STARTTLS, and won't accept the old SSLv2 or SSLv3 protocols. Here we
       * enable only the TLSv1 protocol. XXX - this should probably be
       * parameterized.
       */
      sslsocket.setEnabledProtocols(new String[]{"TLSv1"});
    }
    String ciphers=props.getProperty(prefix + ".ssl.ciphersuites", null);
    if(ciphers != null) sslsocket.setEnabledCipherSuites(stringArray(ciphers));
    /*
