...
public class BASE64DecoderStream extends FilterInputStream{
  ...
  /**
   * Create a BASE64 decoder that decodes the specified input stream. The System
   * property <code>mail.mime.base64.ignoreerrors</code> controls whether errors
   * in the encoded data cause an exception or are ignored. The default is false
  ...
  public int read(byte[] buf, int off, int len) throws IOException{
    ...
    if(index >= bufsize) bufsize=index=0;
   ...
   * cases. Returns the number of bytes decoded, which may be zero. Decoding is
   * done by filling an int with 4 6-bit values by shifting them in from the
   * bottom and then extracting 3 8-bit bytes from the int by shifting them out
   * from the bottom.
   * 
   * @param outbuf
   ...
   * @return the number of bytes filled, always a multiple of three, and may be
   *         zero
   * @exception IOException
    ...
    while(len >= 3){
      ...
      while(got < 4){
        ...
        if(i == -1 || i == -2){
          boolean atEOF;
          if(i == -1){
            if(got == 0) return pos - pos0;
            if(!ignoreErrors) throw new IOException("Error in encoded stream: " +
            ...
            if(got < 2 && !ignoreErrors) throw new IOException("Error in encoded stream: " +
          ...
          while(got < 4){
            if(!atEOF){
              ...
              i=getByte();
              if(i == -1){
                if(!ignoreErrors) throw new IOException(
          ...
          if(size == 2) outbuf[pos + 1]=(byte)(val & 0xff);
          val>>=8;
   ...
   * 
   * @return the next byte, -1 on EOF, or -2 if next byte is '=' (padding at end
   *         of encoded data)
   */
   ...
   * protocol, but not to decode the entire content of a MIME part. NOTE: inbuf
   * may only contain valid base64 characters. Whitespace is not ignored.
   */
  public static byte[] decode(byte[] inbuf){
    ...
    while(size > 0){
      ...
      if(osize > 2) outbuf[outpos + 2]=(byte)(val & 0xff);
      val>>=8;
      if(osize > 1) outbuf[outpos + 1]=(byte)(val & 0xff);
      val>>=8;
   ...
   * BASE64DecoderStream decoder = new BASE64DecoderStream(infile); int c; while
   * ((c = decoder.read()) != -1) System.out.print((char)c); System.out.flush();
   * } end TEST program
   ***/
}
