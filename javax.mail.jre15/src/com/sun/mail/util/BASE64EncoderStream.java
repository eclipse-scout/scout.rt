...
public class BASE64EncoderStream extends FilterOutputStream{
   ...
   *          Integer.MAX_VALUE, in which case no CRLF is inserted. bytesPerLine
  ...
  /**
   * Forces any buffered output bytes to be encoded out to the stream and closes
   * this output stream
   */
  ...
  /**
   * Encode the data stored in <code>buffer</code>. Uses <code>outbuf</code> to
   * store the encoded data before writing.
   * 
   * @exception IOException
   *              if an I/O error occurs.
   */
   ...
   * input buffer to encode. If outbuf is non-null, it's used as is. Otherwise,
   * a new output buffer is allocated.
   */
  private static byte[] encode(byte[] inbuf, int off, int size,
      byte[] outbuf){
    if(outbuf == null) outbuf=new byte[encodedSize(size)];
    int inpos, outpos;
    int val;
   ...
   * while ((c = infile.read()) != -1) encoder.write(c); encoder.close(); } end
   * TEST program
   **/
}
