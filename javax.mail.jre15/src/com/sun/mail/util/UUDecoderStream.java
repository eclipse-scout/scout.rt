...
public class UUDecoderStream extends FilterInputStream{
  ...
  /**
   * UUencoded streams start off with the line: "begin <mode> <filename>" Search
   * for this prefix and gobble it up.
   */
  private void readPrefix() throws IOException{
    ...
    for(;;){
      ...
      if(s == null) throw new IOException("UUDecoder error: No Begin");
  ...
  private boolean decode() throws IOException{
    ...
    do{
       ...
       * starts with a space character, we detect the following "end" line here.
       */
    ...
    /*
     * The first character in a line is the number of original (not the encoded
     * atoms) characters in the line. Note that all the code below has to handle
     * the <SPACE> character that indicates end of encoded stream.
     */
    count=(count - ' ') & 0x3f;
    ...
    byte a, b;
    /*
     * A correct uuencoder always encodes 3 characters at a time, even if there
     * aren't 3 characters left. But since some people out there have broken
     * uuencoders we handle the case where they don't include these
   ...
   * UUDecoderStream decoder = new UUDecoderStream(infile); int c; try { while
   * ((c = decoder.read()) != -1) System.out.write(c); System.out.flush(); }
   * catch (Exception e) { e.printStackTrace(); } } end TEST program
   ****/
}
