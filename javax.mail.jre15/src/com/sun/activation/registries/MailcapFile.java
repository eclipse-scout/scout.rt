...
public class MailcapFile{
  ...
  /**
   * A Map indexed by MIME type (string) that references a Map of commands for
   * each type. The comand Map is indexed by the command name and references a
   * List of class names (strings) for each command.
   */
  ...
  /**
   * A Map indexed by MIME type (string) that references a List of native
   * commands (string) corresponding to the type.
   */
   ...
   * 
   * @param new_fname
   *          The file name of the mailcap file.
   */
  public MailcapFile(String new_fname) throws IOException {
    if(LogSupport.isLoggable()) LogSupport.log("new MailcapFile: file " + new_fname);
  ...
  public MailcapFile(InputStream is) throws IOException {
    if(LogSupport.isLoggable()) LogSupport.log("new MailcapFile: InputStream");
   ...
   */
  public MailcapFile() {
    if(LogSupport.isLoggable()) LogSupport.log("new MailcapFile: default");
  }

  /**
   * Get the Map of MailcapEntries based on the MIME type.
   * <p>
   * <strong>Semantics:</strong> First check for the literal mime type, if that
   * fails looks for wildcard <type>/\* and return that. Return the list of all
   * that hit.
   */
  ...
  /**
   * Merge the first hash into the second. This merge will only effect the
   * hashtable that is returned, we don't want to touch the one passed in since
  ...
  /**
   * appendToMailcap: Append to this Mailcap DB, use the mailcap format: Comment
   * == "# <i>comment string</i> Entry == "mimetype; javabeanclass
   * <nl>
   * Example: # this is a comment image/gif jaf.viewers.ImageViewer
   */
  public void appendToMailcap(String mail_cap){
    if(LogSupport.isLoggable()) LogSupport.log("appendToMailcap: " + mail_cap);
    try{
    ...
    while((line=buf_reader.readLine()) != null){
      ...
      try{
        ...
        if(line.charAt(line.length() - 1) == '\\'){
          if(continued != null) continued+=line.substring(0, line.length() - 1);
          else continued=line.substring(0, line.length() - 1);
        }
  ...
  /**
   * A routine to parse individual entries in a Mailcap file. Note that this
   * routine does not handle line continuations. They should have been handled
  ...
  protected void parseLine(String mailcapEntry)
    ...
    if(LogSupport.isLoggable()) LogSupport.log("parse: " + mailcapEntry);
    ...
    if(LogSupport.isLoggable()) LogSupport.log("  Type: " + mimeType);
    ...
    if(currentToken == MailcapTokenizer.SEMICOLON_TOKEN){
        ...
        if(currentToken == MailcapTokenizer.EQUALS_TOKEN){
          ...
          if(paramName.startsWith("x-java-")){
            ...
            if(commandName.equals("fallback-entry") &&
              ...
              if(LogSupport.isLoggable()) LogSupport.log("    Command: " + commandName +
                  ", Class: " + paramValue);
              ...
              if(addReverse) classes.add(0, paramValue);
              else classes.add(paramValue);
            }
          }
      ...
      if(curcommands == null){
        ...
        if(LogSupport.isLoggable()) LogSupport.log("Merging commands for type " + mimeType);
        ...
        while(cn.hasNext()){
          ...
          while(cvn.hasNext()){
            ...
            if(!ccv.contains(clazz)) if(addReverse) ccv.add(0, clazz);
            else ccv.add(clazz);
          }
        }
  ...
  protected static void reportParseError(int expectedToken,
      ...
      String actualTokenValue) throws MailcapParseException{
    if(LogSupport.isLoggable()) LogSupport.log("PARSE ERROR: " + "Encountered a " +
        MailcapTokenizer.nameForToken(actualToken) + " token (" +
        actualTokenValue + ") while expecting a " +
  ...
  /**
   * for debugging public static void main(String[] args) throws Exception { Map
   * masterHash = new HashMap(); for (int i = 0; i < args.length; ++i) {
   * System.out.println("Entry " + i + ": " + args[i]); parseLine(args[i],
   * masterHash); } Enumeration types = masterHash.keys(); while
   * (types.hasMoreElements()) { String key = (String)types.nextElement();
   * System.out.println("MIME Type: " + key); Map commandHash =
   * (Map)masterHash.get(key); Enumeration commands = commandHash.keys(); while
   * (commands.hasMoreElements()) { String command =
   * (String)commands.nextElement(); System.out.println("    Command: " +
   * command); Vector classes = (Vector)commandHash.get(command); for (int i =
   * 0; i < classes.size(); ++i) { System.out.println("        Class: " +
   * (String)classes.elementAt(i)); } } System.out.println(""); } }
   */
}
