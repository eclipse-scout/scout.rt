...
/**
 * A tokenizer for strings in the form of "foo/bar; prop1=val1; ... ". Useful
 * for parsing MIME content types.
 */
public class MailcapTokenizer{
  ...
  /**
   * Set whether auto-quoting is on or off. Auto-quoting means that all
   * characters after the first non-whitespace, non-control character up to the
   * auto-quote terminator character or EOI (minus any whitespace immediatley
   * preceeding it) is considered a token. This is required for handling command
   * strings in a mailcap entry.
   */
   ...
   * ++i) { MailcapTokenizer tokenizer = new MailcapTokenizer(args[i]);
   * System.out.println("Original: |" + args[i] + "|"); int currentToken =
   * tokenizer.nextToken(); while (currentToken != EOI_TOKEN) {
   * switch(currentToken) { case UNKNOWN_TOKEN:
   * System.out.println("  Unknown Token:           |" +
   * tokenizer.getCurrentTokenValue() + "|"); break; case START_TOKEN:
   * System.out.println("  Start Token:             |" +
   * tokenizer.getCurrentTokenValue() + "|"); break; case STRING_TOKEN:
   * System.out.println("  String Token:            |" +
   * tokenizer.getCurrentTokenValue() + "|"); break; case EOI_TOKEN:
   * System.out.println("  EOI Token:               |" +
   * tokenizer.getCurrentTokenValue() + "|"); break; case SLASH_TOKEN:
   * System.out.println("  Slash Token:             |" +
   * tokenizer.getCurrentTokenValue() + "|"); break; case SEMICOLON_TOKEN:
   * System.out.println("  Semicolon Token:         |" +
   * tokenizer.getCurrentTokenValue() + "|"); break; case EQUALS_TOKEN:
   * System.out.println("  Equals Token:            |" +
   * tokenizer.getCurrentTokenValue() + "|"); break; default:
   * System.out.println("  Really Unknown Token:    |" +
   * tokenizer.getCurrentTokenValue() + "|"); break; } currentToken =
   * tokenizer.nextToken(); } System.out.println(""); } }
   */
}
