...
public class ExceptionTranslatorTest {
  ...
  @Test
  public void testInvocationCallableException() {
    ProcessingException cause1 = new ProcessingException();
    ProcessingException pe1 = ExceptionTranslator.translate(new ContextInvocationException(cause1));
    assertSame(cause1, pe1);

    RuntimeException cause2 = new RuntimeException();
    ProcessingException pe2 = ExceptionTranslator.translate(new ContextInvocationException(cause2));
    assertSame(cause2, pe2.getCause());

    Exception cause3 = new Exception();
    ProcessingException pe3 = ExceptionTranslator.translate(new ContextInvocationException(cause3));
    assertSame(cause3, pe3.getCause());
  }
  ...
  public void testCallableException() throws Exception {
    ...
    // AssertionException
    AssertionException ae = new AssertionException("assertion error");
    doThrow(ae).when(next).call();
