...
public class DeferredOperationQueueTest {
  ...
  private <T> void testFlushDeferredQueue(int batchSize, long maxDelayMillis, boolean singleRun, Consumer<DeferredOperationQueue<T>> addElements, List<List<T>> expectedBatches) {
    var queue = new FixtureDeferredOperationQueue<T>(QUEUE_TRANSACTION_MEMBER_ID, batchSize, maxDelayMillis, b -> {});
  ...
  public void testTransactionalBatchOperation() throws Exception {
    ...
    var flushedLatch = new CountDownLatch(1);

    var queue = new DeferredOperationQueue<String>(QUEUE_TRANSACTION_MEMBER_ID, 2, 1000, batch -> getOrCreateTransactionMember(batches).addAll(batch)) {
      @Override
      protected void flushDeferred(boolean singleRun) {
  ...
  protected P_FixtureTransactionMember getOrCreateTransactionMember(List<List<String>> batches) {
    var transaction = assertNotNull(ITransaction.CURRENT.get(), "Not running within a transaction");
    var transactionMember = (P_FixtureTransactionMember) transaction.getMember(P_FixtureTransactionMember.ID);
