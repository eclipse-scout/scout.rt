...
public class DefaultValueMapTest {
  ...
  public void testEntrySet() {
    ...
    assertThrows(IllegalStateException.class, emptyMapEntriesIterator::remove);
    assertTrue(emptyMap.isEmpty());
  ...
  @Test
  public void testEntrySetEntryMethods() {
    DefaultValueMap<String, Integer> emptyMap = new DefaultValueMap<>(Map.of("A", 1), true);
    Set<Entry<String, Integer>> entries = emptyMap.entrySet();
    assertTrue(entries.isEmpty());

    emptyMap.put("A", 1);
    assertEquals(1, entries.size()); // changes are reflected in the live-set

    emptyMap.put("B", 2);
    assertEquals(2, entries.size()); // changes are reflected in the live-set

    Iterator<Entry<String, Integer>> iterator = entries.iterator();
    while (iterator.hasNext()) {
      Entry<String, Integer> theEntry = iterator.next();
      if ("A".equals(theEntry.getKey())) {
        assertEquals(1, (long) theEntry.getValue());
        assertEquals(1, (long) theEntry.setValue(3)); // change the entry
        assertEquals(3, (long) theEntry.getValue());
        assertEquals(3, (long) emptyMap.get("A"));
        assertEquals(3, (long) theEntry.setValue(1)); // change the entry (again)
        iterator.remove();
        assertFalse(emptyMap.containsKey("A"));
        theEntry.setValue(4); // behavior of setValue method is undefined if entry has been removed already by iterator remove method (by specification); our impl re-adds it in some cases
        assertTrue(emptyMap.containsKey("A"));
      }
    }
    ...
    while (iterator.hasNext()) {
      Entry<String, Integer> theEntry = iterator.next();
      if ("B".equals(theEntry.getKey())) {
        assertEquals(2, (long) theEntry.getValue());
        assertEquals(2, (long) theEntry.setValue(3)); // change the entry
        assertEquals(3, (long) theEntry.getValue());
        assertEquals(3, (long) emptyMap.get("B"));
        assertEquals(3, (long) theEntry.setValue(1)); // change the entry (again)
        iterator.remove();
        assertFalse(emptyMap.containsKey("B"));
        theEntry.setValue(4); // behavior of setValue method is undefined if entry has been removed already by iterator remove method (by specification); our impl ignores call for some other cases
        assertFalse(emptyMap.containsKey("B"));
      }
    }
  }
