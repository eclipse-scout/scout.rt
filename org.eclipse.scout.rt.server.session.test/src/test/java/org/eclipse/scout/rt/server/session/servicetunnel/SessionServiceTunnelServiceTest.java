...
/**
 * Test for {@link SessionServiceTunnelService}
 */
@RunWith(ServerTestRunner.class)
@RunWithServerSession(TestServerSession.class)
@RunWithSubject("default")
public class SessionServiceTunnelServiceTest {
  ...
  public void before() {
    ...
    when(m_requestMock.getSession()).thenReturn(testHttpSession);
    when(m_requestMock.getSession(true)).thenReturn(testHttpSession);
  ...
  public void testNewSessionCreatedOnLookupHttpSession() {
    createServletRunContext(m_requestMock, m_responseMock).run(() -> {
      ...
      IServerSession session = BEANS.get(HttpServerSessionRunContextProducer.class).getOrCreateScoutSession(m_requestMock, runContext, "testid");
  ...
  public void testNoNewServerSessionOnLookup() {
    RunContexts
        .empty()
        .run(() -> {
          IServerSession session1 = BEANS.get(HttpServerSessionRunContextProducer.class).getOrCreateScoutSession(m_requestMock, ServerRunContexts.empty(), "id1");
          IServerSession session2 = BEANS.get(HttpServerSessionRunContextProducer.class).getOrCreateScoutSession(m_requestMock, ServerRunContexts.empty(), "id1");
  ...
  /**
   * Calls {@link HttpServerSessionRunContextProducer#getOrCreateScoutSession(HttpServletRequest, ServerRunContext, String) in 4
  ...
  private static class HttpSessionLookupCallable implements Callable<IServerSession> {
    ...
    public IServerSession call() {
      return createServletRunContext(m_request, m_response).call(() -> BEANS.get(HttpServerSessionRunContextProducer.class)
