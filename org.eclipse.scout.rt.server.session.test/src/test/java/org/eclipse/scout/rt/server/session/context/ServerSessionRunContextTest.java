...
public class ServerSessionRunContextTest {
  ...
  public void testCopy() {
    ...
    ServerSessionRunContext copy = runContext.copy();
  ...
  public void testRunContextsEmpty() {
    ...
    ServerSessionRunContext serverCtx = (ServerSessionRunContext) runContext;
  ...
  public void testCurrentTransactionScope() {
    RunContexts.empty().run(() -> assertEquals(TransactionScope.REQUIRES_NEW, ServerSessionRunContexts.copyCurrent().getTransactionScope()));
    RunContexts.empty().withTransactionScope(TransactionScope.REQUIRED).run(() -> assertEquals(TransactionScope.REQUIRES_NEW, ServerSessionRunContexts.copyCurrent().getTransactionScope()));
  }
  ...
  public void testCopyCurrentOrElseEmpty() {
    Jobs.schedule(() -> {
          ...
          try {
            ServerSessionRunContexts.copyCurrent(false);
            fail("AssertionException expected because not running in a RunContext");
          }
          catch (AssertionException e) {
            // expected
          ...
          assertNotNull(ServerSessionRunContexts.copyCurrent(true));
        }, Jobs.newInput())
        .awaitDoneAndGet();
  }
