...
import org.eclipse.scout.rt.testing.server.session.runner.statement.ServerSessionRunContextStatement;
 ...
 */
public class ServerSessionTestRunner extends ServerTestRunner {
  ...
  protected Statement interceptClassLevelStatement(Statement next, Class<?> testClass) {
    final Statement s2 = new ServerSessionRunContextStatement(next,  ReflectionUtility.getAnnotation(RunWithServerSession.class, testClass));
  ...
  protected Statement interceptMethodLevelStatement(final Statement next, final Class<?> testClass, final Method testMethod) {
    final Statement s2 = new ServerSessionRunContextStatement(next,  ReflectionUtility.getAnnotation(RunWithServerSession.class, testMethod, testClass));
