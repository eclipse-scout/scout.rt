...
public final class SVGUtility {
  ...
  /**
   * @deprecated Using this method when attaching a GVTTree could lead to memory leaks. Use
   *             {@link #readSVGDocumentForGraphicalModification(InputStream)} or {@link #readSVGDocument(InputStream)}
   *             instead. <b>This method will be removed in the next release.</b>
   * @param in
   * @param attachGVTTree
  ...
  public static SVGDocument readSVGDocument(InputStream in, boolean attachGVTTree) throws ProcessingException {
    if (attachGVTTree) {
      return (SVGDocument) readSVGDocumentForGraphicalModification(in).getDocument();
    }
    else {
      return readSVGDocument(in);
    }
  }
