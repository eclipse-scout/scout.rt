...
scout.Graph.prototype._init = function(options) {
  scout.Graph.parent.prototype._init.call(this, options);

  $.extend(this, options);
  this.initGraph();
};

scout.Graph.prototype.initGraph = function() {
...
scout.Graph.prototype._renderInternal = function($parent) {
  var i, node;

  this.$container = $parent.appendSVG('svg', 'graph');

  // Install layout to update graph when parent size changes
  this.htmlComp = new scout.HtmlComponent($parent, this.session);
...
scout.Graph.prototype.adjustTemperature = function(k) {
  k = +k;
  if (k === 0) {
...
scout.Graph.prototype.start = function() {
  this.adjustTemperature(0.9);
};

scout.Graph.prototype.resume = function() {
  this.adjustTemperature(0.3);
};

//executes one simulation step
scout.Graph.prototype.step = function(draw, recurse) {
...
scout.Graph.prototype.setLabel = function(link) {
  ...
  link.$divText.attr('transform', 'rotate(' + angle + ', ' + ((x1 + x2) / 2) + ', ' + ((y1 + y2) / 2) + ')');
};

// dissolve crossing links
scout.Graph.prototype.disolveLinks = function() {
...
// move node by mouse
scout.Graph.prototype.moveNode = function(event) {
...
scout.Graph.prototype.lockNode = function(node) {
  if (node.userLocked) {
...
scout.Graph.prototype._remove = function() {
  this.stopAnimation();
  this._resetGraph();
  scout.Graph.parent.prototype._remove.call(this);
};

scout.Graph.prototype._resetGraph = function() {
  var i, n1;
  for (i = 0; i < this.nodes.length; i++) {
    n1 = this.nodes[i];
    n1.locked = false;
...
scout.Graph.prototype.stopAnimation = function() {
  this.adjustTemperature(0);
};

scout.Graph.prototype.ensureInside = function(node) {
  var actualX, actualY;
...
scout.Graph.prototype.onResize = function() {
  this.wContainer = this.$container.width();
  this.hContainer = this.$container.height();
  this.resume();
};
...
============================================================
  Chris' prototype (for reference purposes)
  TODO BSH Delete when not used anymore.
============================================================

scout.Graph = function(graphModel) {
  scout.Graph.parent.call(this);
  $.extend(this, graphModel);
  this.cssClass = 'graph';
};
scout.inherits(scout.Graph, scout.Widget);
...
scout.Graph.prototype.doPhysics = function() {
  var totalDiff = 0;
  ...
  for (var n = 0; n < this.nodes.length; n++) {
    ...
    for (var l = 0; l < this.edges.length; l++) {
      ...
      if (link.source === node.id) {
        oppositeNode = this.nodes[link.target];
      } else if (link.target === node.id) {
        oppositeNode = this.nodes[link.source];
      }
...
scout.Graph.prototype._remove = function() {
  this.stopAnimation();
  scout.Graph.parent.prototype._remove();
};
...
scout.Graph.prototype.iterate = function() {
};
