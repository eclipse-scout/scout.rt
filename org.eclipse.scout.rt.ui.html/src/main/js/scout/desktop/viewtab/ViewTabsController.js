...
scout.ViewTabsController.prototype.createAndRenderViewTab = function(view, position) {
  ...
  if (newViewTab) {
    ...
    viewTab.on('remove', this._removeViewTab.bind(this, viewTab));

    var index = position;
    var parentViewTab = this.viewTab(viewTab.view.displayParent);
    if (parentViewTab) {
  ...
  if (this._desktop.header && !viewTab.rendered) {
    viewTab.render(this._desktop.header.viewTabs.$container);
  }
 ...
 */
scout.ViewTabsController.prototype._removeViewTab = function(viewTab) {
  // Unregister the view tab
  var viewId = viewTab.view.id;
...
scout.ViewTabsController.prototype.deselectViewTab = function() {
  ...
  var view = this._selectedViewTab.view;
  if (view.rendered) {
    view.detach();
  }
...
scout.ViewTabsController.prototype._renderView = function(view) {
  if (view.rendered) {
  ...
  var viewArea = this._desktop.bench;
  view.render(this._desktop.bench.$container);
  view.setParent(this._desktop.bench);
  view.$container.addClass('view');
  view.validateRoot = true;
  view.invalidateLayoutTree(false);
  // Layout immediate to prevent 'laggy' form visualization,
  // but not initially while desktop gets rendered because it will be done at the end anyway
  if (this._desktop.rendered) {
    view.validateLayoutTree();
  }

  if (!view.detailForm) {
    // Notify model that this form is active (only for regular views, not detail forms)
    this._desktop._setFormActivated(view);
  }
};
