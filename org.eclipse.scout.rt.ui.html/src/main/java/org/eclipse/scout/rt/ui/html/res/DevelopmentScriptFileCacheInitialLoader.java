...
public class DevelopmentScriptFileCacheInitialLoader implements IPlatformListener {
  ...
  public void stateChanged(PlatformEvent event) {
    if (event.getState() == IPlatform.State.PlatformStarted) {
      ...
      try {
        ...
        for (HttpCacheKey key : m_cachedKeys) {
          cache.scheduleBuildScriptFile(key);
        }
      }
  ...
  protected synchronized Set<HttpCacheKey> laodCachedKeys() throws IOException, ClassNotFoundException {
    ...
    try {
      ...
      if (LOG.isInfoEnabled()) {
        ...
        boolean first = true;
        for (HttpCacheKey key : loadedFiles) {
          if (!first) {
            filesBuilder.append(", ");
          }
          filesBuilder.append(key.getResourcePath());
          first = false;
        }
        LOG.info("Load keys from persist cache in user.home '{}'.", filesBuilder.toString());
      }
      return loadedFiles;
    }
  ...
  public synchronized void storeInitialScriptfiles(Set<HttpCacheKey> keys) throws IOException {
    ...
    if (LOG.isInfoEnabled()) {
      ...
      boolean first = true;
      for (HttpCacheKey key : keys) {
        if (!first) {
          keyPathBuilder.append(", ");
        }
        keyPathBuilder.append(key.getResourcePath());
        first = false;
      }
      LOG.info("Persist cache keys for development script cache '{}'.", keyPathBuilder.toString());
    }
