...
/**
 * Servlet filter that registers and unregisters requests currently being executed so that they can be cancelled.
 * <p>
 * The {@link IAccessControlService#getUserId(Subject)} method is used to identify users, subclasses may overwrite the {@link #resolveUserId(HttpServletRequest)} method to provide a different identifier.
 */
public class RestRequestCancellationServletFilter implements Filter {

  private static final Logger LOG = LoggerFactory.getLogger(RestRequestCancellationServletFilter.class);
