...
public interface IChunkedDataWriter<T> extends Closeable {
  ...
  /**
   * Schedules a new job that executes the given iterator supplier and writes its values to the chunked response created with {@link #toEntity()}.
  ...
  default Response toResponse(Supplier<Iterator<T>> iteratorSupplier) {
    ...
    writeAsync(() -> {
      ...
      catch (@SuppressWarnings("squid:S1181") RuntimeException | Error e) {
