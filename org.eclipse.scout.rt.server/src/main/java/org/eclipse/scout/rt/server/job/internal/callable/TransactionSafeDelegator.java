...
class TransactionSafeDelegator {
  ...
  public boolean commitPhase1() {
    try {
      ...
      LOG.error(String.format("Failed to commit XA transaction [2PC-phase='voting', reason=%s, job=%s, tx=%s]", toReason(e), getJobIdentifier(), m_delegate), e);
      return false;
    }
  }
  ...
  public boolean commitPhase2() {
    try {
      ...
      LOG.error(String.format("Failed to commit XA transaction [2PC-phase='commit', reason=%s, job=%s, tx=%s]", toReason(e), getJobIdentifier(), m_delegate), e);
      return false;
    }
  }
  ...
  public boolean rollback() {
    try {
      ...
      LOG.error(String.format("Failed to rollback XA transaction [reason=%s, job=%s, tx=%s]", toReason(e), getJobIdentifier(), m_delegate), e);
      return false;
    }
  }
  ...
  public boolean release() {
    try {
      ...
      LOG.error(String.format("Failed to release XA transaction members [reason=%s, job=%s, tx=%s]", toReason(e), getJobIdentifier(), m_delegate), e);
      return false;
    }
  }
