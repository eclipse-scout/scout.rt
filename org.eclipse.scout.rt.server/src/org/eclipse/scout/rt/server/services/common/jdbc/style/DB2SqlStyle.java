    ...
    switch (bind.getSqlType()) {
      case Types.CLOB: {
        if (bind.getValue() instanceof Clob) {
          ...
          /*
           * Reflect:
           * CLOB newClob=CLOB.createTemporary(conn,false,oracle.sql.CLOB.DURATION_SESSION);
           * (a) newClob.putString(1,s);
           * (b) newClob.setString(1,s);
           */
          Clob clob;
          try {
            Class clobClass;
            clobClass = Class.forName("com.ibm.db2.app.CLOB", true, ps.getClass().getClassLoader());
      ...
      case Types.BLOB: {
        if (bind.getValue() instanceof Blob) {
          ...
          /*
           * Reflect:
           * BLOB newBlob=BLOB.createTemporary(conn,false,oracle.sql.BLOB.DURATION_SESSION);
           * (a) newBlob.putBytes(1,data);
           * (b) newBlob.setBytes(1,data);
           */
          Blob blob;
          try {
            Class blobClass = Class.forName("com.ibm.db2.app.BLOB", true, ps.getClass().getClassLoader());
