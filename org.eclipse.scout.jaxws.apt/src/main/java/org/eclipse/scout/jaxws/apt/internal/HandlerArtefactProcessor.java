 ...
 * @since 5.1
 */
public class HandlerArtefactProcessor {
  ...
  /**
   * Creates the following artefacts:
   * <ul>
   ...
   */
  public String generateHandlerArtefacts(final JClass portTypeProxy, final PortTypeProxyDescriptor descriptor, final ProcessingEnvironment env, final Logger logger) {
    final List<String> handlers = new ArrayList<>();

    // Generate AuthHandler
    if (descriptor.getAuthentication().enabled()) {
      try {
        logger.logInfo("Generating AuthHandler for '%s'", portTypeProxy.fullName());
        handlers.add(createAndPersistAuthHandler(portTypeProxy, descriptor, env));
      }
    ...
    for (int idx = 0; idx < handlerChain.size(); idx++) {
      ...
      try {
        logger.logInfo("Generating proxy handler for '%s'", handler.getQualifiedName());
        handlers.add(createAndPersistProxyHandler(portTypeProxy, descriptor, handler, idx, env));
      }
      catch (final Exception e) {
    ...
    try {
      logger.logInfo("Generating XML-file for handler-chain for '%s'", portTypeProxy.fullName());
      return createAndPersistHandlerXmlFile(portTypeProxy, descriptor, handlers, env.getFiler());
    }
    catch (final Exception e) {
  ...
  public String createAndPersistAuthHandler(final JClass portTypeProxy, final PortTypeProxyDescriptor descriptor, final ProcessingEnvironment processingEnv) throws Exception {
    ...
    final String fullName = StringUtility.join(".", portTypeProxy._package().name(), "handler", AUTH_HANDLER_NAME);
    ...
    final JClass descriptorClass = model.ref(descriptor.getDeclaringType().getQualifiedName().toString());
    defaultConstructor.body().invoke("super").arg(JExpr.dotclass(descriptorClass).invoke("getAnnotation").arg(JExpr.dotclass(model.ref(JaxWsPortTypeDecorator.class))).invoke("authentication"));

    AptUtil.addJavaDoc(authHandler, String.format("This class is auto-generated by APT triggered by Maven build and is based on the authentication configuration declared in {@link %s}.", descriptor.getDeclaringType().getSimpleName().toString()));
  ...
  public String createAndPersistProxyHandler(final JClass portTypeProxy, final PortTypeProxyDescriptor descriptor, final HandlerDescriptor handler, final int idx, final ProcessingEnvironment env) throws Exception {
    ...
    final String fullName = StringUtility.join(".", portTypeProxy._package().name(), "handler", handler.getSimpleName() + HANDLER_PROXY_SUFFIX);
    ...
    AptUtil.addJavaDoc(handlerProxy, String.format("This class is auto-generated by APT triggered by Maven build and is based on the handler configuration declared in {@link %s}.", descriptor.getDeclaringType().getSimpleName().toString()));
    ...
    defaultConstructor.body().invoke("super").arg(JExpr.dotclass(descriptorClass).invoke("getAnnotation").arg(JExpr.dotclass(model.ref(JaxWsPortTypeDecorator.class))).invoke("handlerChain").component(JExpr.lit(idx)));
  ...
  protected String createAndPersistHandlerXmlFile(final JClass portTypeProxy, final PortTypeProxyDescriptor decoration, final List<String> handlers, final Filer filer) throws Exception {
    ...
    out.println();
    out.printf("This file is auto-generated by APT triggered by Maven build, and is populated with handlers declared in '%s'. This file is referenced in @%s annotation in '%s'.<br/>", decoration.getDeclaringType().getSimpleName(), HandlerChain.class.getSimpleName(), portTypeProxy.name()).println();
    ...
    out.printf("This file is updated once any WSDL artefact or %s changes. If providing a handler chain binding file yourself, this file will not be generated.", decoration.getDeclaringType().getSimpleName()).println();
    out.println();
    ...
    final String fileName = HANDLERS_FILE_NAME;
