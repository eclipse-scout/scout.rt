...
public class MailHelper {
  ...
  protected void autoFixCharset(Part part) {
    ...
    try {
      ...
      String msgId = getMessageIdSafely(part);
      //noinspection PlaceholderCountMatchesArgumentCount
      LOG.info("Mail part '{}' uses an unsupported character set '{}'. Use UTF-8 as fallback.", msgId, charset, LOG.isDebugEnabled() ? e : null);
  ...
  protected void autoFixEncoding(MimePart part) {
    ...
    try {
      ...
      if (exMsg != null && exMsg.regionMatches(true, 0, identifier, 0, identifier.length())) {
        ...
        //noinspection PlaceholderCountMatchesArgumentCount
        LOG.info("Mail part '{}' uses an unsupported Content-Transfer-Encoding '{}' . Use default encoding as fallback.", msgId, encoding, LOG.isDebugEnabled() ? e : null);
        try {
  ...
  protected Charset guessPartCharset(Part part) {
    ...
    try (InputStream in = part.getInputStream()) {
      ...
      LOG.info("Part charset could not be detected for '{}'.", getMessageIdSafely(part), e);
    }
    return null;
  }
  ...
  public List<String> extractInReplyMessageIds(MimeMessage mimeMessage) {
    ...
    try {
      ...
      LOG.info("Could not parse headers for message with id: {}", getMessageIdSafely(mimeMessage), e1);
      return Collections.emptyList();
    }
    ...
    if (replyToHeaders == null || replyToHeaders.length == 0) {
      try {
        ...
        LOG.info("Unable to get third part of dsn-message for message with id: {}", getMessageIdSafely(mimeMessage), e);
        return Collections.emptyList();
      }
    }
   ...
   */
  public String getMessageIdSafely(Part mimeMessage) {
    if (mimeMessage == null) {
      return null;
    }
    ...
    try {
      ...
      LOG.info("Could not retrieve message id", e);
      return null;
    }
  }
  ...
  public String getAttachmentFilename(Part part, Function<String, String> defaultFilenameFunction) {
    ...
    try {
      ...
      LOG.info("Failed to get attachment filename", e);
      return null;
    }
  }
  ...
  protected String decodeAttachmentFilename(String filename) {
    ...
    try {
      ...
      LOG.info("Failed to clean attachment filename", e);
      return filename;
    }
  }
  ...
  protected String encodeAttachmentFilename(String filename, String charset) {
    ...
    try {
      ...
      LOG.info("Failed to encode attachment filename", e);
      return filename;
    }
  }
  ...
  protected String guessAttachmentFileExtension(String contentType) {
    ...
    try {
      ...
      LOG.info("Failed to parse content type '{}'", contentType, e);
      return null;
    }
