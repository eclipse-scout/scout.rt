...
export default class Session {
  ...
  constructor() {
    ...
    this.detachHelper = new DetachHelper(this);
    this.focusManager;
  ...
  getOrCreateWidget(adapterId, parent) {
    if (!adapterId) {
      return null;
    }
    if (typeof adapterId !== 'string') {
  ...
  createModelAdapter(adapterData) {
    ...
    if (objectInfo.modelVariant) {
      objectType = objectInfo.objectType.toString() + 'Adapter' + TypeDescriptor.MODEL_VARIANT_SEPARATOR + objectInfo.modelVariant.toString();
  ...
  _createSessionStartupParams() {
    var params = {
      ...
      geolocationServiceAvailable: false
    };
  ...
  _processStartupResponse(data) {
    ...
    // Register UI session
    this.modelAdapterRegistry[this.uiSessionId] = this;
  ...
  text(textKey) {
    return textKey; //scout.TextMap.prototype.get.apply(this.textMap, arguments);
  };
  ...
  textExists(textKey) {
    return false; //this.textMap.exists(textKey);
  };
