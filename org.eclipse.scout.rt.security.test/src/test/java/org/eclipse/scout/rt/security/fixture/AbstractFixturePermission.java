...
public abstract class AbstractFixturePermission extends AbstractPermission {
  ...
  protected boolean evalPermission(IPermission p) {
    return switch (getLevel().getValue()) {
      case PermissionLevel.LEVEL_NONE -> throw new AssertionError("Precondition violated: LEVEL NONE should be handled by #implies");
      case TestPermissionLevels.LEVEL_DENIED -> false;
      case PermissionLevel.LEVEL_ALL -> true;
      case TestPermissionLevels.LEVEL_GRANTED -> true;
      default -> super.evalPermission(p);
    };
  }
}
