...
public class LinearFillComponentElementFactory extends AbstractComponentElementFactory {
  ...
  @Override
  public Map<CalendarComponent, Element> create(Element container, Date elementDate, CalendarComponent[] components) {
    ...
    for (int i = 0; i < components.length; i++) {
      Element e = createComponentRectangle(container, containerDimension, components[i], i, components.length);
      ret.put(components[i], e);
    }
    return ret;
  }

  private Element createComponentRectangle(Element container, SvgRect containerDimension, CalendarComponent c, int index, int numComponents) {
    Element newEl = createNewComponentElement(container, c);
