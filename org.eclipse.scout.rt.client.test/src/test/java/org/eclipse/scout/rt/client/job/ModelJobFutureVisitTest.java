...
public class ModelJobFutureVisitTest {
  ...
  public void before() throws InterruptedException {
    ...
    bc1 = m_jobManager.createBlockingCondition("BC", true);
    bc2 = m_jobManager.createBlockingCondition("BC", true);
    ...
    }, ClientJobInput.empty().name("session2_job1").session(m_session2));
  ...
  public void testProtocol() throws JobExecutionException, InterruptedException {
    ...
    expected.add("session1_job1"); // blocked
    expected.add("session1_job2"); // waiting on the latch
//    expected.add("session1_job3"); // waiting for execution
    expected.add("session2_job1"); // completed
    expected.add("session2_job2"); // re-acquiring mutex
    expected.add("session2_job3"); // waiting on the latch
//    expected.add("session2_job4"); // waiting for execution
...
//    expected.add("session3_job1"); // waiting on the latch
  ...
  public void testVisitSessionFilterAndId() throws JobExecutionException, InterruptedException {
    final Set<String> visitedFutures = new HashSet<>();
    m_jobManager.visit(new AndFilter<IFuture<?>>(new ClientSessionFilter(m_session2), new JobFilter("ABC")), new IVisitor<IFuture<?>>() {
