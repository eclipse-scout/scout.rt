...
/**
 * A table filter is used to mask out certain rows from a table. All rows that are <i>not</i> accept
 * by a filter, are masked out. The remaining rows (i.e. rows that are accepted by all filters) can
 * be retrieved by {@link ITable#getFilteredRows()}. The rows themselves are still in the table! The methods
